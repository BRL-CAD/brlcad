	...
	while (i < height) {
		...
		for (j=0; j<width; j++) {
			...
			*/
			...
			/* make the region */
			mk_addmember(prototype, &wm_hd.l, WMOP_UNION);
			...
			mk_addmember(scratch, &scanlineList.l, WMOP_UNION);
		}
		...
		sprintf(scratch, "%d.c", i+1);
		mk_addmember(scratch, &allScanlineList.l, WMOP_UNION);
	}
