...
main(int argc, const char **argv)
{
	...
	retval = bg_2d_obr(&center, &u, &v, (const point2d_t *)pnts, n);
	if (retval) return -1;
	...
	retval = bg_3d_coplanar_obr(&center, &u, &v, (const point_t *)test2_points, 17);
	if (retval) return -1;
	...
	retval = bg_3d_coplanar_obr(&center, &u, &v, (const point_t *)test3_points, 17);
	if (retval) return -1;
	VADD3(output_pnts[2], center, u, v);
	VSCALE(u, u, -1);
	VADD3(output_pnts[3], center, u, v);
	...
	retval = bg_3d_coplanar_obr(&center, &u, &v, (const point_t *)test4_points, 9);
	if (retval) return -1;
	VADD3(output_pnts[2], center, u, v);
	VSCALE(u, u, -1);
	VADD3(output_pnts[3], center, u, v);
