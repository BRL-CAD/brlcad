...
struct creo_conv_info {
    ...
    double min_error;            /** Maximum allowable error in facetized approximation (mm) */
    double tol_dist;             /** Ninimum distance between two distinct vertices (mm) */
    ...
    /** CSG settings */
    double min_hole_diameter;    /** If > 0.0, all holes with lesser diameters will be deleted */
    double min_chamfer_dim;      /** If > 0.0, all chamfers with lesser dimensions will be deleted */ 
    double min_round_radius;     /** If > 0.0, all rounds with lesser radii will be deleted */

    /** Internal */
    struct db_i *dbip;           /** Output database */
    struct rt_wdb *wdbp;
    std::set<wchar_t *, WStrCmp> *parts;                            /** List of parts in Creo hierarchy */
    std::set<wchar_t *, WStrCmp> *assems;                           /** List of assemblies in Creo hierarchy */
    std::set<wchar_t *, WStrCmp> *empty;                            /** List of parts and assemblies that are empty */
    ...
    int warn_feature_unsuppress;                                    /** Flag to warn user when feature unsuppression fails */
};
