...
creo_log(struct creo_conv_info *cinfo, int msg_type, const char *fmt, ...) {
    /**
     * NOTE - need Creo specific semaphore lock for this if it's going to be
     * used in multi-threading situations... - probably can't use libbu's
     * logging safely
     */
...
/** 
 * Map a given string to the "stable" version of that string generated
 * by objects_gather
 */
extern "C" wchar_t *
...
get_brlcad_name(struct creo_conv_info *cinfo, wchar_t *name, const char *suffix, int flag)
{
    ...
    /**
     * If the name isn't in either the parts or assemblies sets,
     * it's not an active part of the conversion
     */
    if (!stable) {
