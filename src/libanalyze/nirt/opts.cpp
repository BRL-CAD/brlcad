...
dequeue_scripts(struct bu_vls *UNUSED(msg), size_t UNUSED(argc), const char **UNUSED(argv), void *set_var)
{
	...
	char *s = (char *)BU_PTBL_GET(init_scripts, i);
	bu_free(s, "script");
    }
...
nirt_opt_mk_args(struct bu_ptbl *tbl, struct nirt_opt_vals *tgt, struct nirt_opt_vals *src, int add_scripts)
{
	...
	str = bu_strdup("-H");
	bu_ptbl_ins(tbl, (long *)str);
	bu_vls_sprintf(&tmp, "%d", src->header_mode);
	...
	str = bu_strdup("-u");
	bu_ptbl_ins(tbl, (long *)str);
	bu_vls_sprintf(&tmp, "%d", src->use_air);
	str = bu_strdup(bu_vls_cstr(&tmp));
	bu_ptbl_ins(tbl, (long *)str);
    }
	...
	str = bu_strdup("-O");
	bu_ptbl_ins(tbl, (long *)str);
	bu_vls_sprintf(&tmp, "%d", src->overlap_claims);
	str = bu_strdup(bu_vls_cstr(&tmp));
	bu_ptbl_ins(tbl, (long *)str);
    }
	...
	str = bu_strdup("-X");
	bu_ptbl_ins(tbl, (long *)str);
	str = bu_strdup(bu_vls_cstr(&src->nirt_debug));
	bu_ptbl_ins(tbl, (long *)str);
    }
	...
	str = bu_strdup("--xyz");
	bu_ptbl_ins(tbl, (long *)str);
	bu_vls_sprintf(&tmp, "%0.17f,%0.17f,%0.17f", V3ARGS(src->center_model));
	str = bu_strdup(bu_vls_cstr(&tmp));
	bu_ptbl_ins(tbl, (long *)str);
    }
    ...
    if (add_scripts) {
	for (size_t i = 0; i < BU_PTBL_LEN(&src->init_scripts); i++) {
	    char *script = (char *)BU_PTBL_GET(&src->init_scripts, i);
	    str = bu_strdup("-e");
	    bu_ptbl_ins(tbl, (long *)str);
