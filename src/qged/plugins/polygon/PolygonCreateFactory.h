/*           P O L Y G O N C R E A T E F A C T O R Y . H
 * BRL-CAD
 *
 ...
 * information.
 */
/** @file PolygonCreateFactory.h
 *
 * Qt plugin factory + QgToolBase wrapper for polygon-creation tools.
 */

#ifndef POLYGONCREATEFACTORY_H
#define POLYGONCREATEFACTORY_H
...
class PolygonCreateTool : public QgToolBase, public IQgViewEventFilter
{
    Q_OBJECT
    Q_INTERFACES(IQgViewEventFilter)

    public:
	explicit PolygonCreateTool(QgPluginContext *ctx, QObject *parent = nullptr);
    ...
    private:
	QPolyCreate *m_create = nullptr;
	bool m_extra_wired = false;
};

class PolygonCreateFactory : public QObject, public IQgToolFactory
{
    Q_OBJECT
    Q_INTERFACES(IQgToolFactory)
    Q_PLUGIN_METADATA(IID "org.brlcad.qtcad.IQgToolFactory/1" FILE "polygon_create.json")
