...
motion_event_handler(XMotionEvent *xmotion)
{
	...
	case AMM_IDLE:
		...
		bu_vls_printf(&cmd, "M 1 %d %d\n",
			      (int)(dm_Xx2Normal(dmp, mx) * DM_MAX),
			      (int)(dm_Xy2Normal(dmp, my, 0) * DM_MAX));
	...
	case AMM_SCALE:
	    ...
	    break;
	case AMM_ADC_ANG1:
	    fx = dm_Xx2Normal(dmp, mx) * DM_MAX - adc_state->adc_dv_x;
	    fy = dm_Xy2Normal(dmp, my, 1) * DM_MAX - adc_state->adc_dv_y;
	...
	case AMM_ADC_TRAN:
	    ...
	    break;
	case AMM_ADC_DIST:
	    fx = (dm_Xx2Normal(dmp, mx) * DM_MAX - adc_state->adc_dv_x) * view_state->vs_gvp->gv_scale * base2local * INV_GED;
	    fy = (dm_Xy2Normal(dmp, my, 1) * DM_MAX - adc_state->adc_dv_y) * view_state->vs_gvp->gv_scale * base2local * INV_GED;
	    td = sqrt(fx * fx + fy * fy);
	...
	case AMM_CON_XADC:
	    ...
	    bu_vls_printf(&cmd, "knob -i xadc %f\n",
			  f / (fastf_t)dmp->dm_width * DM_RANGE);
	    break;
	case AMM_CON_YADC:
	    if (abs(dx) >= abs(dy))
		f = dx;
	    ...
	    bu_vls_printf(&cmd, "knob -i yadc %f\n",
			  f / (fastf_t)dmp->dm_height * DM_RANGE);
	    break;
	case AMM_CON_ANG1:
	    if (abs(dx) >= abs(dy))
		f = dx;
