...
pnts_in(int argc, char **argv, struct rt_db_internal *intern, char **prompt) {
    ...
    /* prompt for number of points if not provided */
    if (argc < 4) {
    ...
    /* prompt for color */
    if (argc < 6) {
	Tcl_AppendResult(interp, MORE_ARGS_STR, prompt[2], (char *)NULL);
	return CMD_MORE;
    }
    ...
    if (argc < 7) {
	Tcl_AppendResult(interp, MORE_ARGS_STR, prompt[3], (char *)NULL);
	return CMD_MORE;
    }
    ...
    if (argc < 8) {
	Tcl_AppendResult(interp, MORE_ARGS_STR, prompt[4], (char *)NULL);
	return CMD_MORE;
    }
	...
	Tcl_AppendResult(interp, MORE_ARGS_STR, bu_vls_addr(&vls), (char *)NULL);
	...
	switch (type) {
		...
		((struct pnt_color *)point)->c.buc_rgb[RED] = strtod(argv[i + 3], NULL);
		((struct pnt_color *)point)->c.buc_rgb[GRN] = strtod(argv[i + 4], NULL);
		((struct pnt_color *)point)->c.buc_rgb[BLU] = strtod(argv[i + 5], NULL);
		...
		((struct pnt_color_scale *)point)->c.buc_rgb[RED] = strtod(argv[i + 3], NULL);
		((struct pnt_color_scale *)point)->c.buc_rgb[GRN] = strtod(argv[i + 4], NULL);
		((struct pnt_color_scale *)point)->c.buc_rgb[BLU] = strtod(argv[i + 5], NULL);
		...
		((struct pnt_color_normal *)point)->c.buc_rgb[RED] = strtod(argv[i + 6], NULL);
		((struct pnt_color_normal *)point)->c.buc_rgb[GRN] = strtod(argv[i + 7], NULL);
		((struct pnt_color_normal *)point)->c.buc_rgb[BLU] = strtod(argv[i + 8], NULL);
		...
		((struct pnt_color_scale_normal *)point)->c.buc_rgb[RED] = strtod(argv[i + 6], NULL);
		((struct pnt_color_scale_normal *)point)->c.buc_rgb[GRN] = strtod(argv[i + 7], NULL);
		((struct pnt_color_scale_normal *)point)->c.buc_rgb[BLU] = strtod(argv[i + 8], NULL);
