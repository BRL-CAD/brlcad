...
arb8_edge(struct mged_state *s, int arg, int UNUSED(a), int UNUSED(b))
{
    ...
    if (arg == 12) {
	MEDIT(s)->edit_flag = ECMD_ARB_MAIN_MENU;
	sedit(s);
    }
...
{
    /* XXX Why wasn't this done by setting MEDIT(s)->edit_flag = ECMD_SPLINE_VPICK? */
    if (arg < 0) {
	/* Enter picking state */
	chg_state(s, ST_S_EDIT, ST_S_VPICK, "Vertex Pick");
	return;
    }
    /* For example, this will set MEDIT(s)->edit_flag = ECMD_VTRANS */
    MEDIT(s)->edit_flag = arg;
    sedit(s);
...
nmg_ed(struct mged_state *s, int arg, int UNUSED(a), int UNUSED(b))
{
	...
	case ECMD_NMG_LEXTRU:
		...
		m = (struct model *)MEDIT(s)->es_int.idb_ptr;
		NMG_CK_MODEL(m);
    ...
    /* For example, this will set MEDIT(s)->edit_flag = ECMD_NMG_EPICK */
    MEDIT(s)->edit_flag = arg;
    sedit(s);
}
...
{
    es_menu = arg - 1;
    MEDIT(s)->edit_flag = ECMD_ARB_SETUP_ROTFACE;
    if (arg == 7)
	MEDIT(s)->edit_flag = ECMD_ARB_MAIN_MENU;
...
{
    es_menu = arg;
    MEDIT(s)->edit_flag = ECMD_ARB_SPECIFIC_MENU;
    sedit(s);
}
...
pipe_ed(struct mged_state *s, int arg, int UNUSED(a), int UNUSED(b))
{
	...
	case MENU_PIPE_DEL_PT:
	    es_menu = arg;
	    MEDIT(s)->edit_flag = ECMD_PIPE_PT_DEL;
	    sedit(s);
	    break;
    }
    set_e_axes_pos(s, 1);
...
vol_ed(struct mged_state *s, int arg, int UNUSED(a), int UNUSED(b))
{
    ...
    switch (arg) {
	case MENU_VOL_FNAME:
	    MEDIT(s)->edit_flag = ECMD_VOL_FNAME;
	    break;
	case MENU_VOL_FSIZE:
	    MEDIT(s)->edit_flag = ECMD_VOL_FSIZE;
	    break;
	case MENU_VOL_CSIZE:
...
ebm_ed(struct mged_state *s, int arg, int UNUSED(a), int UNUSED(b))
{
    ...
    switch (arg) {
	case MENU_EBM_FNAME:
	    MEDIT(s)->edit_flag = ECMD_EBM_FNAME;
	    break;
	case MENU_EBM_FSIZE:
	    MEDIT(s)->edit_flag = ECMD_EBM_FSIZE;
	    break;
...
dsp_ed(struct mged_state *s, int arg, int UNUSED(a), int UNUSED(b))
{
    ...
    switch (arg) {
	case MENU_DSP_FNAME:
	    MEDIT(s)->edit_flag = ECMD_DSP_FNAME;
	    break;
	case MENU_DSP_FSIZE:
	    MEDIT(s)->edit_flag = ECMD_DSP_FSIZE;
	    break;
...
metaball_ed(struct mged_state *s, int arg, int UNUSED(a), int UNUSED(b))
{
	...
	case MENU_METABALL_MOV_PT:
	    ...
	    es_menu = arg;
	    MEDIT(s)->edit_flag = ECMD_METABALL_PT_MOV;
	    sedit(s);
	    break;
	...
	case MENU_METABALL_DEL_PT:
	    es_menu = arg;
	    MEDIT(s)->edit_flag = ECMD_METABALL_PT_DEL;
	    sedit(s);
	    break;
	case MENU_METABALL_ADD_PT:
	    es_menu = arg;
	    MEDIT(s)->edit_flag = ECMD_METABALL_PT_ADD;
	    break;
    }
    set_e_axes_pos(s, 1);
    return;
...
mmenu_set(struct mged_state *s, int index, struct menu_item *value)
{
	...
	if (menu_state == dlp->dm_menu_state &&
	    ...
	    mged_dm_repaint_request(dlp, MGED_REPAINT_DEVICE_SETTING);
	}
    }
}
...
mmenu_display(struct mged_state *s, int y_top)
{
    ...
    dm_draw_line_2d(DMP,
		    ...
		    GED2PM1((int)BSG_VIEW_MIN), GED2PM1(menu_state->ms_top));
	...
	for (item=0, mptr = *m; mptr->menu_string[0] != '\0' && y > TITLE_YBASE; mptr++, y += MENU_DY, item++) {
	    ...
	    dm_draw_line_2d(DMP,
			    ...
			    GED2PM1((int)BSG_VIEW_MIN), GED2PM1(y+(MENU_DY/2)));
		...
		dm_draw_string_2d(DMP, "==>",
				  GED2PM1((int)BSG_VIEW_MIN), GED2PM1(y-15), 0, 0);
	    }
	}
    }
