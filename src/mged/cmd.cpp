...
cmd_ged_gqa(ClientData clientData, Tcl_Interp *interpreter, int argc, const char *argv[])
{
    ...
    int option;
    int ret;
    bool legacy_plot_in_view = false;
    struct cmdtab *ctp = (struct cmdtab *)clientData;
    MGED_CK_CMD(ctp);
    struct mged_state *s = ctp->s;
    size_t args;
	...
	*vp = 0;
    ...
    /* Legacy -A p mutates view objects directly and must remain on the GUI
     * thread.  The experimental --plot-view path uses GED's main-thread
     * callback and may run asynchronously with the other analyses. */
    if (legacy_plot_in_view)
