 ...
 * C M D _ G L O B
 *
 * Assist routine for command processor.  If the current word in the
 * argv[] array contains '*', '?', '[', or '\' then this word is
 * potentially a regular expression, and we will tromp through the
 ...
 * are no matches, but there are escapes, the current word is
 * modified.  All matches are sought for, up to the limit of the
 * argv[] array.
 *
...
cmd_glob(int *argcp, char **argv, int maxargs)
{
    ...
    /* Search for pattern matches.  First, save the pattern (in word),
     * and remove it from argv, as it will be overwritten by the
     * expansions.  If any matches are found, we do not have to worry
     * about '\' escapes since the match coming from dp->d_namep is
     * placed into argv.  Only in the case of no matches do we have to
     * do escape crunching.
     */
