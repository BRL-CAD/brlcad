...
#define PARAM_1ARG (s->s_edit->edit_flag == RT_PARAMS_EDIT_SCALE || \
...
set_e_axes_pos_clbk(int UNUSED(ac), const char **UNUSED(av), void *d, void *id)
{
	...
	case ID_ARB8:
	    ...
	    MAT4X3PNT(s->s_edit->curr_e_axes_pos, s->s_edit->e_mat,
	...
	case ID_REC:
		...
		MAT4X3PNT(tgc_v, s->s_edit->e_mat, tgc->v);
		MAT4X3VEC(tgc_h, s->s_edit->e_mat, tgc->h);
...
init_sedit_vars(struct mged_state *s)
{
    ...
    bv_knobs_reset(&s->s_edit->k, BV_KNOBS_ALL);
...
sedit_menu(struct mged_state *s) {
    ...
    s->s_edit->edit_flag = RT_EDIT_IDLE;	/* Drop out of previous edit mode */
    es_menu = 0;
}
...
sedit_abs_scale(struct mged_state *s)
{
    ...
    if (s->s_edit->edit_flag != RT_PARAMS_EDIT_SCALE && s->s_edit->edit_flag != RT_PARAMS_EDIT_PSCALE)
	return;
...
objedit_mouse(struct mged_state *s, const vect_t mousevec)
{
    ...
    // incr_change was initialized to identity at the beginning of this logic,
    // and all code paths reset it to IDN after work is complete.  Just use a
    // local matrix rather than the s_edit version.
    mat_t incr_mat;
    MAT_IDN(incr_mat);

    // Since objedit_mouse always resulted in an IDN incr_change in sedit, go
    // ahead and make sure that's still true in case something is depending
    // on that behavior.
    MAT_IDN(s->s_edit->incr_change);
