...
init_sedit(struct mged_state *s)
{
    ...
    rt_edit_set_edflag(s->s_edit, RT_EDIT_IDLE);
...
sedit_menu(struct mged_state *s) {
    ...
    rt_edit_set_edflag(s->s_edit, RT_EDIT_IDLE);	/* Drop out of previous edit mode */
}
 ...
 * = 1, set sedraw = 1, and return, allowing rt_edit_process(s) to
...
objedit_mouse(struct mged_state *s, const vect_t mousevec)
{
	...
	switch (edobj) {
	    case BE_O_SCALE:
		s->s_edit->edit_flag = RT_MATRIX_EDIT_SCALE;
		break;
	    case BE_O_XSCALE:
	...
	if (use_x && !use_y)
	    s->s_edit->edit_flag = RT_MATRIX_EDIT_TRANS_VIEW_X;
	if (!use_x && use_y)
	    s->s_edit->edit_flag = RT_MATRIX_EDIT_TRANS_VIEW_Y;
	if (use_x && use_y)
	    s->s_edit->edit_flag = RT_MATRIX_EDIT_TRANS_VIEW_XY;
	MAT_COPY(s->s_edit->model2objview, view_state->vs_model2objview);
    } else {
