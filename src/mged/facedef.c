...
f_facedef(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
    ...
    if (rt_arb_calc_planes(&error_msg, arb, s->edit_state.e_type, planes, &s->tol.tol)) {
	...
	case 24:plane=0;	/* face 1234 of arb8 */
	    ...
	    /* face 1234 of arb6 */
	    /* face 1234 of arb5 */
	    if (s->edit_state.e_type==4 && prod==24)
	...
	case 'c':
	    /* special case for arb7, because of 2 4-pt planes meeting */
	    if (s->edit_state.e_type == 7 && (plane != 0 && plane != 3)) {
		if (argc < 5) {
		    /* total # of args under this option */
    ...
    if (rt_arb_calc_points(arb, s->edit_state.e_type, (const plane_t *)planes, &s->tol.tol) < 0) {
