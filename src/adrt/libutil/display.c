  ...
  while (SDL_WaitEvent(&event) >= 0) {
	...
	switch (event.key.keysym.sym) {
	  ...
	  default:
		...
		case SDLK_x: /* cut */
		  bu_strlcpy(paste, content_buffer[v_ind], sizeof(paste));
		  for (i = v_ind; i < *content_lines; i++)
		    bu_strlcpy(content_buffer[i], content_buffer[i+1], 80);
		  if (*content_lines) {
		    (*content_lines)--;
		  } else {
		    content_buffer[0][0] = 0;
		...
		case SDLK_v: /* paste */
		  for (i = *content_lines; i >= v_ind; i--)
		    bu_strlcpy(content_buffer[i+1], content_buffer[i], 80);
		  bu_strlcpy(content_buffer[v_ind], paste, 80);
		  (*content_lines)++;
		  h_ind = 0;
		  break;
