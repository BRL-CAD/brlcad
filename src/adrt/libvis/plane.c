...
void* vis_plane_hit(tie_ray_t *ray, tie_id_t *id, tie_tri_t *tri, void *ptr);
void vis_plane(tie_t *tie, tie_ray_t *ray, TIE_3 *pixel);


typedef struct vis_plane_hit_s {
  tie_id_t id;
  common_mesh_t *mesh;
  tfloat plane[4];
  tfloat mod;
...
void vis_plane_init(vis_t *vis, TIE_3 ray_pos, TIE_3 ray_dir) {
  ...
  vis->work = vis_plane_work;
  vis->free = vis_plane_free;

  vis->data = (vis_plane_t *)malloc(sizeof(vis_plane_t));
  d = (vis_plane_t *)vis->data;
...
static void* vis_arrow_hit(tie_ray_t *ray, tie_id_t *id, tie_tri_t *tri, void *ptr) {
  return(tri);
}


void* vis_plane_hit(tie_ray_t *ray, tie_id_t *id, tie_tri_t *tri, void *ptr) {
  vis_plane_hit_t *hit = (vis_plane_hit_t *)ptr;
...
void vis_plane_work(vis_t *vis, tie_t *tie, tie_ray_t *ray, TIE_3 *pixel) {
  ...
  rd = (vis_plane_t *)vis->data;

  /* Draw Ballistic Arrow - Blue */
  if(tie_work(&rd->tie, ray, &id, vis_arrow_hit, NULL)) {
    pixel->v[0] = 0.0;
    pixel->v[1] = 0.0;
    pixel->v[2] = 1.0;
    return;
  ...
  */
  ...
  /* vis Geometry */
  if(!tie_work(tie, ray, &id, vis_plane_hit, &hit))
    return;
