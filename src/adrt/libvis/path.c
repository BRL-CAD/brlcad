...
void vis_path_init(vis_t *vis, int samples) {
  ...
  vis->work = vis_path_work;
  vis->free = vis_path_free;
  vis->data = (vis_path_t *)malloc(sizeof(vis_path_t));
  d = (vis_path_t *)vis->data;
  d->samples = samples;
...
void vis_path_work(vis_t *vis, tie_t *tie, tie_ray_t *ray, TIE_3 *pixel) {
  ...
  for(i = 0; i < rd->samples; i++) {
    ...
    while(propogate) {
      if((new_mesh = (common_mesh_t *)tie_work(tie, &new_ray, &new_id, vis_hit, NULL)) && new_ray.depth < VIS_MAX_DEPTH) {
