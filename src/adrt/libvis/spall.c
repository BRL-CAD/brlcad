...
void* vis_spall_hit(tie_ray_t *ray, tie_id_t *id, tie_tri_t *tri, void *ptr);
void vis_plane(tie_t *tie, tie_ray_t *ray, TIE_3 *pixel);


typedef struct vis_spall_hit_s {
  tie_id_t id;
  common_mesh_t *mesh;
  tfloat plane[4];
  tfloat mod;
...
void vis_spall_init(vis_t *vis, TIE_3 ray_pos, TIE_3 ray_dir, tfloat angle) {
  ...
  vis->work = vis_spall_work;
  vis->free = vis_spall_free;

  vis->data = (vis_spall_t *)malloc(sizeof(vis_spall_t));
  d = (vis_spall_t *)vis->data;
  ...
  vis_util_spall_vec(ray_dir, angle, TESSELATION, vec_list);
...
void* vis_spall_hit(tie_ray_t *ray, tie_id_t *id, tie_tri_t *tri, void *ptr) {
  vis_spall_hit_t *hit = (vis_spall_hit_t *)ptr;
...
void vis_spall_work(vis_t *vis, tie_t *tie, tie_ray_t *ray, TIE_3 *pixel) {
  ...
  */
  ...
  /* vis Geometry */
  if(!tie_work(tie, ray, &id, vis_spall_hit, &hit))
    return;
