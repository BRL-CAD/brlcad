 ...
 * platforms.
 *
 * This is a re-implementation of a core subset of the original libtermio
 * functionality with an eye towards portability across many operating
 * systems, including hiding the various complexities involved with working
 * in multiple Windows command prompt environments.
 *
 * To provide a compatibility layer for BRL-CAD code using the original
 * libtermio API, we have a number of LEGACY definitions set up.  If a
 * caller does not want those defined for any reason, they can define
 * LIBTERMIO_NO_LEGACY_NAMES at build time.
 *
 ...
 *
 * Features:
 *  - ASCII oriented key input (printable 0x20..0x7E).
 *  - Navigation and function keys (F1–F12, arrows, PgUp/PgDn, Home/End,
 *    Insert/Delete) normalized across POSIX & Windows.
 *  - Windows console raw mode configuration with disabling of:
 *      * line input, echo, quick edit, mouse input, insert mode
 *  - Pipe / ConPTY / mintty path with ESC sequence state machine:
 *      * Filters mouse tracking (X10, SGR) & bracketed paste markers (200~,201~)
 *      * Prevents leaked partial sequences
 *  - Optional auto stty raw fallback under mintty when running a native
 *    Win32/MSVC build so you still get unbuffered single-character input.
 *  - Runtime sanitization helper to (re)disable mouse & paste modes.
 *  - Startup drain to remove pre-existing queued mouse/paste sequences.
 *
...
#ifndef LIBTERMIO_NO_LEGACY_NAMES
static inline void save_Tty(int fd){ termio_save_tty(fd); }
static inline void reset_Tty(int fd){ termio_reset_tty(fd); }
static inline void clr_Echo(int fd){ termio_clear_echo(fd); }
static inline void set_Cbreak(int fd){ termio_set_cbreak(fd); }
static inline void set_Raw(int fd){ termio_set_raw(fd); }
#endif
...
/* ================= Implementation ================= */
#if defined(LIBTERMIO_IMPLEMENTATION) && !defined(LIBTERMIO_IMPL_DONE)
#define LIBTERMIO_IMPL_DONE
#ifdef __cplusplus
extern "C" {
#endif
...
static struct termios termio_saved_tio;
static int termio_saved_flag = 0;
...
void termio_save_tty(int fd){
    if (tcgetattr(fd,&termio_saved_tio)==0){
        termio_saved_flag=1;
...
void termio_reset_tty(int fd){
    if (termio_saved_flag)
        tcsetattr(fd,TCSANOW,&termio_saved_tio);
    write(STDOUT_FILENO, TERMIO_RESET_SEQ, sizeof(TERMIO_RESET_SEQ)-1);
    termio_debug_shutdown();
}
