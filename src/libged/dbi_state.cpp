...
std::string
CombInst::instance_name()
{
    return std::string("");
}


GObj::GObj(struct db_i *dbip, struct directory *dp, struct ged_draw_cache *dcache)
{
    if (!dp || !dcache)
	return;
    ...
    if (bsize == sizeof(rflag)) {
	memcpy(&rflag, b, sizeof(rflag));
