...
ged_eye_pos(struct ged *gedp, int argc, const char *argv[])
{
    ...
    /* get eye position */
    if (argc == 1) {
	VSCALE(eye_pos, gedp->dm_gvp->gv_eye_pos, gedp->ged_wdbp->dbip->dbi_base2local);
    ...
    VSCALE(gedp->dm_gvp->gv_eye_pos, eye_pos, gedp->ged_wdbp->dbip->dbi_local2base);

    /* update perspective matrix */
    ged_mike_persp_mat(gedp->dm_gvp->gv_pmat, gedp->dm_gvp->gv_eye_pos);
