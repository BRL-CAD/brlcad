      ...
      plane_t planes[6])
{
	...
	bot = (struct rt_bot_internal *)nmg_bot(s, &rtg_vlfree, &gedp->ged_wdbp->wdb_tol);
	nmg_km(m);
...
nmgin(struct ged *gedp, struct rt_db_internal *ip, fastf_t thick)
{
	...
	while (BU_LIST_NOT_HEAD(s, &r->s_hd)) {
	    ...
	    nmg_shell_coplanar_face_merge(s, &gedp->ged_wdbp->wdb_tol, 1, &rtg_vlfree);
	    if (!nmg_kill_cracks(s))
		(void)nmg_extrude_shell(s, thick, 0, 0, &rtg_vlfree, &gedp->ged_wdbp->wdb_tol);
