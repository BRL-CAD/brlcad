...
static void increment_uses(struct db_i *db_ip, struct directory *dp, genptr_t ptr);
static void increment_nrefs(struct db_i *db_ip, struct directory *dp, genptr_t ptr);
static void Free_uses(struct db_i *dbip);
static void Make_new_name(struct db_i *dbip, struct directory *dp, genptr_t ptr);
static void Do_copy_membs(struct db_i *dbip, struct rt_comb_internal *comb, union tree *comb_leaf, genptr_t user_ptr1, genptr_t user_ptr2, genptr_t user_ptr3);
static struct directory *Copy_solid(struct ged *gedp, struct directory *dp, mat_t xform);
static struct directory *Copy_comb(struct ged *gedp, struct directory *dp, mat_t xform);
