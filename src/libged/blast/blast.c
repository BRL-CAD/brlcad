	...
	struct ged_cmd_validate_result *result)
{
    ...
    ret = ged_cmd_validate(gedp, bu_vls_cstr(&line), draw_cursor,
	&delegated);
    if (!ret) {
	...
	*result = delegated;
	...
	delegated.completion_common_prefix = NULL;
...
{
    struct bu_vls line = BU_VLS_INIT_ZERO;
    struct ged_cmd_analysis delegated = GED_CMD_ANALYSIS_NULL;
    int ret = -1;
...
static char *
blast_grammar_help(const char *invocation)
{
    return ged_cmd_help("draw", invocation);
}


static const struct ged_cmd_grammar blast_grammar = {
    "blast", "Clear the display and draw objects", blast_grammar_validate,
    blast_grammar_analyze, blast_grammar_json, blast_grammar_lint, NULL,
    blast_grammar_help
};
