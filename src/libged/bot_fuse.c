...
ged_bot_fuse(struct ged *gedp, int argc, const char **argv)
{
    ...
    ret = rt_bot_tess(&s, &intern, &gedp->ged_wdbp->wdb_ttol, tol);
    ...
    count = nmg_vertex_fuse(&s->magic, tol);
    total += count;
    ...
    count = nmg_break_e_on_v(&s->magic, tol);
    total += count;
    ...
    bu_log("%s: running nmg_shell_face_fuse\n", argv[0]);
    count = nmg_shell_face_fuse(s, tol);
    total += count;
    bu_log("%s: %s, %d faces fused\n", argv[0], argv[i+1], count);

    /* Step 3 -- edges */
    bu_log("%s: running nmg_edge_fuse\n", argv[0]);
    count = nmg_edge_fuse(&s->magic, tol);
    total += count;
	...
	open_cnt = show_dangling_edges(gedp, &s->magic, bu_vls_addr(&name_prefix), out_type);
