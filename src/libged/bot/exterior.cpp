...
static const struct bu_cmd_option bot_exterior_options[] = {
    ...
    BU_CMD_NUMBER_RANGE(NULL, "visibility-threshold", struct bot_exterior_args,
	visibility_threshold, 0.0, 1.0, "ratio",
...
static const char * const bot_exterior_in_place_option[] = {"in-place", NULL};
static const struct bu_cmd_constraint bot_exterior_constraints[] = {
    BU_CMD_CONSTRAINT_OPERANDS(BU_CMD_CONDITION_ANY_OPTION_PRESENT,
	bot_exterior_in_place_option, 1, 1,
	"--in-place requires exactly one input BoT"),
...
extern "C" const struct bu_cmd_schema ged_bot_exterior_schema = {
    ...
    BU_CMD_SCHEMA_META_HELP(NULL, bot_exterior_constraints, NULL, NULL, NULL)
};
...
{
    char *help = bu_cmd_schema_help(&ged_bot_exterior_schema, cmd);
    if (help) {
