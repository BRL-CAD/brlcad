	...
	void *client_data)
{
    ...
    ret = rt_db_get_internal(&intern, dp, gbdcdp->gedp->dbip, mat);
...
extern "C" const struct bu_cmd_schema ged_bot_dump_schema = {
    ...
    BU_CMD_SCHEMA_META_HELP(NULL, bot_dump_constraints, NULL, NULL, NULL)
};
...
{
    bu_vls_trunc(str, 0);
    (void)bu_cmd_schema_help_append(str, &ged_bot_dump_schema, cmd);
}
...
ged_bot_dump_core(struct ged *gedp, int argc, const char *argv[])
{
	...
	FOR_ALL_DIRECTORY_START(dp, gedp->dbip) {
	    ...
	    /* get the internal form */
	    i = rt_db_get_internal(&intern, dp, gedp->dbip, mat);
	    if (i < 0) {
