...
static int
bot_split_schema_validate(const struct bu_cmd_schema *schema, size_t argc,
...
const struct bu_cmd_schema ged_bot_split_schema = {
    ...
    BU_CMD_SCHEMA_CONSTRAINTS(bot_split_schema_validate, NULL)
};

static void
bot_split_usage(struct bu_vls *result, const char *cmd)
{
    char *option_help = bu_cmd_schema_describe(&ged_bot_split_schema);
...
ged_bot_split_core(struct ged *gedp, int argc, const char *argv[])
{
    ...
    argc--; argv++;
    if (!argc) {
	bot_split_usage(gedp->ged_result_str, cmd);
	return GED_HELP;
    }
