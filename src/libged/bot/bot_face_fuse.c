...
struct bot_face_fuse_args {
    int print_help;
};

static int
bot_face_fuse_schema_validate(const struct bu_cmd_schema *schema, size_t argc,
...
static const struct bu_cmd_option bot_face_fuse_options[] = {
    BU_CMD_FLAG("h", "help", struct bot_face_fuse_args, print_help,
	"Print command help"),
    BU_CMD_OPTION_NULL
};
static const struct bu_cmd_operand bot_face_fuse_operands[] = {
...
const struct bu_cmd_schema ged_bot_face_fuse_schema = {
    "bot_face_fuse", "Fuse duplicate BoT faces", bot_face_fuse_options,
    bot_face_fuse_operands, BU_CMD_PARSE_INTERSPERSED,
    BU_CMD_SCHEMA_CONSTRAINTS(bot_face_fuse_schema_validate, NULL)
};

static void
bot_face_fuse_usage(struct bu_vls *result, const char *cmd)
{
    char *option_help = bu_cmd_schema_describe(&ged_bot_face_fuse_schema);
...
ged_bot_face_fuse_core(struct ged *gedp, int argc, const char *argv[])
{
    ...
    struct bot_face_fuse_args args = {0};
    int operand_index;

    argc--; argv++;
    if (!argc) {
	bot_face_fuse_usage(gedp->ged_result_str, cmd);
	return GED_HELP;
    }

    operand_index = bu_cmd_schema_parse_complete(&ged_bot_face_fuse_schema,
