...
static int
bot_flip_schema_validate(const struct bu_cmd_schema *schema, size_t argc,
...
const struct bu_cmd_schema ged_bot_flip_schema = {
    ...
    BU_CMD_SCHEMA_CONSTRAINTS(bot_flip_schema_validate, NULL)
};

static void
bot_flip_usage(struct bu_vls *result, const char *cmd)
{
    char *option_help = bu_cmd_schema_describe(&ged_bot_flip_schema);
