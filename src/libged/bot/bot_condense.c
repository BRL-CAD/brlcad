...
struct bot_condense_args {
    int print_help;
};

static int
bot_condense_schema_validate(const struct bu_cmd_schema *schema, size_t argc,
...
static const struct bu_cmd_option bot_condense_options[] = {
    BU_CMD_FLAG("h", "help", struct bot_condense_args, print_help,
	"Print command help"),
    BU_CMD_OPTION_NULL
};
static const struct bu_cmd_operand bot_condense_operands[] = {
    BU_CMD_OPERAND("new_bot", BU_CMD_VALUE_STRING, 1, 1,
	"Name for the condensed BoT", NULL),
    BU_CMD_OPERAND("old_bot", BU_CMD_VALUE_DB_OBJECT, 1, 1,
	"Source BoT object", "ged.db_object"),
    BU_CMD_OPERAND_NULL
};
const struct bu_cmd_schema ged_bot_condense_schema = {
    "bot_condense", "Condense unused BoT vertices", bot_condense_options,
    bot_condense_operands, BU_CMD_PARSE_INTERSPERSED,
    BU_CMD_SCHEMA_CONSTRAINTS(bot_condense_schema_validate, NULL)
};

static void
bot_condense_usage(struct bu_vls *result, const char *cmd)
{
    char *option_help = bu_cmd_schema_describe(&ged_bot_condense_schema);
...
ged_bot_condense_core(struct ged *gedp, int argc, const char *argv[])
{
    ...
    struct bot_condense_args args = {0};
    int operand_index;

    argc--; argv++;
    if (!argc) {
	bot_condense_usage(gedp->ged_result_str, cmd);
	return GED_HELP;
    }

    operand_index = bu_cmd_schema_parse_complete(&ged_bot_condense_schema,
