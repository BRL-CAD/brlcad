...
_select_cmd_list(void *bs, int argc, const char **argv)
{
    ...
    if (!argc) {
	std::vector<std::string> ssets = dbis->FindSelectionSets();
    ...
    std::vector<std::string> ss = dbis->FindSelectionSets(sname);
    if (!ss.size()) {
	...
	BSelectState *s = dbis->GetSelectionSet(ss[i].c_str());
...
_select_cmd_clear(void *bs, int argc, const char **argv)
{
    ...
    std::vector<std::string> ssets = dbis->FindSelectionSets(sname);
    if (!ssets.size()) {
	if (sname)
	    bu_vls_printf(gedp->ged_result_str, ": %s does not match any selection sets\n", sname);
	return BRLCAD_ERROR;
    ...
    for (size_t i = 0; i < ssets.size(); i++) {
	BSelectState *s = dbis->GetSelectionSet(ssets[i].c_str());
	if (!s)
	    continue;
	s->Clear();
    }
...
_select_cmd_add(void *bs, int argc, const char **argv)
{
    ...
    std::vector<std::string> ss = dbis->FindSelectionSets(sname);
    BSelectState *s = (ss.size() == 1) ? dbis->GetSelectionSet(ss[0].c_str()) : NULL;
    if (!s) {
	if (sname)
