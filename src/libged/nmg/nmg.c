...
static const struct bu_cmd_form nmg_native_forms[] = {
    BU_CMD_FORM_TREE("subcommands", "Named NMG operations", &nmg_tree),
    BU_CMD_FORM_SCHEMA("legacy_topology", "Legacy object/operation syntax",
	&nmg_root_schema),
    BU_CMD_FORM_NULL
};

static const struct bu_cmd_form *
nmg_select_native_form(const struct bu_cmd_forms *forms, size_t argc,
	const char * const *argv, void *UNUSED(context))
{
    const char *word = argc > 1 ? argv[1] : "";
    size_t length = word ? strlen(word) : 0;

    if (!length || !bu_strncmp("mm", word, length))
	return &forms->forms[0];
    return &forms->forms[1];
}

static const struct bu_cmd_forms nmg_forms =
    BU_CMD_FORMS("nmg", "Edit NMG topology or create an empty NMG",
	nmg_native_forms, nmg_select_native_form);

static int
nmg_grammar_validate(struct ged *gedp, const char *input, size_t cursor_pos,
	struct ged_cmd_validate_result *result)
{
    return ged_cmd_native_forms_validate(gedp, &nmg_forms, input,
	cursor_pos, result);
}
...
{
    return ged_cmd_native_forms_analyze(gedp, &nmg_forms, input, analysis);
}

static char *
nmg_grammar_json(void)
{
    return ged_cmd_native_forms_describe_json(&nmg_forms);
}

static int
nmg_grammar_lint(struct bu_vls *msgs)
{
    return ged_cmd_native_forms_lint(&nmg_forms, msgs);
}

static char *
nmg_grammar_help(const char *invocation)
{
    return ged_cmd_native_forms_help(&nmg_forms, invocation);
}

static const struct ged_cmd_grammar nmg_grammar = {
    "nmg", "Edit NMG topology or create an empty NMG", nmg_grammar_validate,
    nmg_grammar_analyze, nmg_grammar_json, nmg_grammar_lint, NULL,
    nmg_grammar_help
};
