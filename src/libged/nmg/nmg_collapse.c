...
ged_nmg_collapse_core(struct ged *gedp, int argc, const char *argv[])
{
    ...
    /* triangulate model */
    nmg_triangulate_model(m, &RTG.rtg_vlfree, &gedp->ged_wdbp->dbip->db_tol);

    count = (size_t)nmg_edge_collapse(m, &gedp->ged_wdbp->dbip->db_tol, tol_coll, min_angle, &RTG.rtg_vlfree);
