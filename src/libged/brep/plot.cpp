...
int
brep_trim_plot(struct bu_vls *vls, const ON_Brep *brep, struct bn_vlblock *vbp, int index, struct bu_color *color, int plotres, bool dim3d)
{
    ON_wString wstr;
    ON_TextLog tl(wstr);
...
brep_plot(struct _ged_brep_info *gb, int argc, const char **argv)
{
	...
	if (BU_STR_EQUAL(part, "S")) {
	    ...
	    for (e_it = elements.begin(); e_it != elements.end(); e_it++) {
		ret = brep_facecdt_plot(vls, solid_name, ttol, tol, brep, vbp, (*e_it), 0);
	    }
	    ...
	    snprintf(commtag, 64, "_BC_FCDT_");
	    for (e_it = elements.begin(); e_it != elements.end(); e_it++) {
		ret = brep_facecdt_plot(vls, solid_name, ttol, tol, brep, vbp, (*e_it), 1, numpoints);
	    }
	} else if (BU_STR_EQUAL(part, "FCDT2d")) {
	    snprintf(commtag, 64, "_BC_FCDT2d_");
	    for (e_it = elements.begin(); e_it != elements.end(); e_it++) {
		ret = brep_facecdt_plot(vls, solid_name, ttol, tol, brep, vbp, (*e_it), 2);
	    }
	} else if (BU_STR_EQUAL(part, "FCDTm2d")) {
	    snprintf(commtag, 64, "_BC_FCDTm2d_");
	    for (e_it = elements.begin(); e_it != elements.end(); e_it++) {
		ret = brep_facecdt_plot(vls, solid_name, ttol, tol, brep, vbp, (*e_it), 3, numpoints);
	    }
	} else if (BU_STR_EQUAL(part, "FCDTp2d")) {
	    snprintf(commtag, 64, "_BC_FCDTp2d_");
	    for (e_it = elements.begin(); e_it != elements.end(); e_it++) {
		ret = brep_facecdt_plot(vls, solid_name, ttol, tol, brep, vbp, (*e_it), 4, numpoints);
	    }
