...
read_units_double(double *val, char *buf, const struct cvt_tab *cvt)
{
	...
	*val = a;
	...
	bu_vls_printf(&ged_current_gedp->ged_result_str, "Bad units specifier \"%s\" on value \"%s\"\n", units_string, buf);
	return 1;
	...
	*val = a * cvt->val;
    ...
    bu_vls_printf(&ged_current_gedp->ged_result_str, "%s sscanf problem on \"%s\"  got %d\n", BU_FLSTR, buf, i);
    return 1;
}
	...
	switch (c) {
	    ...
	    case 'g'	:
		{
			...
			*p++ = '\0';
		    ...
		    bu_vls_printf(&ged_current_gedp->ged_result_str, "set grid spacing:%g %s limit:%g %s\n",
		...
		if (sscanf(bu_optarg, "%lg", &a) != 1 || a <= 1.0) {
		    bu_vls_printf(&ged_current_gedp->ged_result_str, "error in specifying minimum samples per model axis: \"%s\"\n", bu_optarg);
		    break;
		}
	    ...
	    case 'u'	:
		{
		    ...
		    bu_vls_printf(&ged_current_gedp->ged_result_str, "Units: ");
		    for (i=0; i < 3; i++) {
			bu_vls_printf(&ged_current_gedp->ged_result_str, " %s: %s", dim[i], units[i]->name);
		    }
...
parse_densities_buffer(char *buf, unsigned long len)
{
	...
	if (densities[idx].magic == DENSITY_MAGIC)
	    bu_vls_printf(&ged_current_gedp->ged_result_str, "%4d %6g %s\n",
			  idx,
...
#if 1
	bu_vls_printf(&ged_current_gedp->ged_result_str, "Could not open file - %s\n", name);
#else
	perror(name);
#endif
	return GED_ERROR;
...
#if 1
	bu_vls_printf(&ged_current_gedp->ged_result_str, "Could not fstat file - %s\n", name);
#else
	perror(name);
#endif
	return GED_ERROR;
...
list_report(struct region_pair *list)
{
    ...
    bu_vls_printf(&ged_current_gedp->ged_result_str, "list %s:\n", (char *)list->r.name);

    for (BU_LIST_FOR (rp, region_pair, &(list->l))) {
	if (rp->r2) {
	    bu_vls_printf(&ged_current_gedp->ged_result_str, "%s %s count:%lu dist:%g%s @ (%g %g %g)\n",
			  rp->r.r1->reg_name, rp->r2->reg_name, rp->count,
			  rp->max_dist / units[LINE]->val, units[LINE]->name, V3ARGS(rp->coord));
	} else {
	    bu_vls_printf(&ged_current_gedp->ged_result_str, "%s count:%lu dist:%g%s @ (%g %g %g)\n",
...
options_prep(struct rt_i *rtip, vect_t span)
{
	...
	bu_vls_printf(&ged_current_gedp->ged_result_str, "Grid spacing %g %s is does not allow %g samples per axis\n",
		      gridSpacing / units[LINE]->val, units[LINE]->name, Samples_per_model_axis - 1);

	bu_vls_printf(&ged_current_gedp->ged_result_str, "Adjusted to %g %s to get %g samples per model axis\n",
	...
	if (volume_tolerance == -1.0) {
	    ...
	    bu_vls_printf(&ged_current_gedp->ged_result_str, "setting volume tolerance to %g %s\n",
			  volume_tolerance / units[VOL]->val, units[VOL]->name);
	} else
	    bu_vls_printf(&ged_current_gedp->ged_result_str, "volume tolerance   %g\n", volume_tolerance);
	if (plot_files)
	...
	if (weight_tolerance == -1.0) {
	    ...
	    bu_vls_printf(&ged_current_gedp->ged_result_str, "setting weight tolerance to %g %s\n",
			  weight_tolerance / units[WGT]->val,
			  units[WGT]->name);
	} else {
	    bu_vls_printf(&ged_current_gedp->ged_result_str, "weight tolerance   %g\n", weight_tolerance);
	}
    }
    if (analysis_flags & ANALYSIS_GAP) {
	char *name = "gaps.pl";
	...
	if (overlap_tolerance != 0.0)
	    bu_vls_printf(&ged_current_gedp->ged_result_str, "overlap tolerance to %g\n", overlap_tolerance);
	if (plot_files) {
	...
	if ((analysis_flags & (ANALYSIS_VOLUME|ANALYSIS_WEIGHT)) == 0)
	    bu_vls_printf(&ged_current_gedp->ged_result_str, "Note: -r option ignored: neither volume or weight options requested\n");
...
view_reports(struct cstate *state)
{
	...
	for (obj=0; obj < num_objects; obj++) {
		...
		if (verbose)
		    bu_vls_printf(&ged_current_gedp->ged_result_str, "\t%s volume %g %s\n",
				  obj_tbl[obj].o_name,
				  val / units[VOL]->val,
				  units[VOL]->name);
	    }
...
weight_volume_terminate(struct cstate *state)
{
	...
	for (obj=0; obj < num_objects; obj++) {
	    int view = 0;
	    if (verbose)
		bu_vls_printf(&ged_current_gedp->ged_result_str, "object %d\n", obj);
	    ...
	    if (verbose)
		bu_vls_printf(&ged_current_gedp->ged_result_str, "\t%s weight %g %s +%g -%g\n",
			      obj_tbl[obj].o_name,
			      val / units[WGT]->val,
			      units[WGT]->name,
		...
		can_terminate = 0;
		if (verbose)
		    bu_vls_printf(&ged_current_gedp->ged_result_str, "\t%s differs too much in weight per view.\n",
				  obj_tbl[obj].o_name);
	    }
	}
	if (can_terminate) {
	    if (verbose)
		bu_vls_printf(&ged_current_gedp->ged_result_str, "all objects within tolerance on weight calculation\n");
	}
    }
	...
	for (obj=0; obj < num_objects; obj++) {
		...
		can_terminate = 0;
		if (verbose)
		    bu_vls_printf(&ged_current_gedp->ged_result_str, "\tvolume tol not met on %s.  Refine grid\n",
				  obj_tbl[obj].o_name);
		break;
	    }
	}
...
terminate_check(struct cstate *state)
{
	...
	if (verbose)
	    bu_vls_printf(&ged_current_gedp->ged_result_str, "grid spacing refined to %g (below lower limit %g)\n",
	...
	if (BU_LIST_NON_EMPTY(&overlapList.l)) {
	    ...
	    bu_vls_printf(&ged_current_gedp->ged_result_str, "overlaps list is empty\n");
	}
    }
    if ((analysis_flags & ANALYSIS_GAP)) {
	...
	if (analysis_flags & (ANALYSIS_GAP|ANALYSIS_ADJ_AIR|ANALYSIS_OVERLAPS|ANALYSIS_EXP_AIR)) {
	    if (verbose)
		bu_vls_printf(&ged_current_gedp->ged_result_str, "Volume/Weight tolerance met.  Cannot terminate calculation due to error computations\n");
	} else {
	    struct region *regp;
	    int all_hit = 1;
	    unsigned long hits;
		...
		for (BU_LIST_FOR (regp, region, &(state->rtip->HeadRegion))) {
			...
			if (verbose) {
			    if (hits == 0) {
				bu_vls_printf(&ged_current_gedp->ged_result_str, "%s was not hit\n", regp->reg_name);
			    } else {
				bu_vls_printf(&ged_current_gedp->ged_result_str, "%s hit only %lu times (< %u)\n",
		...
		if (all_hit && wv_status == 0) {
		    if (verbose)
			bu_vls_printf(&ged_current_gedp->ged_result_str, "%s: Volume/Weight tolerance met. Terminate\n", BU_FLSTR);
		    return 0; /* terminate */
		}
	    } else {
		if (wv_status == 0) {
...
summary_reports(struct cstate *state, int start, int ac, const char *av[])
{
	...
	bu_vls_printf(&ged_current_gedp->ged_result_str, "Summaries:\n");
    else
	bu_vls_printf(&ged_current_gedp->ged_result_str, "Summary:\n");
	...
	if (print_per_region_stats) {
	    double *wv;
	    bu_vls_printf(&ged_current_gedp->ged_result_str, "\tregions:\n");
	    for (BU_LIST_FOR (regp, region, &(state->rtip->HeadRegion))) {
		double low = HUGE;
		double hi = -HUGE;
    ...
    if (analysis_flags & ANALYSIS_VOLUME) {
	bu_vls_printf(&ged_current_gedp->ged_result_str, "Volume:\n");
