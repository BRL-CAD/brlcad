...
ged_tol_core(struct ged *gedp, int argc, const char *argv[])
{
	...
	if (gedp->dbip->db_ttol.norm > 0.0) {
	    ...
	    bu_vls_printf(gedp->ged_result_str, "\tnorm %g degrees (%d deg %d min %g sec)\n",
			  gedp->dbip->db_ttol.norm * RAD2DEG, deg, min, sec);
	} else {
	...
	if (BU_STR_EQUAL(argv[1], "abs")) {
	    if (gedp->dbip->db_ttol.abs > 0.0)
		bu_vls_printf(gedp->ged_result_str, "%g", gedp->dbip->db_ttol.abs);
	    else {
	...
	if (BU_STR_EQUAL(argv[1], "rel")) {
	    if (gedp->dbip->db_ttol.rel > 0.0) {
		bu_vls_printf(gedp->ged_result_str, "%g", gedp->dbip->db_ttol.rel);
	    } else {
		bu_vls_printf(gedp->ged_result_str, "None");
	    }
	    return BRLCAD_OK;
	...
	if (BU_STR_EQUAL(argv[1], "norm")) {
	    if (gedp->dbip->db_ttol.norm > 0.0) {
		bu_vls_printf(gedp->ged_result_str, "%g", gedp->dbip->db_ttol.norm);
	    } else {
		bu_vls_printf(gedp->ged_result_str, "None");
	    }
	    return BRLCAD_OK;
	...
	if (BU_STR_EQUAL(argv[1], "dist")) {
	    bu_vls_printf(gedp->ged_result_str, "%g", gedp->dbip->db_tol.dist);
	    return BRLCAD_OK;
	}

	if (BU_STR_EQUAL(argv[1], "perp")) {
	    bu_vls_printf(gedp->ged_result_str, "%g", gedp->dbip->db_tol.perp);
	    return BRLCAD_OK;
	}
	...
	if (BU_STR_EQUAL(argv[0], "norm")) {
	    ...
	    gedp->dbip->db_ttol.norm = f * DEG2RAD;
	    valid_tol = 1;
	}
	...
	if (BU_STR_EQUAL(argv[0], "lmin")) {
	    ...
	    /* Note that a value of 0.0 will disable relative tolerance */
	    gedp->dbip->db_ttol.rel_lmin = f;
	    valid_tol = 1;
	}

	if (BU_STR_EQUAL(argv[0], "dist")) {
	    /* Calculational distance tolerance */
	    gedp->dbip->db_tol.dist = f;
	    gedp->dbip->db_tol.dist_sq = gedp->dbip->db_tol.dist * gedp->dbip->db_tol.dist;
	    valid_tol = 1;
	}
	if (BU_STR_EQUAL(argv[0], "perp")) {
	    ...
	    gedp->dbip->db_tol.perp = f;
	    gedp->dbip->db_tol.para = 1.0 - f;
	    valid_tol = 1;
	}
