...
_arb_cmd_repair(void *bs, int argc, const char **argv)
{
    ...
    BU_OPT(d[2], "t", "tol", "<#>", bu_opt_fastf_t, &ftol, "Distance tolerance for snapping vertices");
    BU_OPT_NULL(d[3]);
    ...
    if (print_help || !argc) {
        ...
        bu_vls_printf(gedp->ged_result_str, "Usage: arb repair [options] input_arb\n\n");
        bu_vls_printf(gedp->ged_result_str, "Attempts to repair and canonicalize an ARB.\n\n");
        if (option_help) {
