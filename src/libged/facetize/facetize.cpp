...
_try_nmg_facetize(struct ged *gedp, int argc, const char **argv, int nmg_use_tnurbs, struct _ged_facetize_opts *o)
{
    ...
    init_state.ts_tol = &gedp->ged_wdbp->dbip->db_tol;
	...
	if (!BU_SETJUMP) {
	    /* try */
	    failed = nmg_boolean(facetize_tree, nmg_model, &RTG.rtg_vlfree, &gedp->ged_wdbp->dbip->db_tol, &rt_uniresource);
	} else {
	    /* catch */
	    BU_UNSETJUMP;
	    _ged_facetize_log_default(o);
...
_try_nmg_triangulate(struct ged *gedp, struct model *nmg_model, struct _ged_facetize_opts *o)
{
    ...
    if (!BU_SETJUMP) {
	/* try */
	nmg_triangulate_model(nmg_model, &RTG.rtg_vlfree, &gedp->ged_wdbp->dbip->db_tol);
    } else {
	/* catch */
	BU_UNSETJUMP;
...
_try_nmg_to_bot(struct ged *gedp, struct model *nmg_model, struct _ged_facetize_opts *o)
{
    ...
    if (!BU_SETJUMP) {
	/* try */
	bot = (struct rt_bot_internal *)nmg_mdl_to_bot(nmg_model, &RTG.rtg_vlfree, &gedp->ged_wdbp->dbip->db_tol);
    } else {
	/* catch */
	BU_UNSETJUMP;
	_ged_facetize_log_default(o);
...
_ged_facetize_objlist(struct ged *gedp, int argc, const char **argv, struct _ged_facetize_opts *opts)
{
    ...
    struct bg_tess_tol *tol = &(gedp->ged_wdbp->dbip->db_ttol);
...
_ged_facetize_regions_resume(struct ged *gedp, int argc, const char **argv, struct _ged_facetize_opts *opts)
{
    ...
    opts->tol = &(gedp->ged_wdbp->dbip->db_ttol);
