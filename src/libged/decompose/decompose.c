...
ged_decompose_core(struct ged *gedp, int argc, const char *argv[])
{
	...
	for (BU_LIST_FOR(s, shell, &r->s_hd)) {
	    ...
	    /* duplicate shell */
	    tmp_s = (struct shell *)nmg_dup_shell(s, &trans_tbl, &RTG.rtg_vlfree, &gedp->ged_wdbp->dbip->db_tol);
	    ...
	    (void) nmg_decompose_shell(tmp_s, &RTG.rtg_vlfree, &gedp->ged_wdbp->dbip->db_tol);
		...
		(void)nmg_ks(kill_s);
		nmg_shell_a(decomp_s, &gedp->ged_wdbp->dbip->db_tol);
		new_s = (struct shell *)nmg_dup_shell(decomp_s, &trans_tbl, &RTG.rtg_vlfree, &gedp->ged_wdbp->dbip->db_tol);
		...
		(void)nmg_rebound(new_m, &gedp->ged_wdbp->dbip->db_tol);
