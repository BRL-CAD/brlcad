...
ged_decompose_core(struct ged *gedp, int argc, const char *argv[])
{
	...
	for (BU_LIST_FOR(s, shell, &r->s_hd)) {
	    ...
	    /* duplicate shell */
	    tmp_s = (struct shell *)nmg_dup_shell(s, &trans_tbl, &rtg_vlfree, &gedp->ged_wdbp->wdb_tol);
	    ...
	    (void) nmg_decompose_shell(tmp_s, &rtg_vlfree, &gedp->ged_wdbp->wdb_tol);
		...
		new_s = (struct shell *)nmg_dup_shell(decomp_s, &trans_tbl, &rtg_vlfree, &gedp->ged_wdbp->wdb_tol);
		(void)nmg_mv_shell_to_region(new_s, decomp_r);

		/* move this region to a different model */
		new_m = (struct model *)nmg_mk_model_from_region(decomp_r, 1, &rtg_vlfree);
