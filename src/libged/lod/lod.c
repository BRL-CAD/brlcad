...
static const struct bu_cmd_form ged_lod_native_forms[] = {
    BU_CMD_FORM_SCHEMA("action_grammar", "Level-of-detail action grammar",
	&ged_view_lod_schema),
    BU_CMD_FORM_NULL
};
...
ged_lod_select_native_form(const struct bu_cmd_forms *forms,
	size_t UNUSED(argc), const char * const *UNUSED(argv),
	void *UNUSED(context))
{
    return &forms->forms[0];
}

static const struct bu_cmd_forms ged_lod_forms =
    BU_CMD_FORMS("lod", "Manage level-of-detail drawing settings",
	ged_lod_native_forms, ged_lod_select_native_form);
...
{
    return ged_cmd_native_forms_validate(gedp, &ged_lod_forms, input,
	cursor_pos, result);
}
...
{
    return ged_cmd_native_forms_analyze(gedp, &ged_lod_forms, input,
	analysis);
}
...
{
    return ged_cmd_native_forms_lint(&ged_lod_forms, msgs);
}

static char *
ged_lod_grammar_help(const char *invocation)
{
    return ged_view_lod_help(invocation);
}


static const struct ged_cmd_grammar ged_lod_grammar = {
    "lod", "Manage level-of-detail drawing settings", ged_lod_grammar_validate,
    ged_lod_grammar_analyze, ged_view_lod_grammar_json,
    ged_lod_grammar_lint, NULL, ged_lod_grammar_help
};
