...
_pnts_inside_obj(struct ged *gedp, int argc, const char **argv)
{
    ...
    int opt_ret = 0;
    const char *usage = "Usage: pnts inside [options] <test_pnts_obj> <test_vol_obj> <output_pnts_obj>\n\n";
    struct bu_opt_desc d[3];
    BU_OPT(d[0], "h", "help",      "",  NULL, &print_help,     "Print help and exit");
    BU_OPT(d[1], "R", "reverse",   "",  NULL, &report_outside, "Report points outside volume.");
    BU_OPT_NULL(d[2]);
    ...
    GED_DB_LOOKUP(gedp, test_pnts_dp, test_pnts_obj, LOOKUP_NOISY, GED_ERROR & GED_QUIET);
    struct directory *test_vol_dp = RT_DIR_NULL;
    GED_DB_LOOKUP(gedp, test_vol_dp, vol_obj, LOOKUP_NOISY, GED_ERROR & GED_QUIET);

    /* For the test points, we need to introspect the object's data */
    struct rt_db_internal tpnts_intern;
    GED_DB_GET_INTERNAL(gedp, &tpnts_intern, test_pnts_dp, bn_mat_identity, &rt_uniresource, GED_ERROR);
    if (tpnts_intern.idb_major_type != DB5_MAJORTYPE_BRLCAD || tpnts_intern.idb_minor_type != DB5_MINORTYPE_BRLCAD_PNTS) {
	bu_vls_printf(gedp->ged_result_str, "pnts inside: %s is not a pnts object!", test_pnts_obj);
	...
	case RT_PNT_TYPE_PNT:
		...
		if ((!report_outside && iostat == -1) || (report_outside && iostat != -1)) {
...
ged_pnts_core(struct ged *gedp, int argc, const char *argv[])
{
    ...
    if (bu_strncmp(argv[0], "inside", len) == 0) return _pnts_inside_obj(gedp, argc, argv);
