...
ged_snap_to_grid(struct ged *gedp, fastf_t *vx, fastf_t *vy)
{
    ...
    if (gedp->dm_gvp == DM_VIEW_NULL)
	return;

    if (ZERO(gedp->dm_gvp->gv_grid.ggs_res_h) ||
	ZERO(gedp->dm_gvp->gv_grid.ggs_res_v))
	return;

    sf = gedp->dm_gvp->gv_scale*gedp->ged_wdbp->dbip->dbi_base2local;
    inv_sf = 1 / sf;

    VSET(view_pt, *vx, *vy, 0.0);
    VSCALE(view_pt, view_pt, sf);  /* view_pt now in local units */

    MAT4X3PNT(view_grid_anchor, gedp->dm_gvp->gv_model2view, gedp->dm_gvp->gv_grid.ggs_anchor);
    VSCALE(view_grid_anchor, view_grid_anchor, sf);  /* view_grid_anchor now in local units */

    grid_units_h = (view_grid_anchor[X] - view_pt[X]) / (gedp->dm_gvp->gv_grid.ggs_res_h * gedp->ged_wdbp->dbip->dbi_base2local);
    grid_units_v = (view_grid_anchor[Y] - view_pt[Y]) / (gedp->dm_gvp->gv_grid.ggs_res_v * gedp->ged_wdbp->dbip->dbi_base2local);
    nh = grid_units_h;
    nv = grid_units_v;
    ...
    if (grid_units_h <= -0.5)
	*vx = view_grid_anchor[X] - ((nh - 1) * gedp->dm_gvp->gv_grid.ggs_res_h * gedp->ged_wdbp->dbip->dbi_base2local);
    else if (0.5 <= grid_units_h)
	*vx = view_grid_anchor[X] - ((nh + 1) * gedp->dm_gvp->gv_grid.ggs_res_h * gedp->ged_wdbp->dbip->dbi_base2local);
    else
	*vx = view_grid_anchor[X] - (nh * gedp->dm_gvp->gv_grid.ggs_res_h * gedp->ged_wdbp->dbip->dbi_base2local);

    if (grid_units_v <= -0.5)
	*vy = view_grid_anchor[Y] - ((nv - 1) * gedp->dm_gvp->gv_grid.ggs_res_v * gedp->ged_wdbp->dbip->dbi_base2local);
    else if (0.5 <= grid_units_v)
	*vy = view_grid_anchor[Y] - ((nv + 1) * gedp->dm_gvp->gv_grid.ggs_res_v * gedp->ged_wdbp->dbip->dbi_base2local);
    else
	*vy = view_grid_anchor[Y] - (nv * gedp->dm_gvp->gv_grid.ggs_res_v * gedp->ged_wdbp->dbip->dbi_base2local);
...
grid_vsnap(struct ged *gedp)
{
    ...
    MAT_DELTAS_GET_NEG(model_pt, gedp->dm_gvp->gv_center);
    ...
    MAT_DELTAS_VEC_NEG(gedp->dm_gvp->gv_center, model_pt);
...
grid_vls_print(struct ged *gedp)
{
    bu_vls_printf(gedp->ged_result_str, "anchor = %g %g %g\n",
		  gedp->dm_gvp->gv_grid.ggs_anchor[0] * gedp->ged_wdbp->dbip->dbi_base2local,
		  gedp->dm_gvp->gv_grid.ggs_anchor[1] * gedp->ged_wdbp->dbip->dbi_base2local,
		  gedp->dm_gvp->gv_grid.ggs_anchor[2] * gedp->ged_wdbp->dbip->dbi_base2local);
    bu_vls_printf(gedp->ged_result_str, "color = %d %d %d\n",
		  gedp->dm_gvp->gv_grid.ggs_color[0],
		  gedp->dm_gvp->gv_grid.ggs_color[1],
		  gedp->dm_gvp->gv_grid.ggs_color[2]);
    bu_vls_printf(gedp->ged_result_str, "draw = %d\n", gedp->dm_gvp->gv_grid.ggs_draw);
    bu_vls_printf(gedp->ged_result_str, "mrh = %d\n", gedp->dm_gvp->gv_grid.ggs_res_major_h);
    bu_vls_printf(gedp->ged_result_str, "mrv = %d\n", gedp->dm_gvp->gv_grid.ggs_res_major_v);
    bu_vls_printf(gedp->ged_result_str, "rh = %g\n", gedp->dm_gvp->gv_grid.ggs_res_h * gedp->ged_wdbp->dbip->dbi_base2local);
    bu_vls_printf(gedp->ged_result_str, "rv = %g\n", gedp->dm_gvp->gv_grid.ggs_res_v * gedp->ged_wdbp->dbip->dbi_base2local);
    bu_vls_printf(gedp->ged_result_str, "snap = %d\n", gedp->dm_gvp->gv_grid.ggs_snap);
}
...
ged_grid(struct ged *gedp, int argc, const char *argv[])
{
	...
	if (argc == 0) {
	    bu_vls_printf(gedp->ged_result_str, "%d", gedp->dm_gvp->gv_grid.ggs_draw);
	    return GED_OK;
	} else if (argc == 1) {
	    i = (int)user_pt[X];

	    if (i)
		gedp->dm_gvp->gv_grid.ggs_draw = 1;
	    else
		gedp->dm_gvp->gv_grid.ggs_draw = 0;
	...
	if (argc == 0) {
	    bu_vls_printf(gedp->ged_result_str, "%d", gedp->dm_gvp->gv_grid.ggs_snap);
	    return GED_OK;
	} else if (argc == 1) {
	    i = (int)user_pt[X];

	    if (i)
		gedp->dm_gvp->gv_grid.ggs_snap = 1;
	    else
		gedp->dm_gvp->gv_grid.ggs_snap = 0;
	...
	if (argc == 0) {
	    ...
	    gedp->dm_gvp->gv_grid.ggs_res_h = user_pt[X] * gedp->ged_wdbp->dbip->dbi_local2base;
	...
	if (argc == 0) {
	    ...
	    gedp->dm_gvp->gv_grid.ggs_res_v = user_pt[X] * gedp->ged_wdbp->dbip->dbi_local2base;
	...
	if (argc == 0) {
	    bu_vls_printf(gedp->ged_result_str, "%d", gedp->dm_gvp->gv_grid.ggs_res_major_h);
	    return GED_OK;
	} else if (argc == 1) {
	    gedp->dm_gvp->gv_grid.ggs_res_major_h = (int)user_pt[X];
	...
	if (argc == 0) {
	    bu_vls_printf(gedp->ged_result_str, "%d", gedp->dm_gvp->gv_grid.ggs_res_major_v);
	    return GED_OK;
	} else if (argc == 1) {
	    gedp->dm_gvp->gv_grid.ggs_res_major_v = (int)user_pt[X];
	...
	if (argc == 0) {
	    bu_vls_printf(gedp->ged_result_str, "%g %g %g",
			  gedp->dm_gvp->gv_grid.ggs_anchor[X] * gedp->ged_wdbp->dbip->dbi_base2local,
			  gedp->dm_gvp->gv_grid.ggs_anchor[Y] * gedp->ged_wdbp->dbip->dbi_base2local,
			  gedp->dm_gvp->gv_grid.ggs_anchor[Z] * gedp->ged_wdbp->dbip->dbi_base2local);
	    return GED_OK;
	} else if (argc == 3) {
	    gedp->dm_gvp->gv_grid.ggs_anchor[0] = user_pt[X] * gedp->ged_wdbp->dbip->dbi_local2base;
	    gedp->dm_gvp->gv_grid.ggs_anchor[1] = user_pt[Y] * gedp->ged_wdbp->dbip->dbi_local2base;
	    gedp->dm_gvp->gv_grid.ggs_anchor[2] = user_pt[Z] * gedp->ged_wdbp->dbip->dbi_local2base;
	...
	if (argc == 0) {
	    bu_vls_printf(gedp->ged_result_str, "%d %d %d",
			  gedp->dm_gvp->gv_grid.ggs_color[X],
			  gedp->dm_gvp->gv_grid.ggs_color[Y],
			  gedp->dm_gvp->gv_grid.ggs_color[Z]);
	    return GED_OK;
	} else if (argc == 3) {
	    gedp->dm_gvp->gv_grid.ggs_color[0] = (int)user_pt[X];
	    gedp->dm_gvp->gv_grid.ggs_color[1] = (int)user_pt[Y];
	    gedp->dm_gvp->gv_grid.ggs_color[2] = (int)user_pt[Z];
