...
ged_bev_core(struct ged *gedp, int argc, const char *argv[])
{
	...
	failed = nmg_boolean(tmp_tree, bev_nmg_model, &rtg_vlfree, &gedp->ged_wdbp->wdb_tol, &rt_uniresource);
	BU_UNSETJUMP;
    } else
	failed = 1;
	...
	nmg_triangulate_model(bev_nmg_model, &rtg_vlfree, &gedp->ged_wdbp->wdb_tol);
	BU_UNSETJUMP;
    }
