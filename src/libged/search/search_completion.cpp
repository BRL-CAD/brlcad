/*            S E A R C H _ C O M P L E T I O N . C P P
 * BRL-CAD
 *
...
ged_search_completion_type_has_prefix(const char *token)
{
    ...
    size_t count = 0;
    const char * const *keywords = db_search_type_terms(1, &count);
    for (size_t i = 0; i < count; i++)
...
{
    size_t count = 0;
    const char * const *keywords = db_search_type_terms(0, &count);
    for (size_t i = 0; i < count; i++)
	ged_search_add_candidate(candidates, keywords[i], prefix);
}
...
ged_search_state_consume(struct ged_search_parse_state *state, const char *arg, size_t arg_index)
{
	...
	if (!state->path_seen && ged_search_is_top_option(arg)) {
	    ...
	    if (ged_search_top_option_parse(arg, &kind, NULL) &&
		kind == GED_SEARCH_TOP_OPTION_HIDDEN)
		state->include_hidden = 1;
	    return;
	}
...
ged_search_validate(struct ged *gedp, const struct ged_input_parse &parsed, const std::string &seed, struct ged_cmd_validate_result *result)
{
	...
	(void)ged_cmd_validate(gedp, nested_line.c_str(), nested_line.size(),
	    &nested);
	...
	result->semantic_provider = nested.semantic_provider;
	result->candidate_validate = nested.candidate_validate;
	...
	if (seed.empty() || ged_search_is_plan_start(seed.c_str())) {
	    ...
	    /* At the initial empty slot both a path and an expression operator are
	     * legal.  Keep the compact static grammar candidates and ask the shared
	     * database provider for the other half of that union. */
	    if (seed.empty() && !state.path_seen) {
		result->semantic_provider = state.include_hidden ?
		    "ged.db_path_geometry_hidden" : "ged.db_path";
	    }
	    ged_set_result_chars_local(result, parsed, token_index, token_index);
	    return 1;
	}
...
ged_search_analyze(struct ged *gedp, const struct ged_input_parse &parsed, struct ged_cmd_analysis *analysis)
{
	...
	if (state.pending_arg) {
		...
		if (exec_end > i) {
		    struct ged_cmd_analysis nested = GED_CMD_ANALYSIS_NULL;
...
ged_search_grammar_json(void)
{
	...
	if (i)
	    bu_vls_putc(&out, ',');
	bu_vls_printf(&out, "{\"name\":\"%s\",\"argument\":\"none\",\"repeatable\":%s,\"help\":",
		options[i].name, options[i].kind == GED_SEARCH_TOP_OPTION_VERBOSE ? "true" : "false");
	bu_cmd_json_string(&out, options[i].help);
	bu_vls_putc(&out, '}');
    }
	...
	if (i)
	    bu_vls_putc(&out, ',');
	bu_vls_printf(&out, "{\"name\":\"%s\",\"argument\":\"%s\",\"help\":",
		terms[i].name, ged_search_syntax_argument_name(terms[i].argument));
	bu_cmd_json_string(&out, terms[i].help);
	bu_vls_putc(&out, '}');
    }
    bu_vls_strcat(&out, "]}");
...
ged_search_grammar_lint(struct bu_vls *msgs)
{
    ...
    for (size_t i = 0; i < count; i++) {
	if (BU_STR_EMPTY(terms[i].name) || BU_STR_EMPTY(terms[i].help) ||
		!names.insert(std::string(terms[i].name)).second) {
	    if (msgs)
	...
	if (BU_STR_EMPTY(options[i].name) || BU_STR_EMPTY(options[i].help) ||
		!names.insert(std::string(options[i].name)).second ||
...
static char *
ged_search_grammar_help(const char *invocation)
{
    struct bu_vls out = BU_VLS_INIT_ZERO;
    char *result = NULL;
    size_t count = 0;
	...
	invocation = "search";
    bu_vls_printf(&out, "Usage: %s [options] [path] [expression ...]\n\n",
	invocation);
    bu_vls_strcat(&out, "Search database objects\n\nOptions:\n");
    for (size_t i = 0; i < option_count; i++)
	bu_vls_printf(&out, "  %-34s %s\n", options[i].name, options[i].help);
    bu_vls_strcat(&out, "\nExpression terms:\n");
    for (size_t i = 0; i < count; i++) {
	const char *argument = ged_search_syntax_argument_name(terms[i].argument);
	struct bu_vls syntax = BU_VLS_INIT_ZERO;
	bu_vls_strcat(&syntax, terms[i].name);
	if (terms[i].argument != DB_SEARCH_SYNTAX_NO_ARGUMENT)
	    bu_vls_printf(&syntax, " %s", argument);
	bu_vls_printf(&out, "  %-34s %s\n", bu_vls_cstr(&syntax),
	    terms[i].help ? terms[i].help : "");
	bu_vls_free(&syntax);
    }
    result = bu_vls_strdup(&out);
    bu_vls_free(&out);
...
extern "C" GED_EXPORT const struct ged_cmd_grammar ged_search_grammar = {
    ...
    ged_search_grammar_analyze, ged_search_grammar_json,
    ged_search_grammar_lint, NULL, ged_search_grammar_help
};
