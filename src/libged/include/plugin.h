...
#define GED_DEFINE_TYPED_OPERAND_SCHEMA_POLICY_PROVIDER(id, cmdstr, helpstr, role, valtype, mincnt, maxcnt, rolehelp, policy, provider) \
    static const struct bu_cmd_operand id##_schema_operands[] = { \
	BU_CMD_OPERAND(role, valtype, mincnt, maxcnt, rolehelp, provider), \
	BU_CMD_OPERAND_NULL \
    }; \
    static const struct bu_cmd_schema id##_cmd_schema = { \
	cmdstr, helpstr, NULL, id##_schema_operands, \
	policy, BU_CMD_SCHEMA_CONSTRAINTS(NULL, NULL) \
    }

#define GED_DEFINE_TYPED_OPERAND_SCHEMA_PROVIDER(id, cmdstr, helpstr, role, valtype, mincnt, maxcnt, rolehelp, provider) \
    GED_DEFINE_TYPED_OPERAND_SCHEMA_POLICY_PROVIDER(id, cmdstr, helpstr, role, valtype, mincnt, maxcnt, rolehelp, BU_CMD_PARSE_STOP_AT_FIRST_OPERAND, provider)

#define GED_DEFINE_TYPED_OPERAND_SCHEMA(id, cmdstr, helpstr, role, valtype, mincnt, maxcnt, rolehelp) \
    GED_DEFINE_TYPED_OPERAND_SCHEMA_PROVIDER(id, cmdstr, helpstr, role, valtype, mincnt, maxcnt, rolehelp, NULL)

/* Compact no-option, no-operand schemas.  The named form is useful for tree
 * nodes whose symbols do not follow the top-level id##_cmd_schema convention. */
#define GED_DEFINE_NO_ARG_SCHEMA_NAMED(symbol, cmdstr, helpstr, policy) \
    static const struct bu_cmd_schema symbol = { \
	cmdstr, helpstr, NULL, NULL, policy, BU_CMD_SCHEMA_CONSTRAINTS(NULL, NULL) \
    }

#define GED_DEFINE_NO_ARG_SCHEMA_POLICY(id, cmdstr, helpstr, policy) \
    GED_DEFINE_NO_ARG_SCHEMA_NAMED(id##_cmd_schema, cmdstr, helpstr, policy)

#define GED_DEFINE_NO_ARG_SCHEMA(id, cmdstr, helpstr) \
    GED_DEFINE_NO_ARG_SCHEMA_POLICY(id, cmdstr, helpstr, BU_CMD_PARSE_STOP_AT_FIRST_OPERAND)
...
#define GED_DEFINE_NATIVE_DISCRETE_COUNT_VALIDATOR(id, count1, count2, count3) \
	...
	int ret = bu_cmd_schema_validate_syntax(&flat, argc, argv, cursor_arg, result); \
...
extern "C" {
    ...
    /** Register one command implementation.  The registry borrows @p cmd,
     * its implementation record, and all referenced strings/callbacks until
     * libged_shutdown() has finished. */
    GED_EXPORT int ged_register_command(const struct ged_cmd *cmd);
    ...
    /** Register borrowed native metadata.  The schema and every table,
     * string, and callback reachable from it must remain valid until
     * libged_shutdown() has finished. */
    GED_EXPORT int ged_register_command_native_schema(const char *name, const struct bu_cmd_schema *schema);
    /** Parser-owned counterpart to native-schema registration, with the same
     * borrowed-storage lifetime. */
    int ged_register_command_grammar(const char *name, const struct ged_cmd_grammar *grammar);
    /** Register a compact command schema.  The specification and metadata
     * container are copied; all tables and strings they reference must remain
     * valid while the command remains registered. */
    int ged_opt_register(const char *name, const ged_opt_spec *spec);
    void ged_ensure_initialized(void);
    GED_EXPORT void libged_shutdown(void);
 ...
 *      struct ged_cmd_impl bot_cmd_impl = { "bot", ged_bot_core, GED_CMD_DEFAULT, NULL, NULL, NULL };
 ...
 *
 *    Expands (conceptually) to:
 *      struct ged_cmd_impl questionmark_cmd_impl = { "?", ged_help_core, GED_CMD_DEFAULT, NULL, NULL, NULL };
...
#define GED__DECL_META_X(token, fn, opts)                                            \
    struct ged_cmd_impl GED_CAT2(GED_CMD_SYM(token), _impl) = { GED_STR(token), fn, opts, NULL, NULL, NULL };

#define GED__DECL_META_XID(sym, cmdstr, fn, opts)                                    \
    struct ged_cmd_impl GED_CAT2(sym, _impl) = { cmdstr, fn, opts, NULL, NULL, NULL };

#define GED__IGNORE_SCHEMA_X(token, fn, opts, schema)
#define GED__IGNORE_SCHEMA_XID(sym, cmdstr, fn, opts, schema)

#define GED__DECL_META_OPT_SPEC_X(token, fn, opts, spec)                             \
    struct ged_cmd_impl GED_CAT2(GED_CMD_SYM(token), _impl) = { GED_STR(token), fn, opts, NULL, NULL, spec };

#define GED__DECL_META_OPT_SPEC_XID(sym, cmdstr, fn, opts, spec)                     \
    struct ged_cmd_impl GED_CAT2(sym, _impl) = { cmdstr, fn, opts, NULL, NULL, spec };

#define GED__DECL_META_NATIVE_SCHEMA_X(token, fn, opts, schema)                      \
    struct ged_cmd_impl GED_CAT2(GED_CMD_SYM(token), _impl) = { GED_STR(token), fn, opts, schema, NULL, NULL };

#define GED__DECL_META_NATIVE_SCHEMA_XID(sym, cmdstr, fn, opts, schema)              \
    struct ged_cmd_impl GED_CAT2(sym, _impl) = { cmdstr, fn, opts, schema, NULL, NULL };

#define GED__DECL_META_GRAMMAR_X(token, fn, opts, grammar)                            \
    struct ged_cmd_impl GED_CAT2(GED_CMD_SYM(token), _impl) = { GED_STR(token), fn, opts, NULL, grammar, NULL };

#define GED__DECL_META_GRAMMAR_XID(sym, cmdstr, fn, opts, grammar)                    \
    struct ged_cmd_impl GED_CAT2(sym, _impl) = { cmdstr, fn, opts, NULL, grammar, NULL };
...
#define GED_DECLARE_COMMAND_METADATA_WITH_OPT_SPEC(LIST_MACRO)                       \
    LIST_MACRO(GED__DECL_META_OPT_SPEC_X, GED__DECL_META_OPT_SPEC_XID)

/* A mixed list takes six macro parameters.  Option-spec, native-schema, and
 * grammar entries use the first, second, and third pairs respectively. */
...
#define GED__DECL_STATIC_ANCHOR_OPT_SPEC_X(token, fn, opts, spec)                    \
    GED_REGISTER_SYM(GED_CMD_SYM(token));

#define GED__DECL_STATIC_ANCHOR_OPT_SPEC_XID(sym, cmdstr, fn, opts, spec)            \
    GED_REGISTER_SYM(sym);
...
#  define GED_DECLARE_STATIC_REGISTRATION_WITH_OPT_SPEC(LIST_MACRO)                  \
      LIST_MACRO(GED__DECL_STATIC_ANCHOR_OPT_SPEC_X, GED__DECL_STATIC_ANCHOR_OPT_SPEC_XID)
...
#define GED_DECLARE_COMMAND_SET_WITH_OPT_SPEC(LIST_MACRO)                            \
    GED_DECLARE_COMMAND_METADATA_WITH_OPT_SPEC(LIST_MACRO)                           \
    GED_DECLARE_STATIC_REGISTRATION_WITH_OPT_SPEC(LIST_MACRO)
...
#define GED__DECL_NATIVE_SCHEMA_X(token, fn, opts, schema) { GED_STR(token), schema, NULL, NULL },
#define GED__DECL_NATIVE_SCHEMA_XID(sym, cmdstr, fn, opts, schema) { cmdstr, schema, NULL, NULL },
...
#define GED__DECL_GRAMMAR_X(token, fn, opts, grammar) { GED_STR(token), NULL, grammar, NULL },
#define GED__DECL_GRAMMAR_XID(sym, cmdstr, fn, opts, grammar) { cmdstr, NULL, grammar, NULL },
#define GED__DECL_PCMD_OPT_SPEC_X(token, fn, opts, spec) { GED_STR(token), fn },
#define GED__DECL_PCMD_OPT_SPEC_XID(sym, cmdstr, fn, opts, spec) { cmdstr, fn },
#define GED__DECL_OPT_SPEC_X(token, fn, opts, spec) { GED_STR(token), NULL, NULL, spec },
#define GED__DECL_OPT_SPEC_XID(sym, cmdstr, fn, opts, spec) { cmdstr, NULL, NULL, spec },
...
#  define GED_DECLARE_PLUGIN_MANIFEST_WITH_OPT_SPEC(plugin_name_str, plugin_version_u32, LIST_MACRO) \
    static bu_plugin_cmd pcommands[] = {                                               \
        LIST_MACRO(GED__DECL_PCMD_OPT_SPEC_X, GED__DECL_PCMD_OPT_SPEC_XID)             \
    };                                                                                 \
    static const struct ged_cmd_schema pschemas[] = {                                  \
        LIST_MACRO(GED__DECL_OPT_SPEC_X, GED__DECL_OPT_SPEC_XID)                       \
    };                                                                                 \
    static bu_plugin_manifest pinfo = {                                                \
        plugin_name_str,                                                               \
        (unsigned int)(plugin_version_u32),                                            \
        (unsigned int)(sizeof(pcommands)/sizeof(pcommands[0])),                        \
        pcommands, BU_PLUGIN_ABI_VERSION, sizeof(bu_plugin_manifest)                   \
    };                                                                                 \
    static const struct ged_plugin_schema_manifest pschema_info = {                    \
        plugin_name_str, (unsigned int)(plugin_version_u32),                           \
        (unsigned int)(sizeof(pschemas)/sizeof(pschemas[0])), pschemas,                \
