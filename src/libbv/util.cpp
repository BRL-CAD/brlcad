...
static void
_bound_objs(int *is_empty, int *have_geom_objs, vect_t min, vect_t max, struct bu_ptbl *so)
{
    vect_t minus, plus;
    for (size_t i = 0; i < BU_PTBL_LEN(so); i++) {
	struct bv_scene_group *g = (struct bv_scene_group *)BU_PTBL_GET(so, i);
	_bound_objs(is_empty, have_geom_objs, min, max, &g->children);
...
bv_autoview(struct bview *v, struct bu_ptbl *so, double factor)
{
    ...
    if (so)
	_bound_objs(&is_empty, &have_geom_objs, min, max, so);
...
void
bv_update(struct bview *gvp, struct bu_ptbl *objs)
{
    vect_t work, work1;
    vect_t temp, temp1;
...
bv_obj_get(struct bv_scene_obj *free_scene_objs, struct bu_list *vlfree)
{
    ...
    if (!free_scene_objs || BU_LIST_IS_EMPTY(&free_scene_objs->l)) {
	BU_ALLOC(s, struct bv_scene_obj);
	s->i = new bv_scene_obj_internal;
    } else {
	s = BU_LIST_NEXT(bv_scene_obj, &free_scene_objs->l);
	BU_LIST_DEQUEUE(&((s)->l));
    }
    ...
    s->free_scene_obj = free_scene_objs;
    s->vlfree = vlfree;
    return s;
}
...
bv_obj_add_child(struct bv_scene_obj *o, struct bv_scene_obj *co)
{
    ...
    bu_ptbl_ins_unique(&o->children, (long *)co);
}

void
bv_obj_reset(struct bv_scene_obj *s)
{
    // Any child objects may be reused elsewhere, so we just
    // reset the table - it is up to the parent to manage the
    // memory of the child objects themselves
