...
/*	Routine to write an shell in the Euclid "decoded" format */
static void
Write_euclid_region(struct shell *s, struct db_tree_state *tsp, FILE *fp_out)
{
    ...
    if (verbose)
	bu_log("Write_euclid_region: s=%p\n", (void *)s);
...
do_region_end(struct db_tree_state *tsp, const struct db_full_path *pathp, union tree *curtree, void *UNUSED(client_data))
{
    ...
    Write_euclid_region(s, tsp, fp_out);
