...
(
    ...
    bu_avs_print(avs, "Region attributes:");
...
(
	...
	std::map<std::string, std::string> bot_attributes    = bot.getAttributes();

	bot.write(wdbp);

	if (bot_attributes.size() > 0) {
	    bot.writeAttributes(wdbp, bot.getName(), bot_attributes);
	}
