	...
	while (is && !is.eof()) {
		...
		if (tokens[0][0] == '$')
			...
			if (command[0] == "INCLUDE") {
			    ...
			    if ((command.size() == 1) || (command[1] == "INERTIA") || (command[1]=="Rivet")) {
				state         = KState::Part;
				partLinesRead = 0;
				partTitle     = "";
			    }
		...
		switch (state) {
			...
			switch (partAttributeRead) {
			   case 0: {
			       ...
			       if (tokens.size() == 2) {
				   data.parts[pid].avp.key = "ADAPTIVE_FAILURE_STARTING_TIME";
				   data.parts[pid].avp.value = tokens[1];
			       }
			       else {
