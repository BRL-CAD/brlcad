...
Find_inner_loops(struct faceuse *fu, struct loop_list *lptr)
{
	...
	if (nmg_classify_lu_lu(lu, lptr->lu, &rtg_vlfree, &tol) == NMG_CLASS_AinB) {
	...
	while (inner1) {
		...
		if (nmg_classify_lu_lu(inner1->lu, inner->lu, &rtg_vlfree, &tol) == NMG_CLASS_AinB) {
...
Orient_face_loops(struct faceuse *fu)
{
	...
	for (BU_LIST_FOR(lu1, loopuse, &fu->lu_hd)) {
	    if (lu1 == lu)
		continue;
	    if (nmg_classify_lu_lu(lu, lu1, &rtg_vlfree, &tol) == NMG_CLASS_AinB) {
		outer = 0;
		break;
	    }
	}
