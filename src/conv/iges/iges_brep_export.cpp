/*                     I G E S _ B R E P _ E X P O R T . C P P
 * BRL-CAD
 *
 ...
 * information.
 */
/** @file iges/iges_brep_export.cpp
 *
...
int
write_on_surface(Writer &writer, const ON_Surface *srf)
{
    ON_NurbsSurface ns;
    if (!srf || !srf->GetNurbForm(ns) || !ns.IsValid())
	return 0;
    ...
    return writer.nurbs_surface(k1, k2, m1, m2, rat ? 1 : 0,
				     ns.IsClosed(0) ? 1 : 0,
				     ns.IsClosed(1) ? 1 : 0,
				     ns.IsPeriodic(0) ? 1 : 0,
				     ns.IsPeriodic(1) ? 1 : 0,
...
int
write_on_curve(Writer &writer, const ON_Curve *crv, int planar, int reverse)
{
    ON_NurbsCurve nc;
    if (!crv || !crv->GetNurbForm(nc) || !nc.IsValid())
	return 0;
    ...
    return writer.nurbs_curve(k, m, rat ? 1 : 0, planar,
				   nc.IsClosed() ? 1 : 0,
				   nc.IsPeriodic() ? 1 : 0,
				   &knots[0], &wts[0], &pts[0],
				   dom.Min(), dom.Max(),
 ...
 * 0 on failure. */
int
write_loop(Writer &writer, const ON_BrepLoop *loop, int surf_de)
{
    ...
    bool complete_model_boundary = true;

    for (int ti = 0; ti < nt; ti++) {
	const ON_BrepTrim *trim = loop->Trim(ti);
	if (!trim || !trim->TrimCurveOf())
	    return 0;
	const int b = write_on_curve(writer, trim, 1, 0);
	if (!b)
	    return 0;
	...
	if (edge) {
	    if (edge->EdgeCurveOf())
		c = write_on_curve(writer, edge, 0, trim->m_bRev3d ? 1 : 0);
	    if (!c)
		return 0;
	} else {
	    if (trim->m_type != ON_BrepTrim::singular)
		return 0;
	    complete_model_boundary = false;
	}
	...
	? writer.composite_curve(&bde[0], (int)bde.size())
	: bde[0];
    const int ccurve = !complete_model_boundary ? 0 : (cde.size() > 1)
	? writer.composite_curve(&cde[0], (int)cde.size())
	: cde[0];

    return writer.curve_on_surface(surf_de, bcurve, ccurve);
}
...
int
write_topological_brep(Writer &writer, const ON_Brep &brep, const char *name,
		       const BrepTopologyPlan &plan)
{
    ...
    const int vertex_list_de = writer.vertex_list(vertices.data(),
	vertices.size() / 3);
    if (!vertex_list_de)
	return 0;

    std::vector<BrepEdge> edges(brep.m_E.Count());
    for (int index = 0; index < brep.m_E.Count(); ++index) {
	const ON_BrepEdge &edge = brep.m_E[index];
	edges[index].curve_de = write_on_curve(writer, &edge, 0, 0);
    ...
    const int edge_list_de = writer.edge_list(vertex_list_de,
	...
	if (!surface_de)
	    surface_de = write_on_surface(writer, face.SurfaceOf());
	if (!surface_de)
	    return 0;
	...
	for (std::vector<const ON_BrepLoop *>::const_iterator loop =
	    ...
	    std::vector<BrepLoopUse> uses((*loop)->TrimCount());
	    for (int trim_index = 0; trim_index < (*loop)->TrimCount(); ++trim_index) {
		const ON_BrepTrim *trim = (*loop)->Trim(trim_index);
		BrepLoopUse &use = uses[trim_index];
		use.parameter_curve_de = write_on_curve(writer, trim, 1, 0);
		use.isoparametric = trim->m_iso != ON_Surface::not_iso;
		if (trim->m_ei >= 0) {
		    use.kind = BrepLoopUse::Edge;
		    use.index = static_cast<size_t>(trim->m_ei);
		    use.orientation = trim->m_bRev3d ? 0 : 1;
		} else {
		    use.kind = BrepLoopUse::Vertex;
	    ...
	    const int loop_de = writer.loop(vertex_list_de,
		edge_list_de, uses.data(), uses.size());
	    if (!loop_de)
		return 0;
	    loop_des.push_back(loop_de);
	}
	face_des[face_index] = writer.face(surface_de,
	    loop_des.data(), loop_des.size(), 1);
	...
	shell_des[component] = writer.shell(component_faces.data(),
	    face_orientations.data(), component_faces.size());
    ...
    return writer.solid(name, shell_des[plan.outer_component],
...
int
write_trimmed_brep(Writer &writer, ON_Brep *brep, const char *name)
{
    if (!brep)
	return 0;
	...
	if (!srf) {
	    writer.omission("BRep face " + std::to_string(fi) + " has no surface geometry");
	    continue;
	}

	const int surf_de = write_on_surface(writer, srf);
	if (!surf_de) {
	    writer.omission("BRep face " + std::to_string(fi) + " surface is not representable");
	    continue;
	}
	...
	for (int li = 0; li < face.LoopCount(); li++) {
	    ...
	    if (!loop || (loop->m_type != ON_BrepLoop::outer && loop->m_type != ON_BrepLoop::inner)) {
		complete = false;
		break;
	    }
	    const int cos_de = write_loop(writer, loop, surf_de);
	    if (!cos_de) {
		complete = false;
		break;
	    }
		...
		complete = complete && !outer_de;
		outer_de = cos_de;
	    }
	}

	if (!complete || !outer_de) {
	    writer.omission("BRep face " + std::to_string(fi) + " has an incomplete trimming boundary");
	    continue;
	}

	const int face_id = writer.trimmed_surface(surf_de, outer_de,
    ...
    return writer.group(name ? name : "", faces);
}


int
write_on_brep(Writer &writer, ON_Brep *brep, const char *name)
{
    if (!brep)
	return 0;
    if (writer.options().flatten_brep)
	return write_trimmed_brep(writer, brep, name);
    BrepTopologyPlan plan;
    std::string diagnostic;
    if (preflight_topology(*brep, plan, diagnostic))
	return write_topological_brep(writer, *brep, name, plan);
...
int
export_surface(Writer &writer, const ON_Surface &surface)
{
    return write_on_surface(writer, &surface);
}

int
export_curve(Writer &writer, const ON_Curve &curve, bool planar)
{
    return write_on_curve(writer, &curve, planar ? 1 : 0, 0);
}


ExportedEntity
export_brep(Writer &writer, struct rt_db_internal &internal, const std::string &name)
{
    ON::Begin();
    if (internal.idb_type == ID_BREP) {
	const auto *source = static_cast<const struct rt_brep_internal *>(internal.idb_ptr);
	if (!source || source->magic != RT_BREP_INTERNAL_MAGIC || !source->brep)
	    return {};
	return {write_on_brep(writer, source->brep, name.c_str()), true};
    }
    if (internal.idb_type <= ID_NULL || internal.idb_type > ID_MAXIMUM ||
	!OBJ[internal.idb_type].ft_brep)
	return {};
    auto brep = primitive_brep(internal, writer.options().tolerance);
    if (!brep || !brep->IsValid())
	return {};
    if (internal.idb_type == ID_CLINE &&
	static_cast<const struct rt_cline_internal *>(internal.idb_ptr)->thickness > 0.0)
	bu_log("g-iges: %s CLINE plate thickness exported as a fixed wall\n", name.c_str());
    return {write_on_brep(writer, brep.get(), name.c_str()), true};
}
