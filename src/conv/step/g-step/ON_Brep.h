...
struct ON_Brep_Info_AP203 {
    ...
    std::vector<STEPentity *> shells;
    std::vector<STEPentity *> oriented_shells;
    STEPentity *closed_shell;
    STEPentity *manifold_solid_brep;
    STEPentity *advanced_brep;

    std::map<STEPentity*, GenericAggregate * > surf_genagg;
    std::map<STEPentity*, std::vector<std::vector<STEPentity *> > > surface_cv;
};

void ON_3dPoint_to_Cartesian_point(ON_3dPoint *inpnt, STEPentity *step_pnt);
void ON_3dVector_to_Direction(ON_3dVector *invect, STEPentity *step_direction);
...
bool ON_BRep_to_STEP(struct directory *dp, ON_Brep *brep, AP203_Contents *sc,
        STEPentity **brep_shape, STEPentity **brep_product, STEPentity **brep_manifold,
	bool create_product = true, std::string *diagnostic = NULL);
