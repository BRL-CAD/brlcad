if(NOT DEFINED STEP_G OR NOT DEFINED INPUT OR NOT DEFINED OUTPUT_A OR NOT DEFINED OUTPUT_B)
  message(FATAL_ERROR "STEP_G, INPUT, OUTPUT_A, and OUTPUT_B are required")
endif()

file(REMOVE "${OUTPUT_A}" "${OUTPUT_B}")
execute_process(
  COMMAND "${CMAKE_COMMAND}" -E env MALLOC_PERTURB_=17 "${STEP_G}" -O "${OUTPUT_A}" "${INPUT}"
  RESULT_VARIABLE result_a
  OUTPUT_VARIABLE stdout_a
  ERROR_VARIABLE stderr_a
)
execute_process(
  COMMAND "${CMAKE_COMMAND}" -E env MALLOC_PERTURB_=203 "${STEP_G}" -O "${OUTPUT_B}" "${INPUT}"
