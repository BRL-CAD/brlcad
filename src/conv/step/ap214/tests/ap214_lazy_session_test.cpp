...
main(int argc, char **argv)
{
    ...
    const std::vector<brlcad::step::StepSchemaRuntime::Alias> aliases = {
	{"DESIGN_CONTEXT", "PRODUCT_DEFINITION_CONTEXT"},
	{"MECHANICAL_CONTEXT", "PRODUCT_CONTEXT"}
    };
    const brlcad::step::StepSchemaRuntime runtime("ap214", SchemaInit, aliases);

    brlcad::step::STEPLazySession session(runtime);
    ...
    brlcad::step::STEPLazySession diagnostic_session(runtime);
    ...
    brlcad::step::STEPLazySession legacy_context_session(runtime);
    ...
    brlcad::step::STEPLazySession cancelled_session(runtime);
