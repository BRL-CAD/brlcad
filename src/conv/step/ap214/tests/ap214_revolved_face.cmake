...
execute_process(
  COMMAND "${MGED}" -c "${OUTPUT}" brep Revolved_Face_swept_item.s info
  OUTPUT_VARIABLE brep_output
  ERROR_VARIABLE brep_error
)
set(brep_text "${brep_output}\n${brep_error}")
if(NOT brep_text MATCHES "Valid: YES, Solid: YES")
  message(FATAL_ERROR "revolved-face BREP validation failed\n${brep_output}\n${brep_error}")
endif()

execute_process(
  COMMAND "${MGED}" -c "${OUTPUT}" attr show Revolved_Face_swept_item
  OUTPUT_VARIABLE attr_output
  ERROR_VARIABLE attr_error
)
set(attr_text "${attr_output}\n${attr_error}")
if(NOT attr_text MATCHES "step:source_id[ ]+43")
  message(FATAL_ERROR "revolved-face metadata validation failed\n${attr_output}\n${attr_error}")
endif()

execute_process(
  COMMAND "${MGED}" -c "${OUTPUT}" bb -q -e Revolved_Face_swept_item.s
...
if(NOT bbox_text MATCHES "min \\{-7\\.00[0-9]+ -2\\.00[0-9]+ -7\\.00[0-9]+\\} max \\{7\\.00[0-9]+ 2\\.00[0-9]+ 7\\.00[0-9]+\\}")
  message(FATAL_ERROR "revolved-face bounding box validation failed\n${bbox_output}\n${bbox_error}")
endif()
