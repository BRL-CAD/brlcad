if(NOT DEFINED STEP_G OR NOT DEFINED INPUT OR NOT DEFINED OUTPUT_SAFE OR
    NOT DEFINED OUTPUT_NONE OR NOT DEFINED REPORT_SAFE OR NOT DEFINED REPORT_NONE)
  message(FATAL_ERROR "STEP_G, INPUT, outputs, and reports are required")
endif()

file(REMOVE "${OUTPUT_SAFE}" "${OUTPUT_NONE}" "${REPORT_SAFE}" "${REPORT_NONE}")
execute_process(
  COMMAND "${STEP_G}" --repair safe -O "${OUTPUT_SAFE}" --report "${REPORT_SAFE}" "${INPUT}"
  RESULT_VARIABLE safe_result
  OUTPUT_VARIABLE safe_output
  ERROR_VARIABLE safe_error
)
execute_process(
  COMMAND "${STEP_G}" --repair none -O "${OUTPUT_NONE}" --report "${REPORT_NONE}" "${INPUT}"
