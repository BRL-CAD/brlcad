...
public:
    ...
    explicit STEPLazySession(const StepSchemaRuntime &runtime);
    ~STEPLazySession();
    ...
    std::vector<std::string> ComponentTypes(uint64_t id) const;
    ...
    size_t SchemaEntityCount() const;
    std::string SchemaName() const;
    InstMgrBase *ReferenceManager() const;
    const EntityDescriptor *Entity(const char *name) const;
    const TypeDescriptor *Type(const char *name) const;
