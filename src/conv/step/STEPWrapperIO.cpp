...
STEPWrapper::parseListOfReals(const char *in)
{
    ...
    ra->StrToVal(in, &errdesc, SchemaType("PARAMETER_VALUE"), referenceManager(), 0);
...
STEPWrapper::parseListOfPointEntities(const char *in)
{
    ...
    ag->StrToVal(in, &errdesc, SchemaEntity("CARTESIAN_POINT"), referenceManager(), 0);
...
{
    size_t schema_entities = 0;
    std::string schema_name = SchemaName();
#ifdef HAVE_STEPCODE_LAZY
    if (lazy_session) {
	schema_entities = lazy_session->SchemaEntityCount();
	const std::string lazy_schema_name = lazy_session->SchemaName();
	if (!lazy_schema_name.empty()) schema_name = lazy_schema_name;
    } else
#endif
    if (registry) {
	schema_entities = static_cast<size_t>(registry->GetEntityCnt());
	registry->ResetSchemas();
	const SchemaDescriptor *schema = registry->NextSchema();
	if (schema && schema->Name()) schema_name = schema->Name();
    }

    std::cout << "Loaded " << Statistics().input_instances << " instances from ";
    ...
    const size_t count_column = 69;
    for (std::map<std::string, uint64_t>::const_iterator type =
	    Document().entity_counts.begin(); type != Document().entity_counts.end(); ++type) {
	std::string display_name = type->first;
	const EntityDescriptor *descriptor = SchemaEntity(type->first.c_str());
	if (descriptor && descriptor->Name()) display_name = descriptor->Name();
	std::cout << '\t' << display_name;
	if (display_name.size() < count_column)
	    std::cout << std::string(count_column - display_name.size(), '.');
	else
	    std::cout << ' ';
	std::cout << type->second << std::endl;
    }
    std::cout << "Used " << Document().entity_counts.size()
	<< " entities of the available " << schema_entities << " in schema \""
