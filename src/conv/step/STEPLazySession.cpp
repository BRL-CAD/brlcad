...
public:
    explicit Impl(const StepSchemaRuntime &runtime)
	: registry(new Registry(runtime.Initializer())), manager(new lazyInstMgr)
    {
	manager->setRegistry(registry.get());
	for (const StepSchemaRuntime::Alias &alias : runtime.LazyAliases())
	    manager->setMaterializationTypeAlias(alias.first, alias.second);
	install_callbacks();
    }
...
STEPLazySession::STEPLazySession(const StepSchemaRuntime &runtime) : impl(new Impl(runtime)) {}
...
STEPLazySession::ComponentTypes(uint64_t id) const
{
    if (!impl) return std::vector<std::string>();
    const std::vector<std::string> &types =
	impl->manager->componentTypes(static_cast<instanceID>(id));
    return types;
}
...
size_t
STEPLazySession::SchemaEntityCount() const
{
    return impl ? static_cast<size_t>(impl->registry->GetEntityCnt()) : 0;
}

std::string
STEPLazySession::SchemaName() const
{
    if (!impl) return std::string();
    impl->registry->ResetSchemas();
    const SchemaDescriptor *schema = impl->registry->NextSchema();
    return schema && schema->Name() ? schema->Name() : std::string();
}
...
STEPLazySession::Entity(const char *name) const
{
    return impl && name ? impl->registry->FindEntity(name) : NULL;
}

const TypeDescriptor *
STEPLazySession::Type(const char *name) const
{
    return impl && name ? impl->registry->FindType(name) : NULL;
}
