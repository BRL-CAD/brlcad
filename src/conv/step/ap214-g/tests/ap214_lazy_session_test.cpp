...
{
    if (argc != 3) {
	std::cerr << "usage: ap214_lazy_session_test input.stp diagnostics.stp\n";
	return 2;
    }

    brlcad::step::AP214LazySession session;
    uint64_t progress_events = 0;
    session.SetProgressCallback([&progress_events](const brlcad::step::AP214LazyProgress &) {
	++progress_events;
    });
    expect(session.Open(argv[1]), "lazy AP214 scan succeeds");

    const brlcad::step::AP214LazyStatistics scanned = session.Statistics();
	...
	{
	    brlcad::step::AP214LazyBatch first = session.LoadBatch(products[0]);
	    ...
	    const brlcad::step::AP214LazyStatistics first_stats = session.Statistics();
	    ...
	    {
		brlcad::step::AP214LazyBatch second = session.LoadBatch(products[0]);
	...
	for (int cycle = 0; cycle < 3; ++cycle) {
	    {
		brlcad::step::AP214LazyBatch batch = session.LoadBatch(solids[0]);
    ...
    const brlcad::step::AP214LazyStatistics final_stats = session.Statistics();
    ...
    brlcad::step::AP214LazySession diagnostic_session;
    uint64_t diagnostic_events = 0;
    diagnostic_session.SetDiagnosticCallback(
	[&diagnostic_events](const brlcad::step::AP214LazyDiagnostic &) {
	    ++diagnostic_events;
	});
    expect(diagnostic_session.Open(argv[2]), "malformed-reference fixture is indexed");
    const std::vector<brlcad::step::AP214LazyDiagnostic> &diagnostics =
    ...
    brlcad::step::AP214LazySession cancelled_session;
    ...
    const brlcad::step::AP214LazyStatistics cancelled_stats =
