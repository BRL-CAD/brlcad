...
/* Match the AP214 importer's bounded automatic concurrency.  Exact BREP jobs can retain
...
#if defined(AP203e2)
#  define STEP_IMPORT_CLI_NAME step_ap203e2_import_cli
#elif defined(AP242)
#  define STEP_IMPORT_CLI_NAME step_ap242_import_cli
#else
#  define STEP_IMPORT_CLI_NAME step_ap203_import_cli
#endif

extern "C" int
STEP_IMPORT_CLI_NAME(int argc, const char *argv[])
{
    int ret = 0;
	...
	{"S", "summary", "FILE", bu_opt_str, &summary_log_file, "per-instance conversion status CSV"},
	...
	if (bu_file_exists(ofile.filename, NULL)) {
	    bu_log("ERROR: refusing to overwrite existing output file:"
	...
	bu_log("ERROR: unable to read input \"%s\" STEP file\n", argv[0]);
	return 2;
    }
	...
	if (!dotg) {
		...
		if (signal_exit_status()) {
		    ...
		    ret = 4;
		} else if (stats.outcome == "empty") {
		    ret = STEP_PLUGIN_STATUS_EMPTY;
		} else if (stats.geometry_written == 0 ||
			stats.outcome == "failed") {
		    ret = 3;
		} else if (stats.outcome == "partial") {
		    ret = 1;
		}
		...
		else if (ret == STEP_PLUGIN_STATUS_EMPTY)
		    std::cerr << "no geometric output was requested by the represented products" << std::endl;
		else if (ret == 4)
