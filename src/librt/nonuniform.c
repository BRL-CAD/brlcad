...
#define NU_MAT_VALUE_BUFSIZE (NU_MAT_PRECISION + 16)
...
_rt_nonuniform_attr_set(struct rt_db_internal *ip, const mat_t mat)
{
    ...
    for (i = 0; i < 16; i++) {
	char value[NU_MAT_VALUE_BUFSIZE];
	int written = snprintf(value, sizeof(value), "%.*g", NU_MAT_PRECISION, (double)mat[i]);
