...
HIDDEN void
_db5_calc_subsize(int *ret, union tree *tp, struct db_i *dbip, int *depth, int max_depth, int flags,
	void (*traverse_func) (int *ret, struct directory *, struct db_i *, int *, int, int))
{
    struct directory *dp;
    if (!tp) return;
    RT_CHECK_DBI(dbip);
	...
	case OP_SUBTRACT:
	case OP_XOR:
	    _db5_calc_subsize(ret, tp->tr_b.tb_right, dbip, depth, max_depth, flags, traverse_func);
	case OP_NOT:
	case OP_GUARD:
	case OP_XNOP:
	    _db5_calc_subsize(ret, tp->tr_b.tb_left, dbip, depth, max_depth, flags, traverse_func);
	    (*depth)--;
	    break;
	case OP_DB_LEAF:
		...
		if (!(dp->d_flags & RT_DIR_HIDDEN)) {
		    traverse_func(ret, dp, dbip, depth, max_depth, flags);
		}
		(*depth)--;
		break;
	    }
...
HIDDEN void
_db5_calc_size(int *ret, struct directory *dp, struct db_i *dbip, int *depth, int max_depth, int flags)
{
	...
	_db5_calc_subsize(ret, comb->tree, dbip, depth, max_depth, flags, _db5_calc_size);
...
db5_size(struct db_i *dbip, struct directory *dp, int flags)
{
	...
	_db5_calc_size(&full_size, dp, dbip, &depth, local_flags, DB_SIZE_CYCLIC_LIMIT);
    } else {
	_db5_calc_size(&full_size, dp, dbip, &depth, local_flags, 0);
    }
