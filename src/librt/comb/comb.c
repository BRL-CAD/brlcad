 ...
 *
 * NOTE - we're faced with a dilemma here.  From an API standpoint, this is the
 * logical place for the comb -> NMG tessellation logic.  However, putting it
 * here requires the rt_db_internal to have knowledge of it's parent dbip and
 * dp, and for those to be current.  That is not something historically
 * required for rt_db_internal.
 *
 * Using another tree walker it might be possible to avoid the need for src_dp
 * here, but a valid dbip is a hard requirement since tessellating a comb
 * requires knowing about the geometry definitions of its leaves.
 *
 ...
 * API guarantee that such a rt_db_internal couldn't be created.  The struct
 * slots in rt_db_internal strongly caution against casual use of ip->src_*
 * vars, and anyone reading this code should also take note - ONLY use src_dbip
 * and src_dp when that's the only way and ALWAYS validate with RT_CK_*.
 */
int
rt_comb_tess(struct nmgregion **r, struct model *m, struct rt_db_internal *ip, const struct bg_tess_tol *ttol, const struct bn_tol *tol)
{
    ...
    db_init_db_tree_state(&init_state, (struct db_i *)ip->src_dbip, &rt_uniresource);
    ...
    const char *argv[2];
    argv[0] = ip->src_dp->d_namep;
    argv[1] = NULL;


    if (!BU_SETJUMP) {
        /* try */
        i = db_walk_tree((struct db_i *)ip->src_dbip, 1, (const char **)argv,
                         1,
                        &init_state,
                         0,                     /* take all regions */
                         facetize_region_end,
    ...
    if (facetize_tree) {
        if (!BU_SETJUMP) {
            /* try */
            failed = nmg_boolean(facetize_tree, m, &RTG.rtg_vlfree, tol, &rt_uniresource);
        } else {
            /* catch */
            BU_UNSETJUMP;
            return BRLCAD_ERROR;
