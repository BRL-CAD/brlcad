...
static int
prim_tess_metric_failed(fastf_t value)
{
    return !isfinite(value) || NEAR_EQUAL(value, -1.0, SMALL_FASTF);
}
	       ...
	       const char *bot_name_override)
{
	...
	if (!BU_SETJUMP) {
		...
		if (prim_tess_metric_failed(analytic_sa)) {
		...
		if (prim_tess_metric_failed(analytic_v)) {
		    analytic_query_failed = 1;
		    if (!quiet)
	...
	if (g_validate_metrics && !quiet && (!ft->ft_surf_area || !ft->ft_volume)) {
		...
		if (!prim_tess_metric_failed(croft_sa)) {
		...
		if (!prim_tess_metric_failed(croft_v)) {
		    analytic_v = croft_v;
		    have_analytic_v = 1;
		}
	    }
...
scan_input_g(const char *g_path, const char *g_root)
{
	...
	if (ok && !g_no_crofton) {
		...
		if (rt_gettree(cr_rtip, dp->d_namep) == 0) {
		    ...
		    cr = rt_crofton_shoot(&csa, &cv, NULL, NULL, NULL, NULL,
			NULL, cr_rtip, &crp, NULL, NULL);
		}
