...
static int
body_internal_init(struct rt_db_internal *body, const struct rt_db_internal *ip)
{
    /* The body view shares geometry with ip but owns its attribute set. */
...
rt_obj_volume(fastf_t *volume, const struct rt_db_internal *ip)
{
    ...
    const struct rt_db_internal *metric_ip = ip;
    int id;
	...
	return -4;
    if (have_nonuniform > 0) {
	if (body_internal_init(&body, ip) < 0)
	    return -4;
	metric_ip = &body;
    }

    if (OBJ[id].ft_volume) {
	OBJ[id].ft_volume(volume, metric_ip);
    } else {
	struct rt_crofton_params params = {50000u, 0.0, 0.0};
	rt_crofton_sample(NULL, volume, metric_ip, &params);
	if (*volume <= 0.0)
	    ret = -3;
    }

    if (!ret && have_nonuniform > 0)
