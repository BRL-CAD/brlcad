...
rt_vol_tess(struct nmgregion **r, struct model *m, struct rt_db_internal *ip, const struct bg_tess_tol *ttol, const struct bn_tol *tol)
{
    ...
    /* fuse model */
    nmg_model_fuse(m_tmp, &rtg_vlfree, tol);
	...
	for (BU_LIST_FOR(lu, loopuse, &fu->lu_hd))
	    (void)nmg_kill_snakes(lu,&rtg_vlfree);
    }

    (void)nmg_unbreak_region_edges((uint32_t *)(&s->l),&rtg_vlfree);

    (void)nmg_mark_edges_real((uint32_t *)&s->l,&rtg_vlfree);
