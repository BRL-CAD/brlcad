...
extern "C" {
    extern void rt_arb_tess(struct shell **s, struct rt_db_internal *ip, const struct rt_tess_tol *ttol, const struct bn_tol *tol);
...
rt_arb_brep(ON_Brep **b, const struct rt_db_internal *ip, const struct bn_tol *tol)
{
    ...
    rt_arb_tess(&s, tmp_internal, ttol, tol);
