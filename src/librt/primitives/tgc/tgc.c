...
clt_shot(size_t size, struct cl_hit *hits, struct xray *rp, struct soltab *stp)
{
    ...
    struct tgc_shot_specific in;

    VMOVE(r_pt.s, rp->r_pt);
    VMOVE(r_dir.s, rp->r_dir);
    
    VMOVE(in.tgc_V, tgc->tgc_V);
    in.tgc_sH = tgc->tgc_sH;
    in.tgc_A = tgc->tgc_A;
    in.tgc_B = tgc->tgc_B;
    in.tgc_CdAm1 = tgc->tgc_CdAm1;
    in.tgc_DdBm1 = tgc->tgc_DdBm1;
    in.tgc_AAdCC = tgc->tgc_AAdCC;
    in.tgc_BBdDD = tgc->tgc_BBdDD;
    VMOVE(in.tgc_N, tgc->tgc_N);
    MAT_COPY(in.tgc_ScShR, tgc->tgc_ScShR);
    in.tgc_AD_CB = tgc->tgc_AD_CB;
