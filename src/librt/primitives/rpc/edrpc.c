...
static int
rt_solid_edit_rpc_pscale(struct rt_solid_edit *s)
{
    if (s->e_inpara > 1) {
...
int
rt_solid_edit_rpc_edit(struct rt_solid_edit *s)
{
    switch (s->edit_flag) {
	...
	case RT_SOLID_EDIT_ROT:
	    ...
	    break;
	default:
	    return rt_solid_edit_rpc_pscale(s);
    }
    return 0;
}

int
rt_solid_edit_rpc_edit_xy(
