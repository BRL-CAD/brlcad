...
get_tcl_curve(struct bu_vls *logstr, struct rt_curve *crv, const char *argv1)
{
	...
	if (BU_STR_EQUAL(seg_argv[0], "line")) {
		...
		switch (*elem) {
			...
			num_points = 0;
			(void)tcl_list_to_int_array(sval, &bsg->ctl_points, &num_points);
		...
		switch (*elem) {
			...
			break;
		    case 'K':
			(void)tcl_list_to_fastf_array(sval, &nsg->k.knots, &nsg->k.k_size);
			break;
		    case 'P' :
			(void)tcl_list_to_int_array(sval, &nsg->ctl_points, &nsg->c_size);
			break;
		    case 'W':
			(void)tcl_list_to_fastf_array(sval, &nsg->weights, &nsg->c_size);
			break;
		}
	    }
...
rt_sketch_adjust(struct bu_vls *logstr, struct rt_db_internal *intern, int argc, const char **argv)
{
	...
	if (BU_STR_EQUAL(argv[0], "V")) {
	    ...
	    len = 0;
	    (void)tcl_list_to_fastf_array(dupstr, &new_verts, &len);
	    ...
	    array_len = 2;
	    if (tcl_list_to_fastf_array(argv[1], &new_vert, &array_len) != array_len) {
		bu_vls_printf(logstr, "ERROR: Incorrect number of coordinates for vertex\n");
		return BRLCAD_ERROR;
	    }
	}
