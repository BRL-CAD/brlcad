...
db_setup(FILE *fp, const char *name, int version)
{
    ...
    // If fp is read-only this will fail
    int result;
    if (version == 5) {
    ...
    dbip->dbi_filename = (name) ? bu_strdup(name) : bu_strdup("db_setup (NULL)");
	...
	if (name)
	    bu_log("db_setup error: db_dirbuild of %s failed\n", name);
    }
