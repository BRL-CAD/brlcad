...
proc PanedWindow::add { path args } {
    ...
    # RAMSAN
    set minsize [Widget::getoption $path.f$num -minsize]
	...
	if {$activator == "button"} {
	    ...
	    #$sep configure -bd 3
	    $sep configure -bd 1
	    $frame configure -width 4 -height 3
	    set placeButton 0
	}
	...
	# RAMSAN minsize
	grid columnconfigure $path [expr {2*$num}] -weight $weight -minsize $minsize
    } else {
	grid $pane -row [expr {2*$num}] -column 0 -sticky nsew
	# RAMSAN minsize
	grid rowconfigure $path [expr {2*$num}] -weight $weight -minsize $minsize
    }
