...
void ON_3dmView::Dump( ON_TextLog& dump ) const
{
  ...
  if ( bValidCamera ) {
    ...
    ON_3dPoint target_point = m_target;
...
void ON_3dmView::Default()
{
  ...
  m_vp.Initialize();
  m_target = ON_origin;
  //m_vp.GetFrustumCenter(m_target);
  m_cplane.Default();
...
bool ON_3dmView::Write( ON_BinaryArchive& file ) const
{
  ...
  if(rc) {
    ...
    if(rc) {
      if(rc) rc = file.WritePoint(m_target);
      if ( !file.EndWrite3dmChunk() )
        rc = false;
    }
  }
...
static bool ON_3dmSettings_Read_v1_TCODE_VIEW(ON_BinaryArchive& file, ON_3dmView& view)
{
  ...
  while(rc)
  {
    ...
    if (!rc) break;
    rc = file.ReadPoint( view.m_target );
    if (!rc) break;
