...
{
  BOOL rc = ( m_real_curve ) ? m_real_curve->IsArc(plane,arc,tolerance) : false;
        ...
        ) const
{
	...
	if( m_bReversed)
  {
	...
	BOOL rc =  m_real_curve->GetNormalizedArcLengthPoints(count, s, t , absolute_tolerance, fractional_tolerance, &scratch_domain);
	if( rc)
  {
		for(i=0;i<count; i++)
    {
