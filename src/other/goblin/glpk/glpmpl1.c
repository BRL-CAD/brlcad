...
{     char *image, *s;
      if (mpl->token == T_EOF)
         ...
         image = mpl->image;
      insist(0 <= mpl->c_ptr && mpl->c_ptr < CONTEXT_SIZE);
...
{     insist(0 <= mpl->imlen && mpl->imlen <= MAX_LENGTH);
...
conv:    /* convert numeric literal to floating-point */
         if (str2dbl(mpl->image, &mpl->value))
err:     {  enter_context(mpl);
      ...
      {  /* symbol */
         ...
         while (isalnum(mpl->c) || strchr("+-._", mpl->c) != NULL)
            append_char(mpl);
         switch (str2dbl(mpl->image, &mpl->value))
         {  case 0:
               mpl->token = T_NUMBER;
               break;
            case 1:
...
{     CODE *code;
      ...
      {  case O_NUMBER:
         ...
         case O_SQRT:
            /* unary operation */
            insist(arg->arg.x != NULL);
            insist(arg->arg.x->up == NULL);
            arg->arg.x->up = code;
         ...
         case O_NOTWITHIN:
            ...
            insist(arg->arg.x->up == NULL);
            arg->arg.x->up = code;
            insist(arg->arg.y != NULL);
            insist(arg->arg.y->up == NULL);
            arg->arg.y->up = code;
            code->arg.arg.x = arg->arg.x;
         ...
         case O_BUILD:
            ...
            insist(domain != NULL);
            if (domain->code != NULL)
            {  insist(domain->code->up == NULL);
               domain->code->up = code;
            }
            for (block = domain->list; block != NULL; block =
               block->next)
            {  insist(block->code != NULL);
               insist(block->code->up == NULL);
               block->code->up = code;
            }
            if (arg->loop.x != NULL)
            {  insist(arg->loop.x->up == NULL);
               arg->loop.x->up = code;
            }
...
{     AVLNODE *node;
      ...
      insist(mpl->token == T_NAME);
      node = avl_find_by_key(mpl->tree, mpl->image);
      if (node == NULL)
      ...
      {  case A_INDEX:
         ...
         case A_PARAMETER:
            /* model parameter */
            par = (PARAMETER *)node->link;
            name = par->name;
            dim = par->dim;
            break;
         case A_VARIABLE:
            /* model variable */
            var = (VARIABLE *)node->link;
            name = var->name;
            dim = var->dim;
            break;
         case A_CONSTRAINT:
            /* model constraint or objective */
            con = (CONSTRAINT *)node->link;
...
-- This routine parses primary expression using the syntax:
--
-- <primary expression> ::= abs ( <argument> )
...
-- <primary expression> ::= min ( <argument list> )
-- <primary expression> ::= sqrt ( <argument> )
...
{     CODE *code;
      ...
      {  /* other functions allow only one argument */
         arg.arg.x = numeric_argument(mpl, func);
         /* check a token that follows the argument */
         if (mpl->token == T_COMMA)
            error(mpl, "%s allows only one argument", func);
         else if (mpl->token == T_RIGHT)
            ;
         else
      ...
      {  if (dim > max_dim)
         ...
         {  /* symbolic name is recognized as dummy index only if:
            ...
            if (!(flag_x &&
                  ...
                  avl_find_by_key(mpl->tree, mpl->image) == NULL))
...
expr:    /* parse expression that follows either the keyword 'in' (in
         ...
         if (slot->name != NULL)
         {  AVLNODE *node;
            insist(avl_find_by_key(mpl->tree, slot->name) == NULL);
            node = avl_insert_by_key(mpl->tree, slot->name);
            node->type = A_INDEX;
            node->link = (void *)slot;
         }
...
{     DOMAIN_BLOCK *block;
      ...
      {  for (slot = block->list; slot != NULL; slot = slot->next)
         {  if (slot->name != NULL)
            {  node = avl_find_by_key(mpl->tree, slot->name);
               insist(node != NULL);
               insist(node->type == A_INDEX);
...
{     CODE *code;
      ...
      insist(strlen(opstr) < sizeof(opstr));
      get_token(mpl /* <symbolic name> */);
...
{     SET *set;
      ...
      if (avl_find_by_key(mpl->tree, mpl->image) != NULL)
      ...
      {  AVLNODE *node;
         node = avl_insert_by_key(mpl->tree, set->name);
         node->type = A_SET;
         node->link = (void *)set;
      }
...
{     PARAMETER *par;
      ...
      {  AVLNODE *node;
         node = avl_insert_by_key(mpl->tree, par->name);
...
{     VARIABLE *var;
      ...
      {  AVLNODE *node;
         node = avl_insert_by_key(mpl->tree, var->name);
...
{     CONSTRAINT *con;
      ...
      {  AVLNODE *node;
         node = avl_insert_by_key(mpl->tree, con->name);
...
{     CONSTRAINT *obj;
      ...
      /* include the constraint name in the symbolic names table */
      {  AVLNODE *node;
         node = avl_insert_by_key(mpl->tree, obj->name);
         node->type = A_CONSTRAINT;
         node->link = (void *)obj;
      }
...
{     DISPLAY *dpy;
      ...
      {  /* create new display entry */
         ...
         {  AVLNODE *node;
            ...
            if (node == NULL)
               error(mpl, "%s not defined", mpl->image);
            if (next_token == T_LBRACKET && node->type != A_CONSTRAINT)
            {  /* symbolic name begins expression */
               goto expr;
            }
            ...
            switch (node->type)
            {  case A_INDEX:
                  entry->u.slot = (DOMAIN_SLOT *)node->link;
                  break;
               case A_SET:
                  entry->u.set = (SET *)node->link;
                  break;
               case A_PARAMETER:
                  entry->u.par = (PARAMETER *)node->link;
                  break;
               case A_VARIABLE:
                  entry->u.var = (VARIABLE *)node->link;
                  break;
               case A_CONSTRAINT:
                  entry->u.con = (CONSTRAINT *)node->link;
                  break;
               default:
                  insist(node != node);
            }
            ...
            if (mpl->token == T_LBRACKET)
            {  CONSTRAINT *con;
               insist(entry->type == A_CONSTRAINT);
               con = entry->u.con;
               if (con->dim == 0)
...
-- <end statement> ::= end
--
-- Semicolon and end of file must follow the end statement. */
...
{     STATEMENT *stmt;
      insist(mpl->model == NULL);
      /* parse model section */
      while (!(mpl->token == T_EOF ||
               is_keyword(mpl, "data") || is_keyword(mpl, "end")))
      {  /* create new statement descriptor and append it to the list
            of model statements */
         if (mpl->model == NULL)
            stmt = (mpl->model = alloc(STATEMENT));
         else
            stmt = (stmt->next = alloc(STATEMENT));
         stmt->line = mpl->line;
         stmt->next = NULL;
         /* parse particular model statement */
