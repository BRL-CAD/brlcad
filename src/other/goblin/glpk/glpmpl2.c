...
{     while (!(mpl->token == T_EOF || is_literal(mpl, "end")))
      {  if (is_literal(mpl, "set"))
            ...
            parameter_data(mpl);
         else if (is_literal(mpl, "check"))
         {  warning(mpl, "check statement not allowed in data section; "
               "statement ignored");
            while (mpl->token != T_SEMICOLON)
            {  if (mpl->token == T_EOF)
                  error(mpl, "semicolon missing where expected");
               get_token(mpl);
            }
         }
         else if (is_literal(mpl, "display"))
         {  warning(mpl, "display statement not allowed in data section"
               "; statement ignored");
            while (mpl->token != T_SEMICOLON)
            {  if (mpl->token == T_EOF)
                  error(mpl, "semicolon missing where expected");
