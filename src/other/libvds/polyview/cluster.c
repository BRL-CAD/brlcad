...
vdsNode *clusterOctree(vdsNode **nodes, int nnodes, int depth)
{
	...
	for (j=0;j<3;j++)
	{
	    if (nodes[i]->coord[j] > max[j]) { max[j] = nodes[i]->coord[j]; }
	    if (nodes[i]->coord[j] < min[j]) { min[j] = nodes[i]->coord[j]; }
	}
	...
	{
	    int whichchild = 0;
	    if (nodes[i]->coord[0] > center[0]) { whichchild |= 1; }
	    if (nodes[i]->coord[1] > center[1]) { whichchild |= 2; }
	    if (nodes[i]->coord[2] > center[2]) { whichchild |= 4; }
    ...
    for (i=0;i<8;i++)
    {
	if (nchildnodes[i]) { free(childnodes[i]); }
    }
    return thisnode;
}
