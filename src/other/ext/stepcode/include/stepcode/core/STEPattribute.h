...
extern STEPCODE_CORE_EXPORT int SetErrOnNull( const char * attrValue, ErrorDescriptor * error );

extern STEPCODE_CORE_EXPORT SDAI_Application_instance * ReadEntityRef( istream & in, ErrorDescriptor * err, const char * tokenList,
        InstMgr * instances, int addFileId );

extern STEPCODE_CORE_EXPORT SDAI_Application_instance * ReadEntityRef( const char * s, ErrorDescriptor * err, const char * tokenList,
        InstMgr * instances, int addFileId );

extern STEPCODE_CORE_EXPORT Severity EntityValidLevel( SDAI_Application_instance * se,
        const TypeDescriptor * ed, ///< entity type that entity se needs to match. (this must be an EntityDescriptor)
        ErrorDescriptor * err );

extern STEPCODE_CORE_EXPORT Severity EntityValidLevel( const char * attrValue, ///< string containing entity ref
...
extern STEPCODE_CORE_EXPORT SDAI_Application_instance * STEPread_reference( const char * s, ErrorDescriptor * err,
        InstMgr * instances, int addFileId );
////////////////////

extern STEPCODE_CORE_EXPORT int QuoteInString( istream & in );

extern STEPCODE_CORE_EXPORT void AppendChar( char c, int & index, char *& s, int & sSize );
...
class STEPCODE_CORE_EXPORT STEPattribute {
...
///////////// Read, Write, Assign attr value
        ...
        void STEPwrite( std::ostream & out = std::cout, const char * = 0 );
