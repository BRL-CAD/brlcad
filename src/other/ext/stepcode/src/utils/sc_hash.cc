...
#define STEPCODE_HASH_Table_new()        new Hash_Table
#define STEPCODE_HASH_Table_destroy(x)   delete x
#define STEPCODE_HASH_Element_new()      new Element
#define STEPCODE_HASH_Element_destroy(x) delete x
...
Address     STEPCODE_HASHhash( char *, Hash_TableP );
static void STEPCODE_HASHexpand_table( Hash_TableP );
struct Element *STEPCODE_HASHsearch( Hash_TableP table, const struct Element * item, Action action );
...
void *
STEPCODE_HASHfind( Hash_TableP t, char * s ) {
    struct Element e;
    struct Element * ep;

    e.key = s;
    e.symbol = 0; /*  initialize to 0 - 25-Apr-1994 - kcm */
    ep = STEPCODE_HASHsearch( t, &e, HASH_FIND );
    return( ep ? ep->data : 0 );
}

void
STEPCODE_HASHinsert( Hash_TableP t, char * s, void * data ) {
    struct Element e, *e2;

    e.key = s;
    e.data = data;
    e.symbol = 0;
    e2 = STEPCODE_HASHsearch( t, &e, HASH_INSERT );
    if( e2 ) {
...
Hash_TableP
STEPCODE_HASHcreate( unsigned count ) {
    unsigned int i;
    Hash_TableP table;
    ...
    */
    ...
    table = ( Hash_TableP ) STEPCODE_HASH_Table_new();
...
void
STEPCODE_HASHlistinit( Hash_TableP table, HashEntry * he ) {
    he->i = he->j = 0;
    he->p = 0;
    he->table = table;
    he->type = '*';
...
void
STEPCODE_HASHlistinit_by_type( Hash_TableP table, HashEntry * he, char type ) {
    he->i = he->j = 0;
    he->p = 0;
    he->table = table;
    he->type = type;
...
struct Element *
STEPCODE_HASHlist( HashEntry * he ) {
    int i2 = he->i;
    int j2 = he->j;
    struct Element ** s;
...
void
STEPCODE_HASHdestroy( Hash_TableP table ) {
    struct Element ** s;
    struct Element * p, *q;
    ...
    if( table != HASH_NULL ) {
        ...
        for( i = 0; i < table->SegmentCount; i++ ) {
            ...
            if( ( s = table->Directory[i] ) != NULL ) {
                for( j = 0; j < SEGMENT_SIZE; j++ ) {
                    ...
                    while( p != NULL ) {
                        q = p->next;
                        STEPCODE_HASH_Element_destroy( p );
                        p = q;
                    }
                }
        ...
        STEPCODE_HASH_Table_destroy( table );
...
struct Element *
STEPCODE_HASHsearch( Hash_TableP table, const struct Element * item, Action action ) {
    Address h;
...
# ifdef HASH_STATISTICS
    HashAccesses++;
# endif
    h = STEPCODE_HASHhash( item->key, table );
    ...
    switch( action ) {
        ...
        case HASH_DELETE:
            ...
            *p = q->next;
            ...
            STEPCODE_HASH_Element_destroy( deleteme );
            --table->KeyCount;
        ...
        case HASH_INSERT:
            ...
            q = ( ElementP ) STEPCODE_HASH_Element_new();
            *p = q;             /* link into chain  */
            /*
            ...
            */
            if( ++table->KeyCount / MUL( table->SegmentCount, SEGMENT_SIZE ) > table->MaxLoadFactor ) {
                STEPCODE_HASHexpand_table( table );    /* doesn't affect q   */
            }
    }
...
Address
STEPCODE_HASHhash( char * Key, Hash_TableP table ) {
    Address     h, address;
...
static
void
STEPCODE_HASHexpand_table( Hash_TableP table ) {
    ...
    if( table->maxp + table->p < MUL( DIRECTORY_SIZE, SEGMENT_SIZE ) ) {
        ...
        *LastOfNew = NULL;
        while( Current != NULL ) {
            if( STEPCODE_HASHhash( Current->key, table ) == NewAddress ) {
                /*
...
main() {
    ...
    t = STEPCODE_HASHcreate( 100 );
    e = STEPCODE_HASHsearch( t, &e1, HASH_INSERT );
    e = STEPCODE_HASHsearch( t, &e2, HASH_INSERT );
    e = STEPCODE_HASHsearch( t, &e3, HASH_INSERT );
    STEPCODE_HASHlistinit( t, &he );
    for( ;; ) {
        e = STEPCODE_HASHlist( &he );
        if( !e ) {
            exit( 0 );
        }
