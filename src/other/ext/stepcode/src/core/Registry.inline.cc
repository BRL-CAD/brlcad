...
Registry::Registry( CF_init initFunct )
    ...
    primordialSwamp = STEPCODE_HASHcreate( 1000 );
    active_schemas = STEPCODE_HASHcreate( 10 );
    active_types = STEPCODE_HASHcreate( 100 );
    ...
    initFunct( *this );
    STEPCODE_HASHlistinit( active_types, &cur_type );
    STEPCODE_HASHlistinit( primordialSwamp, &cur_entity ); // initialize cur's
    STEPCODE_HASHlistinit( active_schemas, &cur_schema );
}

Registry::~Registry() {
    DeleteContents();

    STEPCODE_HASHdestroy( primordialSwamp );
    STEPCODE_HASHdestroy( active_schemas );
    STEPCODE_HASHdestroy( active_types );
    delete col;
...
void Registry::DeleteContents() {
    // entities first
    STEPCODE_HASHlistinit( primordialSwamp, &cur_entity );
    while( STEPCODE_HASHlist( &cur_entity ) ) {
        delete( EntityDescriptor * ) cur_entity.e->data;
    }

    // schemas
    STEPCODE_HASHlistinit( active_schemas, &cur_schema );
    while( STEPCODE_HASHlist( &cur_schema ) ) {
        delete( Schema * ) cur_schema.e->data;
    }

    // types
    STEPCODE_HASHlistinit( active_types, &cur_type );
    while( STEPCODE_HASHlist( &cur_type ) ) {
...
const EntityDescriptor * Registry::FindEntity( const char * e, const char * schNm, int check_case ) const {
    ...
    if( check_case ) {
        entd = ( EntityDescriptor * )STEPCODE_HASHfind( primordialSwamp, ( char * )e );
    } else {
        entd = ( EntityDescriptor * )STEPCODE_HASHfind( primordialSwamp,
...
const Schema * Registry::FindSchema( const char * n, int check_case ) const {
    if( check_case ) {
        return ( const Schema * ) STEPCODE_HASHfind( active_schemas, ( char * ) n );
    }

    return ( const Schema * ) STEPCODE_HASHfind( active_schemas,
                                           ( char * )PrettyTmpName( n ) );
}

const TypeDescriptor * Registry::FindType( const char * n, int check_case ) const {
    if( check_case ) {
        return ( const TypeDescriptor * ) STEPCODE_HASHfind( active_types, ( char * ) n );
    }
    return ( const TypeDescriptor * ) STEPCODE_HASHfind( active_types,
            ( char * )PrettyTmpName( n ) );
}
...
const TypeDescriptor * Registry::NextType() {
    if( 0 == STEPCODE_HASHlist( &cur_type ) ) {
        return 0;
    }
...
void Registry::AddEntity( const EntityDescriptor & e ) {
    STEPCODE_HASHinsert( primordialSwamp, ( char * ) e.Name(), ( EntityDescriptor * ) &e );
    ++entity_cnt;
    ++all_ents_cnt;
    AddClones( e );
}
...
void Registry::AddSchema( const Schema & d ) {
    STEPCODE_HASHinsert( active_schemas, ( char * ) d.Name(), ( Schema * ) &d );
}

void Registry::AddType( const TypeDescriptor & d ) {
    STEPCODE_HASHinsert( active_types, ( char * ) d.Name(), ( TypeDescriptor * ) &d );
}
...
void Registry::AddClones( const EntityDescriptor & e ) {
    ...
    while( alts ) {
        STEPCODE_HASHinsert( primordialSwamp, ( char * )alts->objName(),
                       ( EntityDescriptor * )&e );
        alts = alts->next;
    }
...
void Registry::RemoveEntity( const char * n ) {
    ...
    tmp.key = ( char * ) n;
    STEPCODE_HASHsearch( primordialSwamp, &tmp, HASH_DELETE ) ? --entity_cnt : 0;
...
void Registry::RemoveSchema( const char * n ) {
    struct Element tmp;
    tmp.key = ( char * ) n;
    STEPCODE_HASHsearch( active_schemas, &tmp, HASH_DELETE );
}

void Registry::RemoveType( const char * n ) {
    struct Element tmp;
    tmp.key = ( char * ) n;
    STEPCODE_HASHsearch( active_types, &tmp, HASH_DELETE );
}
...
void Registry::RemoveClones( const EntityDescriptor & e ) {
    ...
    while( alts ) {
        ...
        STEPCODE_HASHsearch( primordialSwamp, tmp, HASH_DELETE );
        alts = alts->next;
    }
}
...
const EntityDescriptor * Registry::NextEntity() {
    if( 0 == STEPCODE_HASHlist( &cur_entity ) ) {
        return 0;
    }
...
const Schema * Registry::NextSchema() {
    if( 0 == STEPCODE_HASHlist( &cur_schema ) ) {
        return 0;
    }
