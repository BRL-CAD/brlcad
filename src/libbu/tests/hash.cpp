...
/* Slightly tweaked and reformatted text originally generated from
 * libsum.com - 275 words, one of which is an empty string (position
 * 195 after "feugiat.") to test behavior in the NULL conditions.
 */
...
/* Compare the behavior of libbu's hash to that of the C++ map using
 * lorem ipsum strings as keys.
 */
static int
hash_loremipsum() {
    int i = 0;
    ...
    // Set up non-sequential numbers to use as values - easy to
    // compare but shouldn't have any accidental alignments that could
    // obscure bugs.
    for (i = 0; i < 275; i++) lorem_nums[i] = strlen(lorem_ipsum[i]) + i;

    // Initial population of hash set and C++ map.  The hash table
    // will refuse a null key, so make sure we skip putting that in
    // the C++ map.  The key set deliberately contains duplicate keys
    // with different associated values, and both assignments (C++ and
    // hash table) should result in a "last assignment wins" value
    // being stored for subsequent lookups.
    for (i = 0; i < 275; i++) {
    ...
    // Using the C++ map as an oracle, check the values that got
    // stored in the hash table.  All values should match, and all
    // should be present.
    ...
    // Iterate over hash and use find on the C++ map to validate each
    // entry. Also confirms bu hash key copies are correct since any
    // key in the hash table not found in the C++ map is reported as
    // an error.
