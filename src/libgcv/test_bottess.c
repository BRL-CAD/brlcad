...
#define POST(name) for(i=0;i<numsplit;i++){VSET(f.vert[0],_splits[i][0],_splits[i][1],_splits[i][2]);VSET(f.vert[1],_splits[i][3],_splits[i][4],_splits[i][5]);VSET(f.vert[2],_splits[i][6],_splits[i][7],_splits[i][8]);urf[i]=find_tri(&s,&f,&t);if(urf[i]==-1)failure++;}if(nsplt!=2&&urf[0]==-1&&urf[1]==-1){printf("\033[1;31mFAILURE "name"\033[m\n");printf("%lu faces now\n",s.nfaces);for(i=0;i<s.nfaces;i++)printf("%03lu: % 2g,% 2g,% 2g | % 2g,% 2g,% 2g | % 2g,% 2g,% 2g\n",i,V3ARGS(s.faces[i].vert[0]),V3ARGS(s.faces[i].vert[1]),V3ARGS(s.faces[i].vert[2]));count++;}free(s.faces);}
