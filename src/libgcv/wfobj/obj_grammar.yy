...
int obj_parser_parse(yyscan_t scanner)
{
    ...
    bool &error = static_cast<detail::objCombinedState*>
	(scanner)->parser_state.syntaxError;
...
{
    detail::objFileContents::gvertex_t vertex = {{A.real, B.real, C.real, D.real}};
    detail::get_contents(scanner).gvertices_list.push_back(vertex);
}
...
p_v_reference_list(A) ::= INTEGER(B).
{
    ...
    detail::contents_type::polygonal_v_index_type gindex = detail::real_index(B.integer, num_gvertices);

    if (detail::index_check(B.integer, gindex, num_gvertices, detail::vertex_ref_err, scanner)) {
         YYERROR;
    }
...
{
    std::size_t num_gvertices = detail::get_contents(scanner).gvertices_list.size();
    detail::contents_type::polygonal_v_index_type gindex = detail::real_index(B.reference[0], num_gvertices);

    if (detail::index_check(B.reference[0], gindex, num_gvertices, detail::vertex_ref_err, scanner))
        YYERROR;
...
{
    std::size_t num_gvertices = detail::get_contents(scanner).gvertices_list.size();
    detail::contents_type::polygonal_v_index_type gindex = detail::real_index(C.integer, num_gvertices);

    if (detail::index_check(C.integer, gindex, num_gvertices, detail::vertex_ref_err, scanner))
        YYERROR;
...
{
    std::size_t num_gvertices = detail::get_contents(scanner).gvertices_list.size();
    detail::contents_type::polygonal_v_index_type gindex = detail::real_index(C.reference[0], num_gvertices);

    if (detail::index_check(C.reference[0], gindex, num_gvertices, detail::vertex_ref_err, scanner))
        YYERROR;
...
l_tv_reference_list(A) ::= TV_REFERENCE(B).
{
    ...
    if (detail::index_check(B.reference[0], tv_index.v[0], num_gvertices, detail::vertex_ref_err, scanner) ||
        detail::index_check(B.reference[1], tv_index.v[1], num_tvertices, detail::texture_ref_err, scanner))
    {
        YYERROR;
    }
...
l_tv_reference_list(A) ::= l_tv_reference_list(B) TV_REFERENCE(C).
{
    ...
    if (detail::index_check(C.reference[0], tv_index.v[0], num_gvertices, detail::vertex_ref_err, scanner) ||
        detail::index_check(C.reference[1], tv_index.v[1], num_tvertices, detail::texture_ref_err, scanner))
    {
        YYERROR;
    }
...
f_nv_reference_list(A) ::= NV_REFERENCE(B).
{
    ...
    if (detail::index_check(B.reference[0], nv_index.v[0], num_gvertices, detail::vertex_ref_err, scanner) ||
        detail::index_check(B.reference[2], nv_index.v[1], num_nvertices, detail::normal_ref_err, scanner))
    {
        YYERROR;
    }
...
f_nv_reference_list(A) ::= f_nv_reference_list(B) NV_REFERENCE(C).
{
    ...
    if (detail::index_check(C.reference[0], nv_index.v[0], num_gvertices, detail::vertex_ref_err, scanner) ||
        detail::index_check(C.reference[2], nv_index.v[1], num_nvertices, detail::normal_ref_err, scanner))
    {
        YYERROR;
    }
...
f_tnv_reference_list(A) ::= TNV_REFERENCE(B).
{
    ...
    if (detail::index_check(B.reference[0], tnv_index.v[0], num_gvertices, detail::vertex_ref_err, scanner) ||
        detail::index_check(B.reference[1], tnv_index.v[1], num_tvertices, detail::texture_ref_err, scanner) ||
        detail::index_check(B.reference[2], tnv_index.v[2], num_nvertices, detail::normal_ref_err, scanner))
    {
        YYERROR;
    }
...
f_tnv_reference_list(A) ::= f_tnv_reference_list(B) TNV_REFERENCE(C).
{
    ...
    if (detail::index_check(C.reference[0], tnv_index.v[0], num_gvertices, detail::vertex_ref_err, scanner) ||
        detail::index_check(C.reference[1], tnv_index.v[1], num_tvertices, detail::texture_ref_err, scanner) ||
        detail::index_check(C.reference[2], tnv_index.v[2], num_nvertices, detail::normal_ref_err, scanner))
    {
        YYERROR;
    }
...
{
    if (detail::get_contents(scanner).pologonal_tv_indexlist.size()-A.index < 3) {
...
{
    if (detail::get_contents(scanner).pologonal_nv_indexlist.size() - A.index < 3) {
        obj_parser_error(scanner, detail::face_length_err);
        YYERROR;
    }
...
{
    if (detail::get_contents(scanner).pologonal_tnv_indexlist.size()-A.index < 3) {
        obj_parser_error(scanner, detail::face_length_err);
        YYERROR;
    }
