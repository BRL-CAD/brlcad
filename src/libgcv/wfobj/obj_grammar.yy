...
static const char face_length_err[] = "Faces must contain at least 3 "
				      "references";
...
inline static bool index_check(int raw, std::size_t index, size_t vertices,
			       const charT *log, yyscan_t scanner)
{
...
: INTEGER
{
	...
	detail::real_index($1, num_gvertices);

    if (detail::index_check($1, gindex, num_gvertices, detail::vertex_ref_err,
	scanner)) {
	YYERROR;
    }
...
| V_REFERENCE
{
    ...
    if (detail::index_check($1[0], gindex, num_gvertices,
...
| p_v_reference_list INTEGER
{
    ...
    detail::contents_type::polygonal_v_index_type gindex =
	detail::real_index($2, num_gvertices);

    if (detail::index_check($2, gindex, num_gvertices, detail::vertex_ref_err,
	scanner)) {
	YYERROR;
    }
...
| p_v_reference_list V_REFERENCE
{
    ...
    if (detail::index_check($2[0], gindex, num_gvertices,
	detail::vertex_ref_err, scanner)) {
	YYERROR;
    }
...
: TV_REFERENCE
{
    ...
    if (detail::index_check($1[0], tv_index.v[0], num_gvertices,
			    detail::vertex_ref_err, scanner) ||
	detail::index_check($1[1], tv_index.v[1], num_tvertices,
...
| l_tv_reference_list TV_REFERENCE
{
    ...
    if (detail::index_check($2[0], tv_index.v[0], num_gvertices,
			    detail::vertex_ref_err, scanner) ||
	detail::index_check($2[1], tv_index.v[1], num_tvertices,
			    detail::texture_ref_err, scanner))
    {
	YYERROR;
    }
...
: NV_REFERENCE
{
    ...
    if (detail::index_check($1[0], nv_index.v[0], num_gvertices,
			    detail::vertex_ref_err, scanner) ||
	detail::index_check($1[2], nv_index.v[1], num_nvertices,
...
| f_nv_reference_list NV_REFERENCE
{
    ...
    if (detail::index_check($2[0], nv_index.v[0], num_gvertices,
			    detail::vertex_ref_err, scanner) ||
	detail::index_check($2[2], nv_index.v[1], num_nvertices,
			    detail::normal_ref_err, scanner))
    {
	YYERROR;
    }
...
: TNV_REFERENCE
{
    ...
    if (detail::index_check($1[0], tnv_index.v[0], num_gvertices,
			    detail::vertex_ref_err, scanner) ||
	detail::index_check($1[1], tnv_index.v[1], num_tvertices,
			    detail::texture_ref_err, scanner) ||
	detail::index_check($1[2], tnv_index.v[2], num_nvertices,
			    detail::normal_ref_err, scanner))
    {
	YYERROR;
    }
...
| f_tnv_reference_list TNV_REFERENCE
{
    ...
    if (detail::index_check($2[0], tnv_index.v[0], num_gvertices,
			    detail::vertex_ref_err, scanner) ||
	detail::index_check($2[1], tnv_index.v[1], num_tvertices,
			    detail::texture_ref_err, scanner) ||
	detail::index_check($2[2], tnv_index.v[2], num_nvertices,
			    detail::normal_ref_err, scanner))
    {
	YYERROR;
    }
