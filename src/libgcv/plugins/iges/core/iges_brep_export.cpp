...
export_brep(Writer &writer, struct rt_db_internal &internal, const std::string &name)
{
	...
	return {};
    // A native primitive's interior is not encoded by its callback's face
    // senses.  Correct definite inversions in this temporary representation;
    // authored B-Rep inputs above retain their original orientation.
    const auto orientation = repair_brep_orientation(*brep, [&](size_t completed, size_t total) {
	if (writer.options().progress)
	    writer.options().progress("orientation", name.c_str(), completed, total);
    });
    ...
    if (reversed_faces)
	bu_log("g-iges: %s primitive B-Rep oriented outward (%zu faces reversed)\n", name.c_str(), reversed_faces);
