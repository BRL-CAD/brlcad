    ...
    const ImportOptions &options, const BrepImportResult &result)
{
    ...
    const auto reoriented_loops = std::count_if(result.diagnostics.begin(), result.diagnostics.end(),
	[](const ImportDiagnostic &diagnostic) { return diagnostic.code == "parameter_loop_reoriented"; });
	...
	<< result.statistics.repairs << ", \"orientation_faces_reversed\": "
	<< result.statistics.orientation_faces_reversed << ", \"parameter_loops_reoriented\": "
	<< reoriented_loops << "},\n  \"orientation\": ";
    write_orientation_json(output, result.orientation);
    output << ",\n  \"diagnostics\": [";
