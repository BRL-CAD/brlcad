...
bool
same_faces(const ON_Brep &source, const ON_Brep &imported)
{
    std::vector<bool> matched(imported.m_F.Count(), false);
    for (int i = 0; i < source.m_F.Count(); ++i) {
	bool found = false;
	for (int j = 0; j < imported.m_F.Count(); ++j) {
	    if (!matched[j] && same_face(source.m_F[i], imported.m_F[j])) {
		matched[j] = true;
		found = true;
		break;
	    }
...
main(int argc, const char **argv)
{
    ...
    if (!same_faces(*source, *imported)) {
	if (!allow_reversal)
	    bu_exit(1, "BRep round trip changed face geometry or orientation\n");
	source->Flip();
	if (!same_faces(*source, *imported))
	    bu_exit(1, "BRep round trip changed more than global shell orientation\n");
    }
