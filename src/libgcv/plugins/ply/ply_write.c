 ...
 */
 ...
 {
     float rel;                                 /* relative tessolation tolerance */
     float abs;                                 /* absolute tessolation tolerance */
     float norm;                                /* surface normal tessolation tolerance */
     float dist;                                /* distance calculation tolerance */
     char* o_file;                              /* output file name */
     char* type;                                /* storage type */
     int merge_all;                             /* flag to merge all objects into one ply file */
 ...
 nmg_to_ply(struct nmgregion *r, const struct db_full_path *pathp, int UNUSED(region_id), int UNUSED(material_id), struct db_tree_state *tsp, struct conversion_state* pstate)
 {
     ...
     if (pstate->ply_write_options->merge_all) {
 	if (!tsp->ts_mater.ma_color_valid) {
 	    ...
 	    color_info = 1;
 	} else if (color_info == 1 && (pstate->all_colors[0] != color [0] || pstate->all_colors[1] != color [1] || pstate->all_colors[2] != color [2])) {
 	    color_info = 3;
 	}
     }
     NMG_CK_REGION(r);
 	...
 	region_file = (char *) bu_calloc(strlen(region_name) + strlen(pstate->file_name), sizeof(char), "region_file");
 	sprintf(region_file, "%s_%s", region_name+1, pstate->file_name);
     ...
     /* Output triangles */
     if (pstate->ply_write_options->verbose)
 	...
 	if (write_verts(ply_fp, pstate->v_tbl_regs[pstate->cur_region])) {
 	    bu_hash_destroy(pstate->v_tbl_regs[pstate->cur_region]);
 ...
 // do_region_end(struct nmgregion *r, const struct db_full_path *pathp, int UNUSED(region_id), int UNUSED(material_id), float UNUSED(color[3]), void *client_data)
 {
 ...
 ply_write_create_opts(struct bu_opt_desc **options_desc, void **dest_options_data)
 {
     ...
     *options_desc = (struct bu_opt_desc *)bu_malloc(11 * sizeof(struct bu_opt_desc), "options_desc");
     ...
     /* defaults */
     options_data->rel = 0.01;
     options_data->abs = 0.0;
     options_data->norm = 0.0;
     options_data->dist = 0.005;
     options_data->o_file = NULL;
     options_data->type = "de";
     options_data->merge_all = 1;
     options_data->verbose = 0;
     options_data->x = NULL;
     options_data->X = NULL;

     BU_OPT((*options_desc)[0], "r", "rel_tess_tol",  "float", NULL, &options_data->rel,       "specify the relative tessolation tolerance");
     BU_OPT((*options_desc)[1], "a", "abs_tess_tol",  "float", NULL, &options_data->abs,       "specify the absolute tessolation tolerance");
     BU_OPT((*options_desc)[2], "n", "norm_tess_tol", "float", NULL, &options_data->norm,      "specify the surface normal tessolation tolerance");
     BU_OPT((*options_desc)[3], "D", "dist_calc_tol", "float", NULL, &options_data->dist,      "specify the distance calculation tolerance");
     BU_OPT((*options_desc)[4], "o", "output_file",   "char*", NULL, &options_data->o_file,    "specify the output file name");
     BU_OPT((*options_desc)[5], "t", "type",          "char*", NULL, &options_data->type,      "specify the output file type (asc: ascii), (le: little endian), (be: big endian)");
     BU_OPT((*options_desc)[6], "s", "separate",      "",      NULL, &options_data->merge_all, "specify a flag to generate a separate file per object");
     BU_OPT((*options_desc)[7], "v", "verbose",       "",      NULL, &options_data->verbose,   "specify a flag for verbose output");
     BU_OPT((*options_desc)[8], "x", "rt_dubug",      "char*", NULL, &options_data->o_file,    "specify debug script for rt_debug");
     BU_OPT((*options_desc)[9], "X", "nmg_debug",     "char*", NULL, &options_data->o_file,    "specify debug script for nmg_debug");
 ...
 ply_write_gcv(struct gcv_context* context, const struct gcv_opts* gcv_options, const void* options_data, const char* dest_path)
 {
     ...
     if (state.ply_write_options->o_file) {        // check if user explicitly set legacy -o
         state.output_file = state.ply_write_options->o_file;
     }
     state.dbip = context->dbip;
 //     BU_VLS_INIT(&state.file_name);

 //     gcvwriter.write_region = do_region_end;
     ...
     /* TODO: FIXME
        update tolerances if user explicitly set */
     // if (state.ply_write_options->abs)
     //     tree_state.ts_ttol->abs = state.ply_write_options->abs;
     // if (state.ply_write_options->rel)
     //     tree_state.ts_ttol->rel = state.ply_write_options->rel;
     // if (state.ply_write_options->norm)
     //     tree_state.ts_ttol->norm = state.ply_write_options->norm;
     // if (state.ply_write_options->dist) {
     //     tree_state.ts_tol->dist = state.ply_write_options->dist;
     //     tree_state.ts_tol->dist_sq = state.ply_write_options->dist * state.ply_write_options->dist;
     // }
     ...
     if (state.ply_write_options->merge_all) {
         /* open outputfile using specified type */
     ...
     if (state.ply_write_options->verbose) {
     ...
     (void) db_walk_tree(state.dbip,
 			...
 			region_stub,
 			leaf_stub,
 			(void*)&state);	/* in librt/nmg_bool.c */
     if (state.ply_write_options->verbose)
 ...
 // 			(void *)&gcvwriter);
 	...
 	for (ri = 0; ri < state.cur_region; ri++)
 	    if (write_verts(ply_fp, state.v_tbl_regs[ri])) {
 		bu_log("ERROR: No coordinates found for vertex!\n");
 		ply_close(ply_fp);
 		goto free_all;
 	    }
     ...
     bu_log("  abs  [-a]    %g\n", state.gcv_options->tessellation_tolerance.abs);
     bu_log("  rel  [-r]    %g\n", state.gcv_options->tessellation_tolerance.rel);
     bu_log("  norm [-n]    %g\n", state.gcv_options->tessellation_tolerance.norm);
     bu_log("  dist [-D]    %g\n", state.gcv_options->calculational_tolerance.dist);
