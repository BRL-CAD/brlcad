		      ...
		      const char *argv[])
{
	...
	if (argc == 3) {
	    ...
	    if (to_extract_contours_av(interp, gedp, gdvp, &gdpsp->gdps_polygons.polygon[i],
				       contour_ac, contour_av, gdvp->gvs.mode, 0) != GED_OK) {
		Tcl_Free((char *)contour_av);
		return GED_ERROR;
	    }
	...
	if (argc == 3) {
	    ...
	    if (to_extract_polygons_av(interp, gedp, gdvp, gdpsp, polygon_ac, polygon_av, gdvp->gvs.mode, vflag) != GED_OK) {
	...
	if (to_extract_contours_av(interp, gedp, gdvp, &gp, contour_ac, contour_av, gdvp->gvs.mode, 0) != GED_OK) {
	    Tcl_Free((char *)contour_av);
	    return GED_ERROR;
	}
		       ...
		       const char *usage)
{
    ...
    gdvp->gvs.mode = BVIEW_POLY_CIRCLE_MODE;
			...
			int doBind)
{
    ...
    gdvp->gv_prevMouseX = x;
    gdvp->gv_prevMouseY = y;
    gdvp->gvs.mode = BVIEW_POLY_CONTOUR_MODE;
		      ...
		      const char *usage)
{
    ...
    gdvp->gv_prevMouseX = x;
    gdvp->gv_prevMouseY = y;
    gdvp->gvs.mode = TCLCAD_POLY_ELLIPSE_MODE;
		       ...
		       const char *usage)
{
    ...
    if (sflag)
	gdvp->gvs.mode = TCLCAD_POLY_SQUARE_MODE;
    else
	gdvp->gvs.mode = TCLCAD_POLY_RECTANGLE_MODE;
