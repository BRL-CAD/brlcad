...
static int
fb_wrapper_func(ClientData data, Tcl_Interp *interp, int argc, const char *argv[])
{
    struct bu_cmdtab *ctp = (struct bu_cmdtab *)data;
...
fb_register_cmds(Tcl_Interp *interp, struct bu_cmdtab *cmds)
{
    ...
    for (ctp = cmds; ctp->ct_name != (char *)NULL; ctp++) {
	(void)Tcl_CreateCommand(interp, ctp->ct_name, fb_wrapper_func, (ClientData)ctp, (Tcl_CmdDeleteProc *)NULL);
    }
}
