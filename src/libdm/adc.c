...
dm_draw_ticks(struct dm *dmp, struct bv_adc_state *adcp, fastf_t angle)
{
    ...
    /* Tick distance */
    c_tdist = ((fastf_t)(adcp->dv_dist) + BV_MAX) * M_SQRT1_2;
...
dm_draw_adc(struct dm *dmp, struct bv_adc_state *adcp, mat_t view2model, mat_t model2view)
{
	...
	dx = view_pt[X] * BV_MAX - adcp->dv_x;
	dy = view_pt[Y] * BV_MAX - adcp->dv_y;

	if (!ZERO(dx) || !ZERO(dy)) {
	    adcp->a1 = RAD2DEG*atan2(dy, dx);
	    adcp->dv_a1 = (1.0 - (adcp->a1 / 45.0)) * BV_MAX;
	}
    }
	...
	if (!ZERO(dx) || !ZERO(dy)) {
	    ...
	    adcp->dv_a2 = (1.0 - (adcp->a2 / 45.0)) * BV_MAX;
	}
    }
	...
	adcp->dv_dist = (dist / M_SQRT1_2) - BV_MAX;
    } else
    ...
    /* Horizontal */
    dm_draw_line_2d(dmp,
		    GED_TO_PM1(BV_MIN), GED_TO_PM1(adcp->dv_y) * dmp->i->dm_aspect,
		    GED_TO_PM1(BV_MAX), GED_TO_PM1(adcp->dv_y) * dmp->i->dm_aspect);

    /* Vertical */
    dm_draw_line_2d(dmp,
		    GED_TO_PM1(adcp->dv_x), GED_TO_PM1(BV_MAX),
		    GED_TO_PM1(adcp->dv_x), GED_TO_PM1(BV_MIN));
