...
static void stbtt__rasterize(stbtt__bitmap *result, stbtt__point *pts, int *wcount, int windings, float scale_x, float scale_y, float shift_x, float shift_y, int off_x, int off_y, int invert, void *userdata)
{
   ...
   for (i=0; i < windings; ++i) {
      ...
      for (k=0; k < wcount[i]; j=k++) {
         ...
         if (NEAR_ZERO(p[j].y - p[k].y, SMALL_FASTF))
            continue;
...
unsigned char *stbtt_GetGlyphBitmapSubpixel(const stbtt_fontinfo *info, float scale_x, float scale_y, float shift_x, float shift_y, int glyph, int *width, int *height, int *xoff, int *yoff)
{
   ...
   if (NEAR_ZERO(scale_x, SMALL_FASTF)) scale_x = scale_y;
   if (NEAR_ZERO(scale_y, SMALL_FASTF)) {
      if (NEAR_ZERO(scale_x, SMALL_FASTF)) return NULL;
      scale_y = scale_x;
   }
