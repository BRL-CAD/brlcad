...
static int togl_actively_drawing;
HIDDEN XVisualInfo *togl_choose_visual(struct dm *dmp, Tk_Window tkwin);
...
HIDDEN fastf_t togl_default_viewscale = 1000.0;
HIDDEN double	togl_xlim_view = 1.0;	/* args for glOrtho*/
HIDDEN double	togl_ylim_view = 1.0;

/* lighting parameters */
HIDDEN float togl_amb_three[] = {0.3, 0.3, 0.3, 1.0};


HIDDEN float togl_light0_direction[] = {0.0, 0.0, 1.0, 0.0};
HIDDEN float togl_light0_position[] = {100.0, 200.0, 100.0, 0.0};
HIDDEN float togl_light0_diffuse[] = {1.0, 1.0, 1.0, 1.0}; /* white */
HIDDEN float togl_wireColor[4];
HIDDEN float togl_ambientColor[4];
HIDDEN float togl_specularColor[4];
HIDDEN float togl_diffuseColor[4];
HIDDEN float togl_backColor[] = {1.0, 1.0, 0.0, 1.0}; /* yellow */
...
togl_open(Tcl_Interp *interp, int argc, char **argv)
{
    ...
    *dmp = dm_togl; /* struct copy */
    ...
    dmp->dm_vp = &togl_default_viewscale;
    ...
    /* this is important so that togl_configureWin knows to set the font */
    ...
    if ((pubvars->vip=togl_choose_visual(dmp,pubvars->xtkwin)) == NULL) {
	bu_log("togl_open: Can't get an appropriate visual.\n");
	(void)togl_close(dmp);
	return DM_NULL;
    }
	...
	bu_log("togl_open: couldn't create glXContext.\n");
	(void)togl_close(dmp);
	return DM_NULL;
    }
	...
	if (list->use == IsXExtensionDevice) {
		...
		if ((dev = XOpenDevice(pubvars->dpy,
		    ...
		    bu_log("togl_open: Couldn't open the dials+buttons\n");
		    goto Done;
		}
 ...
 Done:
	...
	bu_log("togl_open: Couldn't make context current\n");
	(void)togl_close(dmp);
	return DM_NULL;
    }
    ...
    glLoadIdentity();
    glOrtho( -togl_xlim_view, togl_xlim_view, -togl_ylim_view, togl_ylim_view, 0.0, 2.0 );
...
togl_share_dlist(struct dm *dmp1, struct dm *dmp2)
{
	...
	old_glxContext = ((struct togl_vars *)dmp1->dm_vars.priv_vars)->glxc;

	if ((((struct togl_vars *)dmp1->dm_vars.priv_vars)->glxc =
	    ...
	    bu_log("togl_share_dlist: couldn't create glXContext.\nUsing old context\n.");
	    ((struct togl_vars *)dmp1->dm_vars.priv_vars)->glxc = old_glxContext;
	...
	if (!glXMakeCurrent(((struct dm_xvars *)dmp1->dm_vars.pub_vars)->dpy,
			    ...
			    ((struct togl_vars *)dmp1->dm_vars.priv_vars)->glxc)) {
	    bu_log("togl_share_dlist: Couldn't make context current\nUsing old context\n.");
	    ((struct togl_vars *)dmp1->dm_vars.priv_vars)->glxc = old_glxContext;
	...
	/* display list (fontOffset + char) will display a given ASCII char */
	if ((((struct togl_vars *)dmp1->dm_vars.priv_vars)->fontOffset = glGenLists(128))==0) {
	    bu_log("dm-ogl: Can't make display lists for font.\nUsing old context\n.");
	    ((struct togl_vars *)dmp1->dm_vars.priv_vars)->glxc = old_glxContext;
	...
	/* This is the applications display list offset */
	dmp1->dm_displaylist = ((struct togl_vars *)dmp1->dm_vars.priv_vars)->fontOffset + 128;

	togl_setBGColor(dmp1, 0, 0, 0);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

	if (((struct togl_vars *)dmp1->dm_vars.priv_vars)->mvars.doublebuffer)
	    glDrawBuffer(GL_BACK);
	else
	    glDrawBuffer(GL_FRONT);
	...
	/* do viewport, ortho commands and initialize font */
	(void)togl_configureWin_guts(dmp1, 1);
	...
	glLoadIdentity();
	glOrtho( -togl_xlim_view, togl_xlim_view, -togl_ylim_view, togl_ylim_view, 0.0, 2.0 );
	glGetDoublev(GL_PROJECTION_MATRIX, ((struct togl_vars *)dmp1->dm_vars.priv_vars)->faceplate_mat);
	glPushMatrix();
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glTranslatef(0.0, 0.0, -1.0);
	glPushMatrix();
	glLoadIdentity();
	((struct togl_vars *)dmp1->dm_vars.priv_vars)->face_flag = 1; /* faceplate matrix is on top of stack */
	...
	old_glxContext = ((struct togl_vars *)dmp2->dm_vars.priv_vars)->glxc;

	if ((((struct togl_vars *)dmp2->dm_vars.priv_vars)->glxc =
	     glXCreateContext(((struct dm_xvars *)dmp2->dm_vars.pub_vars)->dpy,
			      ((struct dm_xvars *)dmp2->dm_vars.pub_vars)->vip,
			      ((struct togl_vars *)dmp1->dm_vars.priv_vars)->glxc,
			      GL_TRUE))==NULL) {
	    bu_log("togl_share_dlist: couldn't create glXContext.\nUsing old context\n.");
	    ((struct togl_vars *)dmp2->dm_vars.priv_vars)->glxc = old_glxContext;
	...
	if (!glXMakeCurrent(((struct dm_xvars *)dmp2->dm_vars.pub_vars)->dpy,
			    ...
			    ((struct togl_vars *)dmp2->dm_vars.priv_vars)->glxc)) {
	    bu_log("togl_share_dlist: Couldn't make context current\nUsing old context\n.");
	    ((struct togl_vars *)dmp2->dm_vars.priv_vars)->glxc = old_glxContext;
	...
	((struct togl_vars *)dmp2->dm_vars.priv_vars)->fontOffset = ((struct togl_vars *)dmp1->dm_vars.priv_vars)->fontOffset;
	dmp2->dm_displaylist = dmp1->dm_displaylist;

	togl_setBGColor(dmp2, 0, 0, 0);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

	if (((struct togl_vars *)dmp2->dm_vars.priv_vars)->mvars.doublebuffer)
	    glDrawBuffer(GL_BACK);
	else
	    glDrawBuffer(GL_FRONT);

	/* do viewport, ortho commands and initialize font */
	(void)togl_configureWin_guts(dmp2, 1);
	...
	glLoadIdentity();
	glOrtho( -togl_xlim_view, togl_xlim_view, -togl_ylim_view, togl_ylim_view, 0.0, 2.0 );
	glGetDoublev(GL_PROJECTION_MATRIX, ((struct togl_vars *)dmp2->dm_vars.priv_vars)->faceplate_mat);
	glPushMatrix();
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glTranslatef(0.0, 0.0, -1.0);
	glPushMatrix();
	glLoadIdentity();
	((struct togl_vars *)dmp2->dm_vars.priv_vars)->face_flag = 1; /* faceplate matrix is on top of stack */
...
togl_close(struct dm *dmp)
{
	...
	if (((struct togl_vars *)dmp->dm_vars.priv_vars)->glxc) {
	    ...
	    glXDestroyContext(((struct dm_xvars *)dmp->dm_vars.pub_vars)->dpy,
			      ((struct togl_vars *)dmp->dm_vars.priv_vars)->glxc);
	}
    ...
    bu_free(dmp->dm_vars.priv_vars, "togl_close: togl_vars");
...
togl_drawBegin(struct dm *dmp)
{
    ...
    if (!glXMakeCurrent(((struct dm_xvars *)dmp->dm_vars.pub_vars)->dpy,
			...
			((struct togl_vars *)dmp->dm_vars.priv_vars)->glxc)) {
	bu_log("togl_drawBegin: Couldn't make context current\n");
	return TCL_ERROR;
    }
...
togl_drawEnd(struct dm *dmp)
{
	...
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
	glLightfv(GL_LIGHT0, GL_POSITION, togl_light0_direction);
    }
 ...
 */
HIDDEN int
togl_drawVList(struct dm *dmp, register struct bn_vlist *vp)
{
	...
	for (i = 0; i < nused; i++, cmd++, pt++) {
		...
		case BN_VLIST_LINE_MOVE:
			...
			mflag = 0;
			glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, togl_wireColor);
		...
		case BN_VLIST_POLY_START:
			...
			glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, togl_ambientColor);
			glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, togl_specularColor);

			if (1 < dmp->dm_light) {
			    glMaterialfv(GL_FRONT, GL_DIFFUSE, togl_diffuseColor);
			    glMaterialfv(GL_BACK, GL_DIFFUSE, togl_backColor);
			} else
			    glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, togl_diffuseColor);
...
*/
	...
	if (nvectors >= vectorThreshold) {
	    if (dmp->dm_debugLevel)
		bu_log("togl_drawVList(): handle Tcl events\n");
	    ...
	    if (dmp->dm_debugLevel)
		bu_log("togl_drawVList(): handled Tcl events successfully\n");
	}
#endif
    }
 ...
 */
HIDDEN int
togl_drawString2D(struct dm *dmp, register char *str, fastf_t x, fastf_t y, int size, int use_aspect)
{
    if (dmp->dm_debugLevel)
...
togl_setFGColor(struct dm *dmp, unsigned char r, unsigned char g, unsigned char b, int strict, fastf_t transparency)
{
    ...
    togl_wireColor[0] = r / 255.0;
    togl_wireColor[1] = g / 255.0;
    togl_wireColor[2] = b / 255.0;
    togl_wireColor[3] = transparency;
	...
	if (dmp->dm_light) {
	    ...
	    togl_ambientColor[0] = togl_wireColor[0] * 0.2;
	    togl_ambientColor[1] = togl_wireColor[1] * 0.2;
	    togl_ambientColor[2] = togl_wireColor[2] * 0.2;
	    togl_ambientColor[3] = togl_wireColor[3];

	    togl_specularColor[0] = togl_ambientColor[0];
	    togl_specularColor[1] = togl_ambientColor[1];
	    togl_specularColor[2] = togl_ambientColor[2];
	    togl_specularColor[3] = togl_ambientColor[3];

	    togl_diffuseColor[0] = togl_wireColor[0] * 0.6;
	    togl_diffuseColor[1] = togl_wireColor[1] * 0.6;
	    togl_diffuseColor[2] = togl_wireColor[2] * 0.6;
	    togl_diffuseColor[3] = togl_wireColor[3];
...
togl_setBGColor(struct dm *dmp, unsigned char r, unsigned char g, unsigned char b)
{
	...
	if (!glXMakeCurrent(((struct dm_xvars *)dmp->dm_vars.pub_vars)->dpy,
			    ((struct dm_xvars *)dmp->dm_vars.pub_vars)->win,
			    ((struct togl_vars *)dmp->dm_vars.priv_vars)->glxc)) {
	    bu_log("togl_setBGColor: Couldn't make context current\n");
	    return TCL_ERROR;
	}
 ...
 */
HIDDEN XVisualInfo *
togl_choose_visual(struct dm *dmp, Tk_Window tkwin)
{
	...
	for (i=0, j=0, vip=vibase; i<num; i++, vip++) {
		...
		if (fail || !stereo) {
		    bu_log("togl_choose_visual: failed visual - GLX_STEREO\n");
		    continue;
		}
	    }
	...
	if (j >= 1) {
		...
		if (Tk_SetWindowVisual(tkwin,
		    ...
		    glXGetConfig(((struct dm_xvars *)dmp->dm_vars.pub_vars)->dpy,
				 ...
				 &((struct togl_vars *)dmp->dm_vars.priv_vars)->mvars.depth);
		    if (((struct togl_vars *)dmp->dm_vars.priv_vars)->mvars.depth > 0)
			((struct togl_vars *)dmp->dm_vars.priv_vars)->mvars.zbuf = 1;
...
togl_configureWin_guts(struct dm *dmp, int force)
{
    ...
    if (!glXMakeCurrent(((struct dm_xvars *)dmp->dm_vars.pub_vars)->dpy,
			((struct dm_xvars *)dmp->dm_vars.pub_vars)->win,
			((struct togl_vars *)dmp->dm_vars.priv_vars)->glxc)) {
	bu_log("togl_configureWin_guts: Couldn't make context current\n");
	return TCL_ERROR;
    }
	...
	if ((((struct dm_xvars *)dmp->dm_vars.pub_vars)->fontstruct =
	    ...
	    if ((((struct dm_xvars *)dmp->dm_vars.pub_vars)->fontstruct =
		 XLoadQueryFont(((struct dm_xvars *)dmp->dm_vars.pub_vars)->dpy,
				FONTBACK)) == NULL) {
		bu_log("togl_configureWin_guts: Can't open font '%s' or '%s'\n", FONT9, FONTBACK);
		return TCL_ERROR;
	    }
	}
	glXUseXFont( ((struct dm_xvars *)dmp->dm_vars.pub_vars)->fontstruct->fid,
		     0, 127, ((struct togl_vars *)dmp->dm_vars.priv_vars)->fontOffset);
    }
...
togl_setLight(struct dm *dmp, int lighting_on)
{
    ...
    if (!glXMakeCurrent(((struct dm_xvars *)dmp->dm_vars.pub_vars)->dpy,
			((struct dm_xvars *)dmp->dm_vars.pub_vars)->win,
			((struct togl_vars *)dmp->dm_vars.priv_vars)->glxc)) {
	bu_log("togl_setLight: Couldn't make context current\n");
	return TCL_ERROR;
    }
	...
	glLightModelfv(GL_LIGHT_MODEL_AMBIENT, togl_amb_three);
	glLightModeli(GL_LIGHT_MODEL_LOCAL_VIEWER, GL_FALSE);

#if 0
	glLightModeli(GL_LIGHT_MODEL_TWO_SIDE, GL_TRUE);
#endif
	glLightfv(GL_LIGHT0, GL_DIFFUSE, togl_light0_diffuse);
	glLightfv(GL_LIGHT0, GL_SPECULAR, togl_light0_diffuse);
		    ...
		    int transparency_on)
{
    ...
    if (!glXMakeCurrent(((struct dm_xvars *)dmp->dm_vars.pub_vars)->dpy,
			((struct dm_xvars *)dmp->dm_vars.pub_vars)->win,
			((struct togl_vars *)dmp->dm_vars.priv_vars)->glxc)) {
	bu_log("togl_setTransparency: Couldn't make context current\n");
	return TCL_ERROR;
    }
...
togl_setDepthMask(struct dm *dmp,
    ...
    if (!glXMakeCurrent(((struct dm_xvars *)dmp->dm_vars.pub_vars)->dpy,
			((struct dm_xvars *)dmp->dm_vars.pub_vars)->win,
			((struct togl_vars *)dmp->dm_vars.priv_vars)->glxc)) {
	bu_log("togl_setDepthMask: Couldn't make context current\n");
	return TCL_ERROR;
    }
...
togl_setZBuffer(struct dm *dmp, int zbuffer_on)
{
    ...
    if (!glXMakeCurrent(((struct dm_xvars *)dmp->dm_vars.pub_vars)->dpy,
			((struct dm_xvars *)dmp->dm_vars.pub_vars)->win,
			((struct togl_vars *)dmp->dm_vars.priv_vars)->glxc)) {
	bu_log("togl_setZBuffer: Couldn't make context current\n");
	return TCL_ERROR;
    }
...
{
    if (dmp->dm_debugLevel)
	bu_log("togl_beginDList()\n");

    if (!glXMakeCurrent(((struct dm_xvars *)dmp->dm_vars.pub_vars)->dpy,
			((struct dm_xvars *)dmp->dm_vars.pub_vars)->win,
			((struct togl_vars *)dmp->dm_vars.priv_vars)->glxc)) {
	bu_log("togl_beginDList: Couldn't make context current\n");
	return TCL_ERROR;
    }
...
{
    if (dmp->dm_debugLevel)
	bu_log("togl_freeDLists()\n");

    if (!glXMakeCurrent(((struct dm_xvars *)dmp->dm_vars.pub_vars)->dpy,
			((struct dm_xvars *)dmp->dm_vars.pub_vars)->win,
			((struct togl_vars *)dmp->dm_vars.priv_vars)->glxc)) {
	bu_log("togl_freeDLists: Couldn't make context current\n");
	return TCL_ERROR;
    }
