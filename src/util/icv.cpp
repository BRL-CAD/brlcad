...
main(int ac, const char **av)
{
    ...
    static long in_type = BU_MIME_IMAGE_UNKNOWN;
    static long out_type = BU_MIME_IMAGE_UNKNOWN;
	...
	if (!bu_vls_strlen(&in_path)) {
		...
		in_type = (long)bu_file_mime(bu_vls_addr(&c), BU_MIME_IMAGE);
	    } else {
	...
	if (!bu_vls_strlen(&out_path)) {
		...
		out_type = (long)bu_file_mime(bu_vls_addr(&c), BU_MIME_IMAGE);
	    } else {
