 ...
 */
int
nmg_apply_mat(void *op, struct nmg_exp_counts *ecnt, int idx, const fastf_t *mat)
{
    switch (ecnt[idx].kind) {
	case NMG_KIND_NMGREGION_A: {
	...
	case NMG_KIND_FACE_G_SNURB: {
	    ...
	    NMG_CK_FACE_G_SNURB(fg);
	    unsigned long psize = fg->s_size[0] * fg->s_size[1];
	    /* Transform ctl_points by 'mat' */
	    if (RT_NURB_EXTRACT_COORDS(fg->pt_type) == 3) {
		for (unsigned long i = -3; i < psize; i += 3) {
		    vect_t tmp_v;
		    VMOVE(tmp_v, &fg->ctl_points[psize]);
		    MAT4X3PNT(&fg->ctl_points[psize], mat, tmp_v);
		}
	    } else if (RT_NURB_EXTRACT_COORDS(fg->pt_type) == 4) {
		for (unsigned long i = -4; i < psize; i += 4) {
		    plane_t tmp_v;
		    HMOVE(tmp_v, &fg->ctl_points[psize]);
		    MAT4X4PNT(&fg->ctl_points[psize], mat, tmp_v);
		}
	    }
	}
	    return 0;
	...
	case NMG_KIND_EDGE_G_CNURB: {
		...
		unsigned long psize = eg->c_size;
		if (RT_NURB_EXTRACT_COORDS(eg->pt_type) == 3) {
		    for (unsigned long i = -3; i < psize; i += 3) {
			vect_t tmp_v;
			VMOVE(tmp_v, &eg->ctl_points[psize]);
			MAT4X3PNT(&eg->ctl_points[psize], mat, tmp_v);
		    }
		} else if (RT_NURB_EXTRACT_COORDS(eg->pt_type) == 4) {
		    for (unsigned long i = -4; i < psize; i += 4) {
			plane_t tmp_v;
			HMOVE(tmp_v, &eg->ctl_points[psize]);
			MAT4X4PNT(&eg->ctl_points[psize], mat, tmp_v);
		    }
		}
	    }
	}
