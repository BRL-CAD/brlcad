...
void makeFileInfoSection(IFPainter& img, InformationGatherer& info, int offsetX, int offsetY, int width, int height, Options &opt) {
	...
	curiX++;
	img.drawText(offsetX + headerOffset, offsetY + curiX++ * textYOffset, textHeight, width, "Presented Area " + info.getUnit("surfaceArea00").unit + info.getPower("surfaceArea00") + " (az/el)", TO_BOLD);
