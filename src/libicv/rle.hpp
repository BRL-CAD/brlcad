...
inline const char* error_string(Error e)
{
	...
	case Error::BAD_MAGIC:
	    return "Bad magic";
	case Error::HEADER_TRUNCATED:
	    return "Header truncated";
	case Error::UNSUPPORTED_ENDIAN:
	    return "Unsupported endian";
	case Error::DIM_TOO_LARGE:
	    return "Dimensions exceed max";
	case Error::PIXELS_TOO_LARGE:
	    return "Pixel count exceeds max";
	case Error::ALLOC_TOO_LARGE:
	    return "Allocation exceeds cap";
	case Error::COLORMAP_TOO_LARGE:
	    return "Colormap exceeds cap";
	case Error::COMMENT_TOO_LARGE:
	    return "Comment block too large";
	case Error::INVALID_NCOLORS:
	    return "Invalid ncolors";
	case Error::INVALID_PIXELBITS:
	    return "Invalid pixelbits";
	case Error::INVALID_BG_BLOCK:
	    return "Invalid background block";
	case Error::OPCODE_OVERFLOW:
	    return "Opcode operand overflow";
	case Error::OPCODE_UNKNOWN:
	    return "Unknown opcode";
	case Error::TRUNCATED_OPCODE:
	    return "Truncated opcode data";
	case Error::OP_COUNT_EXCEEDED:
	    return "Opcode count per row exceeded";
...
struct MemWriteStream {
    ...
    explicit MemWriteStream(std::vector<uint8_t>& buffer) : buf(buffer) {}
    bool put(uint8_t c)
    {
	buf.push_back(c);
	return true;
...
struct FileReadStream {
    FILE* f;
    explicit FileReadStream(FILE* file) : f(file) {}
    int get()
    {
	return std::fgetc(f);
    }
    ...
    bool seek(long p)
    {
	return std::fseek(f, p, SEEK_SET) == 0;
    }
};
...
struct MemReadStream {
    ...
    size_t size;
    size_t pos;
    MemReadStream(const uint8_t* buffer, size_t s) : buf(buffer), size(s), pos(0) {}
    int get()
    {
	return (pos < size) ? buf[pos++] : EOF;
    }
    size_t read(void* p, size_t n)
    {
    ...
    bool seek(long p)
    {
	if (p < 0 || static_cast<size_t>(p) > size) return false;
...
inline bool read_u16(Stream& f, Endian e, uint16_t& v)
{
    uint8_t b0, b1;
    int c0 = f.get();
    if (c0 == EOF) return false;
    int c1 = f.get();
...
template<typename Stream>
inline bool write_u16_le(Stream& f, uint16_t v)
{
    return f.put(uint8_t(v & 0xFF)) && f.put(uint8_t((v >> 8) & 0xFF));
}

template<typename Stream>
inline bool read_u8(Stream& f, uint8_t& v)
{
    int c = f.get();
...
template<typename Stream>
inline bool write_u8(Stream& f, uint8_t v)
{
    return f.put(v);
}
...
template<typename Stream>
inline bool discard_bytes(Stream& f, uint64_t n)
{
...
struct Header {
    ...
    {
	return (flags & FLAG_ALPHA) != 0;
    }
    bool has_comments()  const
    {
	return (flags & FLAG_COMMENT) != 0;
    }
    bool no_background() const
    {
	return (flags & FLAG_NO_BACKGROUND) != 0;
    }
    bool clear_first()   const
    {
	return (flags & FLAG_CLEAR_FIRST) != 0;
    }
    ...
    {
	return uint8_t(ncolors + (has_alpha() ? 1 : 0));
    }

    bool validate(Error& err) const
    {
	if (!xlen || !ylen || xlen > MAX_DIM || ylen > MAX_DIM) {
	    err = Error::DIM_TOO_LARGE;
	    return false;
	}
	if (pixelbits != 8) {
	    err = Error::INVALID_PIXELBITS;
	    return false;
	}
	if (ncolors == 0 || ncolors > 254) {
	    err = Error::INVALID_NCOLORS;
	    return false;
	}
	if (!no_background() && background.size() != ncolors) {
	    err = Error::INVALID_BG_BLOCK;
	    return false;
	}
	if (ncmap > 0) {
	    if (ncmap > 3 || cmaplen > 8) {
		err = Error::COLORMAP_TOO_LARGE;
		return false;
	    }
	    uint64_t entries = uint64_t(ncmap) * (uint64_t(1) << cmaplen);
	    if (entries > MAX_COLORMAP_ENTRIES) {
		err = Error::COLORMAP_TOO_LARGE;
		return false;
	    }
	    if (colormap.size() != entries) {
		err = Error::COLORMAP_TOO_LARGE;
		return false;
	    }
	}
	uint64_t pixTotal;
	if (!safe_mul_u64(xlen, ylen, MAX_PIXELS, pixTotal)) {
	    err = Error::PIXELS_TOO_LARGE;
	    return false;
	}
	err = Error::OK;
	return true;
...
template<typename Stream>
inline std::enable_if_t<!std::is_pointer_v<Stream>, bool>
write_header(Stream& f, const Header& h)
{
    Error e;
...
{
    FileWriteStream ws(f);
    return write_header(ws, h);
}

template<typename Stream>
inline std::enable_if_t<!std::is_pointer_v<Stream>, bool>
read_header_single(Stream& f, Header& h, Endian e, Error& err)
{
    long start = f.tell();
    if (start == -1L) {
    ...
    uint16_t magic;
    if (!read_u16(f, e, magic)) {
	err = Error::HEADER_TRUNCATED;
	f.seek(start);
	return false;
    }
    if (magic != RLE_MAGIC) {
    ...
    auto rd8 = [&](uint8_t& v)->bool { int c = f.get(); if (c == EOF) return false; v = uint8_t(c); return true; };
...
if (!read_u16(f, e, h.xpos) || !read_u16(f, e, h.ypos) ||
	...
	uint16_t clen;
	if (!read_u16(f, e, clen)) {
	    err = Error::HEADER_TRUNCATED;
	    f.seek(start);
	    return false;
	}
	if (clen > MAX_COMMENT_LEN) {
	    if (!discard_bytes(f, clen)) {
		err = Error::HEADER_TRUNCATED;
		f.seek(start);
		return false;
	    }
...
{
    FileReadStream rs(f);
    return read_header_single(rs, h, e, err);
}

template<typename Stream>
inline std::enable_if_t<!std::is_pointer_v<Stream>, bool>
read_header_auto(Stream& f, Header& h, Endian& chosen, Error& err)
{
#ifdef STRICT_RLE_ENDIAN
    if (!read_header_single(f, h, Endian::Little, err)) return false;
    chosen = Endian::Little;
    return true;
#else
    long pos = f.tell();
    if (pos == -1L) {
    ...
    if (read_header_single(f, h, Endian::Little, err)) {
	chosen = Endian::Little;
	return true;
    }
    if (err == Error::BAD_MAGIC) {
	f.seek(pos);
	if (read_header_single(f, h, Endian::Big, err)) {
	    chosen = Endian::Big;
	    return true;
	}
    }
...
{
    FileReadStream rs(f);
    return read_header_auto(rs, h, chosen, err);
}
...
struct Image {
    ...
    {
	Error hv;
	if (!header.validate(hv)) {
	    err = hv;
	    return false;
	}
	uint64_t total;
	if (!safe_mul_u64(header.width(), header.height(), MAX_PIXELS, total)) {
	    err = Error::PIXELS_TOO_LARGE;
	    return false;
	}
	uint64_t bytes;
	if (!safe_mul_u64(total, header.channels(), MAX_ALLOC_BYTES, bytes)) {
	    err = Error::ALLOC_TOO_LARGE;
	    return false;
	}
	try {
...
public:
    ...
    template<typename Stream>
    static std::enable_if_t<!std::is_pointer_v<Stream>, bool>
    write(Stream& f, const Image& img, BackgroundMode bg_mode, Error& err)
    {
	Header h = img.header;
	...
	while (rle_y < H) {
		...
		if (skipCount <= 255) {
		    if (!write_u8(f, OPC_SKIP_LINES) || !write_u8(f, uint8_t(skipCount))) {
			err = Error::INTERNAL_ERROR;
			return false;
		    }
		} else {
		    if (!write_u8(f, OPC_SKIP_LINES | OPC_LONG_FLAG) || !write_u8(f, 0) || !write_u16_le(f, uint16_t(skipCount))) {
			err = Error::INTERNAL_ERROR;
			return false;
		    }
		}
		...
		if (!write_u8(f, OPC_SET_COLOR) || !write_u8(f, uint8_t(operand))) {
		    err = Error::INTERNAL_ERROR;
		    return false;
		}
		...
		while (x < W) {
			...
			if (span >= 2) {
			    if (span <= 255) {
				if (!write_u8(f, OPC_SKIP_PIXELS) || !write_u8(f, uint8_t(span))) {
				    err = Error::INTERNAL_ERROR;
				    return false;
				}
			    } else {
				if (!write_u8(f, OPC_SKIP_PIXELS | OPC_LONG_FLAG) || !write_u8(f, 0) || !write_u16_le(f, uint16_t(span))) {
				    err = Error::INTERNAL_ERROR;
				    return false;
				}
			    }
			...
			if (encoded <= 255) {
			    if (!write_u8(f, OPC_RUN_DATA) || !write_u8(f, uint8_t(encoded))) {
				err = Error::INTERNAL_ERROR;
				return false;
			    }
			} else {
			    if (!write_u8(f, OPC_RUN_DATA | OPC_LONG_FLAG) || !write_u8(f, 0) || !write_u16_le(f, uint16_t(encoded))) {
				err = Error::INTERNAL_ERROR;
				return false;
			    }
			}
			if (!write_u16_le(f, uint16_t(v))) {
			    err = Error::INTERNAL_ERROR;
			    return false;
			}
			x += run_len;
		    ...
		    if (encoded <= 255) {
			if (!write_u8(f, OPC_BYTE_DATA) || !write_u8(f, uint8_t(encoded))) {
			    err = Error::INTERNAL_ERROR;
			    return false;
			}
		    } else {
			if (!write_u8(f, OPC_BYTE_DATA | OPC_LONG_FLAG) || !write_u8(f, 0) || !write_u16_le(f, uint16_t(encoded))) {
			    err = Error::INTERNAL_ERROR;
			    return false;
			}
		    }
	    ...
	    if (rle_y < H) {
		if (!write_u8(f, OPC_SKIP_LINES) || !write_u8(f, 1)) {
		    err = Error::INTERNAL_ERROR;
		    return false;
		}
	    }
	...
	if (!write_u8(f, OPC_EOF) || !write_u8(f, 0)) {
	    err = Error::INTERNAL_ERROR;
	    return false;
	}
	err = Error::OK;
    ...
    {
	FileWriteStream ws(f);
	return write(ws, img, bg_mode, err);
    }
};
...
public:
    template<typename Stream>
    static std::enable_if_t<!std::is_pointer_v<Stream>, DecoderResult>
    read(Stream& f, Image& img)
    {
	DecoderResult res;
	Header h;
	Endian e;
	Error herr;
	if (!read_header_auto(f, h, e, herr)) {
	    res.error = herr;
	    return res;
	}
	img.header = h;
	Error aerr;
	if (!img.allocate(aerr)) {
	    res.error = aerr;
	    return res;
	}
	...
	while (scan_y < ymin + H) {
		...
		case OPC_SET_COLOR: {
		    if (longForm) {
			res.error = Error::OPCODE_UNKNOWN;
			return res;
		    }
		    uint16_t ch = op1;
    ...
    // Wrapper for tests bypassing read_rgb
...
inline bool write_rgb(FILE* f, const uint8_t* interleaved, uint32_t width, uint32_t height,
		      const std::vector<std::string>& comments, const std::vector<uint8_t>& background,
		      bool include_alpha, Encoder::BackgroundMode bg_mode, Error& err)
{
    FileWriteStream ws(f);
    return write_rgb(ws, interleaved, width, height, comments, background, include_alpha, bg_mode, err);
}

inline bool read_rgb(FILE* f, std::vector<uint8_t>& interleaved, uint32_t& width, uint32_t& height,
		     bool* has_alpha_out, std::vector<std::string>* comments_out, Error& err)
{
    FileReadStream rs(f);
    return read_rgb(rs, interleaved, width, height, has_alpha_out, comments_out, err);
}
