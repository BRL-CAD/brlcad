...
/* Public constexpr limits (use these in client code) */
...
/* ----- Header reading (single endian attempt) ----- */
    ...
    static bool write(FILE* f, const Image& img, BackgroundMode bg_mode, Error& err) {
        ...
        while (y < H) {
            if (bg_mode != BG_SAVE_ALL && !h.no_background() && row_is_background(img, y)) {
                ...
                if (skipCount <= 255) {
                    if (!write_short(f, OPC_SKIP_LINES, uint16_t(skipCount))) { err = Error::INTERNAL_ERROR; return false; }
                } else {
                    if (!write_long(f, OPC_SKIP_LINES, uint16_t(skipCount))) { err = Error::INTERNAL_ERROR; return false; }
                }
                continue;
            }
            ...
            for (uint8_t c = 0; c < chans; ++c) {
                ...
                if (!write_short(f, OPC_SET_COLOR, operand)) { err = Error::INTERNAL_ERROR; return false; }

                uint32_t x = 0;
                uint64_t opsThisRow = 0;
                while (x < W) {
                    if (++opsThisRow > uint64_t(MAX_OPS_PER_ROW_FACTOR) * W) {
                        err = Error::OP_COUNT_EXCEEDED; return false;
                    }
                    ...
                    if (bg_mode != BG_SAVE_ALL && c < h.ncolors && pixel_is_background(img, x, y)) {
                        ...
                        if (span >= 2) {
                            if (span <= 255) {
                                if (!write_short(f, OPC_SKIP_PIXELS, uint16_t(span))) { err = Error::INTERNAL_ERROR; return false; }
                            } else {
                                if (!write_long(f, OPC_SKIP_PIXELS, uint16_t(span))) { err = Error::INTERNAL_ERROR; return false; }
                            }
                            continue;
                        } else {
                            x = start;
                    ...
                    if (run_len >= 3) {
                        ...
                        if (encoded <= 255) {
                            if (!write_short(f, OPC_RUN_DATA, uint16_t(encoded))) { err = Error::INTERNAL_ERROR; return false; }
                        } else {
                            if (!write_long(f, OPC_RUN_DATA, uint16_t(encoded))) { err = Error::INTERNAL_ERROR; return false; }
                        }
                    ...
                    if (encoded <= 255) {
                        if (!write_short(f, OPC_BYTE_DATA, uint16_t(encoded))) { err = Error::INTERNAL_ERROR; return false; }
                    } else {
                        if (!write_long(f, OPC_BYTE_DATA, uint16_t(encoded))) { err = Error::INTERNAL_ERROR; return false; }
                    }
                    for (uint8_t pv : lit)
...
private:
    ...
    static bool write_short(FILE* f, uint8_t opcode, uint16_t operand) {
        if (operand > 255) return false;
        return write_u8(f, opcode) && write_u8(f, uint8_t(operand));
    }
    static bool write_long(FILE* f, uint8_t base, uint16_t operand) {
        return write_u8(f, base | OPC_LONG_FLAG) && write_u8(f, 0) && write_u16_le(f, operand);
    }
};
        ...
        while (scan_y < ymin + H) {
            ...
            switch (base) {
                ...
                case OPC_SET_COLOR: {
                    ...
                    uint16_t ch = op1;
                    if (ch == 255 && h.has_alpha()) current_channel = h.ncolors;
                    else current_channel = int(ch);
                    scan_x = xmin;
                } break;
                ...
                case OPC_BYTE_DATA: {
                    ...
                    if (count & 1) {
                        uint8_t filler;
                        if (!read_u8(f, filler)) { res.error = Error::TRUNCATED_OPCODE; return res; }
                    }
                } break;
                case OPC_RUN_DATA: {
                    uint16_t enc;
