...
bool icv_to_u8_interleaved(const icv_image_t *img, std::vector<uint8_t> &buf, bool &has_alpha)
{
    ...
    try {
	buf.resize(static_cast<size_t>(npix) * channels_out);
    } catch (...) {
	return false;
    }
...
rle_write_mem(icv_image_t *bif, unsigned char **outbuffer, size_t *outsize)
{
    ...
    *outsize = 0;
    ...
    if (bif->width > rle::MAX_DIM || bif->height > rle::MAX_DIM) {
	bu_log("rle_write_mem: dimensions exceed maximum (%u x %u)\n", rle::MAX_DIM, rle::MAX_DIM);
	return BRLCAD_ERROR;
    }
    ...
    std::vector<uint8_t> data;
    if (!icv_to_u8_interleaved(bif, data, has_alpha)) {
	bu_log("rle_write_mem: conversion to 8-bit buffer failed\n");
	return BRLCAD_ERROR;
    }
	...
	for (size_t i = 0; i < npix; ++i) {
	    ...
	    rgb_only[3*i + 1] = data[4*i + 1];
	    rgb_only[3*i + 2] = data[4*i + 2];
	}
    }
    ...
    rle::MemWriteStream ws(out_vec);
    rle::Error err;

    bool ok = rle::write_rgb(ws, data.data(), static_cast<uint32_t>(bif->width),
			     static_cast<uint32_t>(bif->height), comments, bgd.color, has_alpha, bgd.mode, err);

    if (!ok || err != rle::Error::OK) {
	log_rle_error("rle_write_mem", err);
	return BRLCAD_ERROR;
    }
...
extern "C" icv_image_t*
rle_read_mem(const unsigned char *buffer, size_t size)
{
    if (!buffer || size == 0) {
	bu_log("rle_read_mem: null buffer\n");
	return NULL;
    }
    ...
    rle::MemReadStream rs(buffer, size);

    bool ok = rle::read_rgb(rs, rgb, width, height,
			    &has_alpha, &comments, err);

    if (!ok || err != rle::Error::OK) {
	log_rle_error("rle_read_mem", err);
	return NULL;
    }

    if (width > rle::MAX_DIM || height > rle::MAX_DIM) {
	bu_log("rle_read_mem: dimensions exceed maximum (%u x %u)\n",
	       rle::MAX_DIM, rle::MAX_DIM);
	return NULL;
    }
    ...
    if (!u8_interleaved_to_icv(rgb, static_cast<size_t>(width),
			       static_cast<size_t>(height), has_alpha, img)) {
	bu_log("rle_read_mem: buffer to icv image conversion failed\n");
	bu_free(img, "icv_image");
	return NULL;
    }
