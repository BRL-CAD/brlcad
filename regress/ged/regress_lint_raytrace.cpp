 ...
 *      Surface BoTs have a valid area but no solid volume.  The analyze
 *      command must return an error while retaining partial output, and
 *      lint must classify the failed analytic query rather than passing it
 *      as an object without an analytic reference.  A parent comb must be
 *      blocked from validation by that failure.
 */
...
tc6_metric_error_propagation(void)
{
    ...
    if (jfp) fclose(jfp);
    struct bu_vls json_vls = BU_VLS_INIT_ZERO;
    bu_vls_sprintf(&json_vls, "%s_tc6.json", json_path);

    const char *obj = "tc6_comb.c";
    std::string lint_output;
    int lint_ret = run_lint_raytrace(gfile.c_str(), bu_vls_cstr(&json_vls),
