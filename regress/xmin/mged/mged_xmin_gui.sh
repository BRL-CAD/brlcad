...
. "$GUI_TEST_SHELL"
GUI_STABLE_QUIET_MS=$STABLE_QUIET
GUI_STABLE_TIMEOUT_MS=$STABLE_TIMEOUT
...
MGED_PIPE_ORACLE=${MGED_PIPE_ORACLE:-${MGED_GUI_RC%/*}/xmin_gui_pipe_oracle.tcl}
...
wait_for_file_with_limit()
{
	...
	if [ -s "$MGED_GUI_TEST_DIR/result" ]; then
	    return 1
	fi
...
start_raytrace()
{
    ...
    if ! wait_for_value "$MGED_GUI_TEST_DIR/raytrace_invocations" "$1"; then
	fail "Raytrace Control Panel did not process button click $1"
    fi
    printf '%s\n' "$1" > "$MGED_GUI_TEST_DIR/raytrace_request"
}
...
query_database()
{
    database=$1
    query=$2
    error_file="$MGED_GUI_TEST_DIR/database_query_error"
    count=0
...
read_target()
{
    TARGET_WINDOW=$(sed -n '1p' "$MGED_GUI_TEST_DIR/$1_window")
    read_point "$MGED_GUI_TEST_DIR/$1"
}

click_target()
{
    read_target "$1"
    "$GUI_TEST_CTL" click --delay "$CLICK_DELAY" "$TARGET_WINDOW" "$POINT_X" "$POINT_Y" "${2:-1}"
}

activate_menu_target()
{
    read_target "$1"
    "$GUI_TEST_CTL" mouse-move "$TARGET_WINDOW" "$POINT_X" "$POINT_Y"
    "$GUI_TEST_CTL" wait-stable --quiet "$STABLE_QUIET" \
	--timeout "$STABLE_TIMEOUT" root >/dev/null 2>&1 || true
    "$GUI_TEST_CTL" button 1 down
    "$GUI_TEST_CTL" button 1 up
}
...
replace_entry_text()
{
    ...
    click_target "$target"
    "$GUI_TEST_CTL" key end
    remaining=${#initial_value}
    while [ "$remaining" -gt 0 ]; do
	"$GUI_TEST_CTL" key backspace
	remaining=$((remaining - 1))
    done
    wait_for_value "$current_file" "" || return 1
    "$GUI_TEST_CTL" type "$expected_value"
...
capture_frame()
{
    frames_dir="$MGED_GUI_RUN_DIR/frames"
    counter_file="$MGED_GUI_RUN_DIR/frame_counter"
    ...
    "$GUI_TEST_CTL" wait-stable --quiet "$STABLE_QUIET" \
	--timeout "$STABLE_TIMEOUT" root >/dev/null 2>&1 || true
    if ! "$GUI_TEST_CTL" capture-screen "$frames_dir/$frame_name" >/dev/null 2>&1; then
	return 1
    fi
    printf '%s\n' "$frame_number" > "$counter_file"
    printf '%06d %s %s\n' "$frame_number" "$MGED_DM_TYPE" "$1" \
	>> "$MGED_GUI_RUN_DIR/frames.log"
}
...
capture_dm_snapshot()
{
    snapshot="$MGED_GUI_TEST_DIR/$1.ppm"
    read -r dm_x dm_y dm_width dm_height \
	< "$MGED_GUI_TEST_DIR/active_dm_geometry"
    if ! gui_capture_region "$snapshot" "$dm_x" "$dm_y" \
        "$dm_width" "$dm_height"; then
...
wait_for_changed_dm_snapshot()
{
    ...
    baseline=$2
    snapshot_name=$3
    snapshot="$MGED_GUI_TEST_DIR/$snapshot_name.ppm"
    count=0
    changed_channels=0
...
assert_raytrace_duration()
{
    request=$1
    description=$2
    elapsed_file="$MGED_GUI_TEST_DIR/raytrace_elapsed_${request}_ms"
...
{
    setting=$1
    initial=$(sed -n '1p' "$MGED_GUI_TEST_DIR/renderer_${setting}_initial")
    mutable=$(sed -n '1p' "$MGED_GUI_TEST_DIR/renderer_${setting}_mutable")
    ...
    if ! wait_for_file "$MGED_GUI_TEST_DIR/renderer_${setting}" ||
       ! wait_for_file "$MGED_GUI_TEST_DIR/renderer_${setting}_window"; then
	fail "Misc menu did not expose $setting"
    fi
    record_state "Misc menu $setting"
    activate_menu_target "renderer_${setting}"
    if ! wait_for_value "$MGED_GUI_TEST_DIR/renderer_${setting}_gui_current" "$expected"; then
    ...
    if ! wait_for_value "$MGED_GUI_TEST_DIR/renderer_${setting}_current" \
    ...
    activate_menu_target "renderer_${setting}"
    if ! wait_for_value "$MGED_GUI_TEST_DIR/renderer_${setting}_gui_current" "$initial"; then
	fail "$setting menu state did not return to $initial"
    fi
    if ! wait_for_value "$MGED_GUI_TEST_DIR/renderer_${setting}_current" "$initial"; then
...
create_recording()
{
    ...
    validation_dir="$MGED_GUI_RUN_DIR/apng_validation"
    ...
    expected_frames=$(sed -n '1p' "$MGED_GUI_RUN_DIR/frame_counter")
...
fail()
{
    ...
    if [ -f "$MGED_GUI_TEST_DIR/result" ]; then
	sed -n '1,20p' "$MGED_GUI_TEST_DIR/result" >&2
    fi
    for diagnostic in "$MGED_GUI_TEST_DIR"/*_debug; do
	if [ -f "$diagnostic" ]; then
	    echo "$(basename "$diagnostic"): $(sed -n '1,20p' "$diagnostic")" >&2
	fi
    done
    if [ -f "$MGED_GUI_TEST_DIR/mged.log" ]; then
	echo "MGED log follows:" >&2
	sed -n '1,240p' "$MGED_GUI_TEST_DIR/mged.log" >&2
    fi
    exit 1
}

if [ "${MGED_GUI_INNER:-0}" -eq 0 ]; then
    gui_require_executable "$MGED_BIN"
    gui_require_executable "$GUI_TEST_CTL"
    gui_require_executable "$ICV_BIN"
    gui_require_executable "$PIXCROP_BIN"
    if [ ! -f "$MGED_GUI_RC" ]; then
	echo "FAIL: MGED GUI rc file is unavailable: $MGED_GUI_RC" >&2
	exit 1
    fi
    ...
    MGED_GUI_RUN_DIR=$(mktemp -d "${TMPDIR:-/tmp}/mged-gui.XXXXXX")
    export MGED_GUI_RUN_DIR
    run_status=0
    cleanup()
    {
	if [ -n "${MGED_PID:-}" ]; then
	...
	if [ "${MGED_GUI_KEEP:-0}" -eq 1 ]; then
	    echo "MGED GUI test artifacts: $MGED_GUI_RUN_DIR" >&2
	else
	    "$CMAKE_COMMAND" -E remove_directory "$MGED_GUI_RUN_DIR"
	fi
    }
    trap cleanup EXIT INT TERM

    MGED_GUI_INNER=1
    export MGED_GUI_INNER
    for MGED_DM_TYPE in $MGED_DM_TYPES; do
	export MGED_DM_TYPE
	MGED_GUI_TEST_DIR="$MGED_GUI_RUN_DIR/cases/$MGED_DM_TYPE"
	export MGED_GUI_TEST_DIR
	"$CMAKE_COMMAND" -E make_directory "$MGED_GUI_TEST_DIR"
	GUI_TEST_DIR=$MGED_GUI_TEST_DIR
	export GUI_TEST_DIR
	test_db="$MGED_GUI_TEST_DIR/gui-edit.g"
	"$MGED_BIN" -c "$test_db" "in gui.s sph 0 0 0 10" \
	    >"$MGED_GUI_TEST_DIR/create.log" 2>&1
	set +e
	sh "$0"
	dm_status=$?
	set -e
	...
	if [ "$sphere_vertex" != "5 0 0" ]; then
	    ...
	    sed -n '1,20p' "$MGED_GUI_TEST_DIR/database_query_error" >&2
	    run_status=1
	    break
	fi
	...
	read -r matrix_x matrix_y < "$MGED_GUI_TEST_DIR/matrix_expected"
...
"$MGED_BIN" --gui --dm-type "$MGED_DM_TYPE" --rcfile "$MGED_GUI_RC" \
    --geom "$COMMAND_GEOMETRY" --ggeom "$GRAPHICS_GEOMETRY" "$test_db" \
    >"$MGED_GUI_TEST_DIR/mged.log" 2>&1 &
MGED_PID=$!

if ! wait_for_file "$MGED_GUI_TEST_DIR/main_window"; then
    fail "MGED main window did not become ready"
fi

main_window=$(sed -n '1p' "$MGED_GUI_TEST_DIR/main_window")
"$GUI_TEST_CTL" activate "$main_window"
record_state "MGED ready"

for menu in $TOP_LEVEL_MENUS; do
    menu_name=${menu%%:*}
    menu_label=${menu#*:}
    if ! wait_for_file "$MGED_GUI_TEST_DIR/main_${menu_name}_menu" ||
       ! wait_for_file "$MGED_GUI_TEST_DIR/main_${menu_name}_menu_window"; then
	...
	if ! wait_for_file "$MGED_GUI_TEST_DIR/faceplate_toggle" ||
	   ! wait_for_file "$MGED_GUI_TEST_DIR/faceplate_toggle_window" ||
	   ! wait_for_file "$MGED_GUI_TEST_DIR/orig_gui_toggle" ||
	   ! wait_for_file "$MGED_GUI_TEST_DIR/orig_gui_toggle_window"; then
	    fail "Misc menu Faceplate targets did not become ready"
	fi
    fi
    "$GUI_TEST_CTL" key escape
done
"$GUI_TEST_CTL" click --delay "$CLICK_DELAY" "$main_window" 100 100 1
"$GUI_TEST_CTL" activate "$main_window"

if [ "$MGED_DM_TYPE" = "tkswrast" ] && [ -n "${MGED_GUI_MANIFEST:-}" ]; then
    if ! wait_for_file "$MGED_GUI_TEST_DIR/menu_inventory"; then
	fail "MGED menu inventory did not become ready"
    fi
    if ! "$CMAKE_COMMAND" -E compare_files \
	"$MGED_GUI_TEST_DIR/menu_inventory" "$MGED_GUI_MANIFEST"; then
	fail "MGED menu inventory differs from $MGED_GUI_MANIFEST"
    fi
fi

for setting in faceplate orig_gui; do
    if ! wait_for_file "$MGED_GUI_TEST_DIR/${setting}_current"; then
	fail "MGED did not report the current $setting state"
    fi
    if [ "$(sed -n '1p' "$MGED_GUI_TEST_DIR/${setting}_current")" -ne 1 ]; then
	click_target main_misc_menu
	record_state "Misc menu $setting"
	if ! wait_for_file "$MGED_GUI_TEST_DIR/${setting}_toggle" ||
	   ! wait_for_file "$MGED_GUI_TEST_DIR/${setting}_toggle_window"; then
	    fail "Misc menu did not expose $setting"
	fi
	activate_menu_target "${setting}_toggle"
	if ! wait_for_value "$MGED_GUI_TEST_DIR/${setting}_current" 1; then
...
if ! wait_for_file "$MGED_GUI_TEST_DIR/faceplate_header" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/faceplate_header_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/faceplate_3525" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/faceplate_3525_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/faceplate_view_initial"; then
    fail "Faceplate targets did not become ready"
fi
faceplate_view_initial=$(sed -n '1p' "$MGED_GUI_TEST_DIR/faceplate_view_initial")
click_target faceplate_header 2
if ! wait_for_file "$MGED_GUI_TEST_DIR/faceplate_general_ready"; then
    fail "Faceplate general menu did not expand"
fi
record_state "Faceplate general menu"
click_target faceplate_3525 2
if ! wait_for_not_value "$MGED_GUI_TEST_DIR/faceplate_view_current" \
...
if ! wait_for_file "$MGED_GUI_TEST_DIR/renderer_capabilities" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/main_misc_menu" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/main_misc_menu_window"; then
    fail "renderer capability inventory did not become ready"
fi
renderer_capabilities=$(sed -n '1p' "$MGED_GUI_TEST_DIR/renderer_capabilities")
if [ "$renderer_capabilities" != "none" ]; then
    for setting in $renderer_capabilities; do
	if ! wait_for_file "$MGED_GUI_TEST_DIR/renderer_${setting}_initial"; then
	    fail "renderer did not report initial $setting state"
	fi
	if ! wait_for_file "$MGED_GUI_TEST_DIR/renderer_${setting}_mutable"; then
...
"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/stop_general_monitors"
if ! wait_for_file "$MGED_GUI_TEST_DIR/renderer_monitor_stopped" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/faceplate_monitor_stopped"; then
    fail "MGED general-interface monitors did not stop"
fi

if ! wait_for_file "$MGED_GUI_TEST_DIR/main_edit_menu" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/main_edit_menu_window"; then
    fail "MGED Edit menu did not become ready"
fi
click_target main_edit_menu
record_state "Edit menu"

if ! wait_for_file "$MGED_GUI_TEST_DIR/primitive_editor_menu" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/primitive_editor_menu_window"; then
    fail "MGED Primitive Editor menu entry did not become ready"
fi
activate_menu_target primitive_editor_menu

if ! wait_for_file "$MGED_GUI_TEST_DIR/editor_window"; then
    fail "Primitive Editor did not open"
fi
record_state "Primitive Editor"

editor_window=$(sed -n '1p' "$MGED_GUI_TEST_DIR/editor_window")
"$GUI_TEST_CTL" activate "$editor_window"

if ! wait_for_file "$MGED_GUI_TEST_DIR/editor_name" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/editor_name_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/editor_name_initial" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/editor_reset" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/editor_reset_window"; then
    fail "Primitive Editor controls did not become ready"
fi
editor_name=$(sed -n '1p' "$MGED_GUI_TEST_DIR/editor_name_initial")
if ! replace_entry_text editor_name "$editor_name" \
    "$MGED_GUI_TEST_DIR/editor_name_current" gui.s; then
...
"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/editor_loaded"
if ! wait_for_file_with_limit "$MGED_GUI_TEST_DIR/sphere_vx" \
...
if ! wait_for_file "$MGED_GUI_TEST_DIR/sphere_vx" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sphere_vx_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sphere_vx_initial" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/apply_button_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/apply_button"; then
    fail "Primitive Editor did not load gui.s"
fi

initial_value=$(sed -n '1p' "$MGED_GUI_TEST_DIR/sphere_vx_initial")
...
if ! replace_entry_text sphere_vx "$initial_value" \
    "$MGED_GUI_TEST_DIR/sphere_vx_current" 5; then
...
"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/primitive_applied"

if ! wait_for_file "$MGED_GUI_TEST_DIR/primitive_result"; then
    fail "MGED did not report a primitive edit result"
fi
primitive_result=$(sed -n '1p' "$MGED_GUI_TEST_DIR/primitive_result")
record_state "sphere edit applied"

"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/begin_pipe"
if ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_ready" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_select" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_select_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_next" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_next_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_split" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_split_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_split_point" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_split_point_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_point" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_point_window"; then
...
"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/pipe_point_clicked"
if ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_selected"; then
    fail "pipe point was not selected through the viewport"
fi
pipe_keypoint=$(sed -n '1p' "$MGED_GUI_TEST_DIR/pipe_keypoint_current")
record_state "pipe point selected"
click_target pipe_next 2
if ! wait_for_not_value "$MGED_GUI_TEST_DIR/pipe_keypoint_current" \
    "$pipe_keypoint"; then
...
"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/pipe_finish"
if ! wait_for_file "$MGED_GUI_TEST_DIR/pipe_done"; then
    fail "pipe edit did not accept cleanly"
fi

"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/begin_sketch"
if ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_ready" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_create_line" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_create_line_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_zoom_in" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_zoom_in_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_reset" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_reset_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_dismiss" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_dismiss_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_canvas_start" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_canvas_start_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_canvas_end" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_canvas_end_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_scale_current" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_vertex_count_current"; then
    fail "Sketch Editor controls did not become ready"
fi
record_state "Sketch Editor ready"
sketch_scale=$(sed -n '1p' "$MGED_GUI_TEST_DIR/sketch_scale_current")
click_target sketch_zoom_in
if ! wait_for_not_value "$MGED_GUI_TEST_DIR/sketch_scale_current" \
    "$sketch_scale"; then
    fail "Sketch Editor Zoom In did not change the canvas scale"
fi
record_state "Sketch Editor zoomed"
sketch_vertex_count=$(sed -n '1p' \
    "$MGED_GUI_TEST_DIR/sketch_vertex_count_current")
...
if ! wait_for_not_value "$MGED_GUI_TEST_DIR/sketch_vertex_count_current" \
...
if ! wait_for_value "$MGED_GUI_TEST_DIR/sketch_vertex_count_current" \
    "$sketch_vertex_count"; then
    fail "Sketch Editor Reset Sketch did not restore the fixture"
fi
record_state "Sketch Editor reset"
click_target sketch_dismiss
if ! wait_for_file "$MGED_GUI_TEST_DIR/sketch_closed"; then
    fail "Sketch Editor did not dismiss cleanly"
fi

"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/begin_matrix"
if ! wait_for_file "$MGED_GUI_TEST_DIR/matrix_dm_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/matrix_dm"; then
...
"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/matrix_clicked"

if ! wait_for_file "$MGED_GUI_TEST_DIR/matrix_result"; then
    fail "MGED did not report a matrix edit result"
fi

matrix_result=$(sed -n '1p' "$MGED_GUI_TEST_DIR/matrix_result")
...
if ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_control_menu" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_control_menu_window"; then
    fail "Tools menu did not expose the Raytrace Control Panel"
fi
activate_menu_target raytrace_control_menu
if ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_ready" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_size" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_size_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_destination" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_destination_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_button" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_button_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_active" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_active_window" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_dismiss" ||
   ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_dismiss_window"; then
    fail "Raytrace Control Panel controls did not become ready"
fi
record_state "Raytrace Control Panel"

raytrace_size=$(sed -n '1p' "$MGED_GUI_TEST_DIR/raytrace_size_initial")
if ! replace_entry_text raytrace_size "$raytrace_size" \
    "$MGED_GUI_TEST_DIR/raytrace_size_current" "$RAYTRACE_SIZE"; then
    fail "Raytrace Control Panel size entry did not accept $RAYTRACE_SIZE"
fi
record_state "embedded raytrace size $RAYTRACE_SIZE"
"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/raytrace_prepare_reference"
if ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_reference_ready"; then
    fail "MGED did not prepare the standalone raytrace reference"
fi
reference_script=$(sed -n '1p' "$MGED_GUI_TEST_DIR/raytrace_reference_script")
reference_image=$(sed -n '1p' "$MGED_GUI_TEST_DIR/raytrace_reference_image")
...
if ! wait_for_value "$MGED_GUI_TEST_DIR/raytrace_complete" 1; then
...
for mode in overlay interlay underlay; do
    ...
    if ! wait_for_value "$MGED_GUI_TEST_DIR/raytrace_overlay_current" "$layer"; then
...
assert_images_differ "embedded raytrace overlay" \
    "$MGED_GUI_TEST_DIR/raytrace_baseline.ppm" \
    "$MGED_GUI_TEST_DIR/raytrace_overlay.ppm"

"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/raytrace_export_requested"
if ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_export_ready"; then
    fail "MGED did not export the embedded framebuffer"
fi
embedded_image=$(sed -n '1p' "$MGED_GUI_TEST_DIR/raytrace_export_image")
...
if ! wait_for_value "$MGED_GUI_TEST_DIR/raytrace_overlay_current" 2; then
    fail "embedded framebuffer did not return to overlay mode"
fi
capture_dm_snapshot raytrace_active
if ! wait_for_value "$MGED_GUI_TEST_DIR/raytrace_fb_current" 1; then
    fail "embedded framebuffer was not active after raytracing"
fi
click_target raytrace_active
if ! wait_for_value "$MGED_GUI_TEST_DIR/raytrace_fb_current" 0; then
...
wait_for_changed_dm_snapshot "framebuffer active toggle" \
    "$MGED_GUI_TEST_DIR/raytrace_active.ppm" raytrace_inactive
click_target raytrace_active
if ! wait_for_value "$MGED_GUI_TEST_DIR/raytrace_fb_current" 1; then
...
file_destination=$(sed -n '1p' "$MGED_GUI_TEST_DIR/raytrace_file_destination")
embedded_destination=$(sed -n '1p' "$MGED_GUI_TEST_DIR/raytrace_destination_initial")
if ! replace_entry_text raytrace_destination "$embedded_destination" \
    "$MGED_GUI_TEST_DIR/raytrace_destination_current" "$file_destination"; then
    fail "Raytrace Control Panel did not accept a file destination"
fi
record_state "raytrace file destination"
file_render_size=$(sed -n '1p' "$MGED_GUI_TEST_DIR/raytrace_size_current")
...
if ! wait_for_value "$MGED_GUI_TEST_DIR/raytrace_complete" 2; then
...
if ! replace_entry_text raytrace_destination "$file_destination" \
    "$MGED_GUI_TEST_DIR/raytrace_destination_current" "$embedded_destination"; then
...
if ! wait_for_file "$MGED_GUI_TEST_DIR/raytrace_closed"; then
...
"$GUI_TEST_CTL" key escape
"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/raytrace_responsive"

if ! wait_for_file "$MGED_GUI_TEST_DIR/result"; then
    fail "MGED did not remain responsive after embedded raytrace"
fi
result=$(sed -n '1p' "$MGED_GUI_TEST_DIR/result")
case "$result" in
    PASS:*) ;;
    *)
...
"$CMAKE_COMMAND" -E touch "$MGED_GUI_TEST_DIR/finish"
