...
FB_EXPORT extern size_t fb_get_max_width(fb *ifp);
FB_EXPORT extern size_t fb_get_max_height(fb *ifp);
FB_EXPORT extern size_t fb_getwidth(fb *ifp);
FB_EXPORT extern size_t fb_getheight(fb *ifp);
...
FB_EXPORT extern ssize_t fb_read(fb *ifp, size_t x, size_t y, unsigned char *pp, size_t count);
FB_EXPORT extern ssize_t fb_write(fb *ifp, size_t x, size_t y, const unsigned char *pp, size_t count);
FB_EXPORT extern int fb_rmap(fb *ifp, ColorMap *cmap);
FB_EXPORT extern int fb_wmap(fb *ifp, const ColorMap *cmap);
FB_EXPORT extern int fb_view(fb *ifp, size_t xcenter, size_t ycenter, size_t xzoom, size_t yzoom);
FB_EXPORT extern int fb_getview(fb *ifp, size_t *xcenter, size_t *ycenter, size_t *xzoom, size_t *yzoom);
FB_EXPORT extern int fb_setcursor(fb *ifp, const unsigned char *bits, size_t xb, size_t yb, size_t xo, size_t yo);
FB_EXPORT extern int fb_cursor(fb *ifp, int mode, size_t x, size_t y);
FB_EXPORT extern int fb_getcursor(fb *ifp, int *mode, size_t *x, size_t *y);
FB_EXPORT extern int fb_readrect(fb *ifp, size_t xmin, size_t ymin, size_t width, size_t height, unsigned char *pp);
FB_EXPORT extern int fb_writerect(fb *ifp, size_t xmin, size_t ymin, size_t width, size_t height, const unsigned char *pp);
FB_EXPORT extern int fb_bwreadrect(fb *ifp, size_t xmin, size_t ymin, size_t width, size_t height, unsigned char *pp);
FB_EXPORT extern int fb_bwwriterect(fb *ifp, size_t xmin, size_t ymin, size_t width, size_t height, const unsigned char *pp);

FB_EXPORT extern fb *fb_open(const char *file, size_t _width, size_t _height);
...
FB_EXPORT extern int fb_seek(fb *ifp, size_t x, size_t y);
FB_EXPORT extern int fb_tell(fb *ifp, size_t *xp, size_t *yp);
...
FB_EXPORT extern int fb_configure_window(fb *, size_t w, size_t h);
FB_EXPORT extern int fb_refresh(fb *ifp, size_t x, size_t y, size_t w, size_t h);
#if !defined(_WIN32) || defined(__CYGWIN__)
FB_EXPORT extern void fb_log(const char *fmt, ...) _BU_ATTR_PRINTF12;
#endif
FB_EXPORT extern int fb_null(fb *ifp);
FB_EXPORT extern int fb_null_setcursor(fb *ifp, const unsigned char *bits, size_t xbits, size_t ybits, size_t xorig, size_t yorig);

/* utility functions */
FB_EXPORT extern int fb_common_file_size(size_t *widthp, size_t *heightp, const char *filename, size_t pixel_size);
...
FB_EXPORT extern int fb_write_fp(fb *ifp, FILE *fp, size_t req_width, size_t req_height, int crunch, int inverse, struct bu_vls *result);
FB_EXPORT extern int fb_read_fd(fb *ifp, int fd, size_t file_width, size_t file_height, off_t file_xoff, off_t file_yoff, size_t scr_width, size_t scr_height, off_t scr_xoff, off_t scr_yoff, int fileinput, char *file_name, int one_line_only, int multiple_lines, int autosize, int inverse, int clear, int zoom, struct bu_vls *result);
...
FB_EXPORT extern fb *fb_open_existing(const char *file, size_t _width, size_t _height, struct fb_platform_specific *fb_p);

/* backward compatibility hacks */
FB_EXPORT extern int fb_reset(fb *ifp);
FB_EXPORT extern int fb_viewport(fb *ifp, size_t left, size_t top, size_t right, size_t bottom);
FB_EXPORT extern int fb_window(fb *ifp, size_t xcenter, size_t ycenter);
FB_EXPORT extern int fb_zoom(fb *ifp, size_t xzoom, size_t yzoom);
FB_EXPORT extern int fb_scursor(fb *ifp, int mode, size_t x, size_t y);
