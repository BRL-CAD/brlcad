...
NMG_EXPORT extern void rt_plot_vlblock(FILE *fp, const struct bn_vlblock *vbp);
NMG_EXPORT extern void rt_nurb_c_print(const struct edge_g_cnurb *crv);
NMG_EXPORT extern void rt_nurb_s_print(char *c, const struct face_g_snurb *srf);
NMG_EXPORT extern void rt_nurb_s_eval(const struct face_g_snurb *srf, fastf_t u, fastf_t v, fastf_t * final_value);
NMG_EXPORT extern void rt_nurb_c_eval(const struct edge_g_cnurb *crv, fastf_t param, fastf_t * final_value);
NMG_EXPORT extern void rt_nurb_kvknot(struct knot_vector *new_knots, int order, fastf_t lower, fastf_t upper, int num, struct resource *res);
NMG_EXPORT extern int rt_nurb_s_bound(struct face_g_snurb *srf, point_t bmin, point_t bmax);
NMG_EXPORT extern void rt_nurb_s_norm(struct face_g_snurb *srf, fastf_t u, fastf_t v, fastf_t * norm);
NMG_EXPORT extern struct edge_g_cnurb *rt_nurb_new_cnurb(int order, int n_knots, int n_pts, int pt_type);
...
NMG_EXPORT extern int nmg_bool_eval_silent;      /* quell output from nmg_booltree_evaluate */
...
NMG_EXPORT extern int nmg_ck_eg_verts(const struct edge_g_lseg *eg,
                                     const struct bn_tol *tol);
NMG_EXPORT extern int nmg_ck_geometry(const struct model *m,
                                     const struct bn_tol *tol);
...
NMG_EXPORT extern void nmg_snurb_fu_get_norm_at_vu(const struct faceuse *fu,
                                                  const struct vertexuse *vu,
                                                  vect_t norm);
NMG_EXPORT extern void nmg_find_zero_length_edges(const struct model *m);
...
NMG_EXPORT extern int nmg_decompose_shell(struct shell *s,
                                         const struct bn_tol *tol);
NMG_EXPORT extern void nmg_stash_model_to_file(const char *filename,
                                              const struct model *m,
                                              const char *title);
NMG_EXPORT extern int nmg_unbreak_region_edges(uint32_t *magic_p);
...
NMG_EXPORT extern int nmg_in_vert(struct vertex *new_v,
                                 const int approximate,
                                 const struct bn_tol *tol);
NMG_EXPORT extern void nmg_mirror_model(struct model *m);
...
/*  nmg_tri_mc.c */
NMG_EXPORT extern void nmg_triangulate_model_mc(struct model *m,
                                               const struct bn_tol *tol);
NMG_EXPORT extern int nmg_mc_realize_cube(struct shell *s,
                                         int pv,
                                         point_t *edges,
                                         const struct bn_tol *tol);
...
NMG_EXPORT extern void nmg_region_v_unique(struct nmgregion *r1,
					  const struct bn_tol *tol);
...
NMG_EXPORT extern int	nmg_two_face_fuse(struct face	*f1,
					  struct face *f2,
					  const struct bn_tol *tol);
NMG_EXPORT extern int nmg_model_face_fuse(struct model *m,
					 const struct bn_tol *tol);
