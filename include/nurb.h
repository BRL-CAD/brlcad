...
#define CURVATURE_INIT_ZERO { VINIT_ZERO, 0.0, 0.0 }
...
/* basis.c */
NURB_EXPORT extern fastf_t nurb_basis_eval(struct knot_vector *knts, int interval, int order, fastf_t mu);

/* bezier.c */
NURB_EXPORT extern int nurb_bezier(struct bu_list *bezier_hd, const struct face_g_snurb * srf, struct resource *res);
NURB_EXPORT extern int nurb_bez_check(const struct face_g_snurb * srf);
...
/* bound.c */
NURB_EXPORT extern int nurb_s_bound(struct face_g_snurb *srf, point_t bmin, point_t bmax);
NURB_EXPORT extern int nurb_c_bound(struct edge_g_cnurb *crv, point_t bmin, point_t bmax);
NURB_EXPORT extern int nurb_s_check(struct face_g_snurb *srf);
NURB_EXPORT extern int nurb_c_check(struct edge_g_cnurb *crv);

/* c2.c */
NURB_EXPORT extern void nurb_curvature(struct curvature *cvp,
				       const struct face_g_snurb *srf,
				       fastf_t u, fastf_t v);

/* copy.c */
NURB_EXPORT extern struct face_g_snurb *nurb_scopy(const struct face_g_snurb *srf, struct resource *res);
NURB_EXPORT extern struct edge_g_cnurb *nurb_crv_copy(const struct edge_g_cnurb * crv);

/* diff.c */
NURB_EXPORT extern struct face_g_snurb *nurb_s_diff(const struct face_g_snurb *srf, int dir);
NURB_EXPORT extern struct edge_g_cnurb *nurb_c_diff(const struct edge_g_cnurb *crv);
NURB_EXPORT extern void nurb_mesh_diff(int order, const fastf_t *o_pts,
				       fastf_t *n_pts,
				       const fastf_t *knots, int o_stride, int n_stride,
				       int o_size, int pt_type);

/* eval.c */
NURB_EXPORT extern void nurb_s_eval(const struct face_g_snurb *srf, fastf_t u, fastf_t v, fastf_t * final_value);
NURB_EXPORT extern void nurb_c_eval(const struct edge_g_cnurb *crv, fastf_t param, fastf_t * final_value);
NURB_EXPORT extern fastf_t *nurb_eval_crv(fastf_t *crv, int order,
					  fastf_t param,
					  const struct knot_vector *k_vec, int k_index, int coords);
NURB_EXPORT extern void nurb_pr_crv(fastf_t *crv, int c_size, int coords);

/* flat.c */
NURB_EXPORT extern int nurb_s_flat(struct face_g_snurb *srf, fastf_t epsilon);
NURB_EXPORT extern fastf_t nurb_crv_flat(fastf_t *crv, int size, int pt_type);

/* interp.c */
NURB_EXPORT extern void nurb_cinterp(struct edge_g_cnurb *crv, int order,
				     const fastf_t *data, int n);
NURB_EXPORT extern void nurb_sinterp(struct face_g_snurb *srf, int order,
				     const fastf_t *data, int ymax, int xmax);

/* knot.c */
NURB_EXPORT extern void nurb_kvknot(struct knot_vector *new_knots, int order,
				    fastf_t lower, fastf_t upper, int num, struct resource *res);
NURB_EXPORT extern void nurb_kvmult(struct knot_vector *new_kv,
				    const struct knot_vector *kv,
				    int num, fastf_t val, struct resource *res);
NURB_EXPORT extern void nurb_kvgen(struct knot_vector *kv,
				   fastf_t lower, fastf_t upper, int num, struct resource *res);
NURB_EXPORT extern void nurb_kvmerge(struct knot_vector *new_knots,
				     const struct knot_vector *kv1,
				     const struct knot_vector *kv2, struct resource *res);
NURB_EXPORT extern int nurb_kvcheck(fastf_t val, const struct knot_vector *kv);
NURB_EXPORT extern void nurb_kvextract(struct knot_vector *new_kv,
				       const struct knot_vector *kv,
				       int lower, int upper, struct resource *res);
NURB_EXPORT extern void nurb_kvcopy(struct knot_vector *new_kv,
				    const struct knot_vector *old_kv, struct resource *res);
NURB_EXPORT extern void nurb_kvnorm(struct knot_vector *kv);
NURB_EXPORT extern int nurb_knot_index(const struct knot_vector *kv, fastf_t k_value, int order);
NURB_EXPORT extern void nurb_gen_knot_vector(struct knot_vector *new_knots,
					     int order, fastf_t lower, fastf_t upper, struct resource *res);

/* norm.c */
NURB_EXPORT extern void nurb_s_norm(struct face_g_snurb *srf, fastf_t u, fastf_t v, fastf_t * norm);

/* oslo_calc.c */
NURB_EXPORT extern struct oslo_mat *nurb_calc_oslo(int order,
						   const struct knot_vector *tau_kv,
						   struct knot_vector *t_kv, struct resource *res);
NURB_EXPORT extern void nurb_pr_oslo(struct oslo_mat *om);
NURB_EXPORT extern void nurb_free_oslo(struct oslo_mat *om, struct resource *res);

/* oslo_map.c */
NURB_EXPORT extern void nurb_map_oslo(struct oslo_mat *oslo,
					 fastf_t *old_pts, fastf_t *new_pts,
					 int o_stride, int n_stride,
					 int lower, int upper, int pt_type);

/* plot.c */
NURB_EXPORT extern void nurb_plot_snurb(FILE *fp, const struct face_g_snurb *srf);
NURB_EXPORT extern void nurb_plot_cnurb(FILE *fp, const struct edge_g_cnurb *crv);
NURB_EXPORT extern void nurb_s_plot(const struct face_g_snurb *srf);

/* poly.c */
NURB_EXPORT extern struct nurb_poly *nurb_to_poly(struct face_g_snurb *srf);
NURB_EXPORT extern struct nurb_poly *nurb_mk_poly(fastf_t *v1, fastf_t *v2, fastf_t *v3,
						  fastf_t uv1[2], fastf_t uv2[2], fastf_t uv3[2]);

/* ray.c */
NURB_EXPORT extern struct face_g_snurb *nurb_project_srf(const struct face_g_snurb *srf,
							 plane_t plane1, plane_t plane2, struct resource *res);
NURB_EXPORT extern void nurb_clip_srf(const struct face_g_snurb *srf,
				      int dir, fastf_t *min, fastf_t *max);
NURB_EXPORT extern struct face_g_snurb *nurb_region_from_srf(const struct face_g_snurb *srf,
							     int dir, fastf_t param1, fastf_t param2, struct resource *res);
NURB_EXPORT extern struct nurb_uv_hit *nurb_intersect(const struct face_g_snurb * srf,
						      plane_t plane1, plane_t plane2, double uv_tol, struct resource *res,
						      struct bu_list *plist);

/* refine.c */
NURB_EXPORT extern struct face_g_snurb *nurb_s_refine(const struct face_g_snurb *srf,
						      int dir, struct knot_vector *kv, struct resource *res);
NURB_EXPORT extern struct edge_g_cnurb *nurb_c_refine(const struct edge_g_cnurb * crv,
						      struct knot_vector *kv);

/* solve.c */
NURB_EXPORT extern void nurb_solve(fastf_t *mat_1, fastf_t *mat_2,
				   fastf_t *solution, int dim, int coords);
NURB_EXPORT extern void nurb_doolittle(fastf_t *mat_1, fastf_t *mat_2,
				       int row, int coords);
NURB_EXPORT extern void nurb_forw_solve(const fastf_t *lu, const fastf_t *b,
					fastf_t *y, int n);
NURB_EXPORT extern void nurb_back_solve(const fastf_t *lu, const fastf_t *y,
					fastf_t *x, int n);
NURB_EXPORT extern void nurb_p_mat(const fastf_t * mat, int dim);

/* split.c */
NURB_EXPORT extern void nurb_s_split(struct bu_list *split_hd, const struct face_g_snurb *srf,
				     int dir, struct resource *res);
NURB_EXPORT extern void nurb_c_split(struct bu_list *split_hd, const struct edge_g_cnurb *crv);

/* trim.c */
NURB_EXPORT extern int nmg_uv_in_lu(const fastf_t u, const fastf_t v, const struct loopuse *lu);

/* util.c */
NURB_EXPORT extern struct face_g_snurb *nurb_new_snurb(int u_order, int v_order,
						       int n_u_knots, int n_v_knots,
						       int n_rows, int n_cols, int pt_type, struct resource *res);
NURB_EXPORT extern struct edge_g_cnurb *nurb_new_cnurb(int order, int n_knots,
						       int n_pts, int pt_type);
NURB_EXPORT extern void nurb_free_snurb(struct face_g_snurb *srf, struct resource *res);
NURB_EXPORT extern void nurb_free_cnurb(struct edge_g_cnurb * crv);
NURB_EXPORT extern void nurb_c_print(const struct edge_g_cnurb *crv);
NURB_EXPORT extern void nurb_s_print(char *c, const struct face_g_snurb *srf);
NURB_EXPORT extern void nurb_pr_kv(const struct knot_vector *kv);
NURB_EXPORT extern void nurb_pr_mesh(const struct face_g_snurb *m);
NURB_EXPORT extern void nurb_print_pt_type(int c);
NURB_EXPORT extern void nurb_clean_cnurb(struct edge_g_cnurb *crv);

/* xsplit.c */
NURB_EXPORT extern struct face_g_snurb *nurb_s_xsplit(struct face_g_snurb *srf, fastf_t param, int dir);
NURB_EXPORT extern struct edge_g_cnurb *nurb_c_xsplit(struct edge_g_cnurb *crv, fastf_t param);
