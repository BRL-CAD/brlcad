 ...
 * manipulation.
 */
#define BV_DBOBJ_BASED    0x001
#define BV_VIEWONLY       0x002
#define BV_LINES          0x004
#define BV_LABELS         0x008
#define BV_AXES           0x010
#define BV_POLYGONS       0x020
#define BV_MESH_LOD       0x040
#define BV_CSG_LOD        0x080
#define BV_CSG2_LOD       0x100
...
/* Experiment... try to define an efficient data structure for polyline
 * LoD data as generated by the CSG wireframe routines, to serve as an
 * analogy to the mesh LoD.  Won't be exactly the same as the data won't
 * be cached but is instead generated by the primitive routines, but see
 * if we can come up with something suitable as input to glDrawArrays.
 *
 * Internal storage is used so we can grow and shrink memory and do other
 * implementation optimizations with minimal impact. */
struct bv_polyline_lod {

    // The "public" data used for drawing.  One point array to a "line"
    // and a set of lines to a scene object.
    int array_cnt;
    point_t **parrays;
    int *pcnts;

    // Bounding box of the source (full detail) object
    point_t bmin;
    point_t bmax;
