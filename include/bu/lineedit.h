...
/**
 * Return the number of terminal character cells occupied by @p length bytes
 * of UTF-8 text.  Combining characters occupy no additional cell and common
 * East Asian wide characters and emoji occupy two.  Invalid bytes count as
 * one cell.  A NULL string has width zero.
 */
BU_EXPORT extern size_t bu_lineedit_text_width(const char *text, size_t length);

/**
 * Estimate the largest candidate set that can be displayed in a completion
 * viewport.  The estimate assumes the narrowest useful one-cell candidate
 * and the shared two-cell column gap, so querying this many candidates is
 * sufficient to fill @p max_lines at @p width without a fixed frontend limit.
 * Zero dimensions select the line editor's 80-by-5 fallback.  The result is
 * capped defensively for pathological terminal dimensions.
 */
BU_EXPORT extern size_t bu_cmd_completion_candidate_budget(size_t width,
	size_t max_lines);
...
/** Initialize @p layout before its first create or clear operation. */
BU_EXPORT extern void bu_cmd_completion_layout_init(
	struct bu_cmd_completion_layout *layout);

/**
 * Release all strings owned by an initialized @p layout and reset it to
 * empty.  Calling this repeatedly is safe; passing uninitialized storage is
 * not.  BU_CMD_COMPLETION_LAYOUT_INIT_ZERO is the static initializer.
 */
