...
The RT_DEBUG_RTCMP flag (0x10000000) (set with LIBRT_DEBUG=10000000) appends
one high-precision rtcmp JSON record per ray with final hit partitions to the
file named by LIBRT_RTCMP_FILE. Misses produce no record. librt never clears
the file; the caller must clear or rotate it before a new comparison as needed.
Separate processes must not write to the same file concurrently. Pending
records are flushed within two seconds under normal operation and during normal
process shutdown. A process terminated abnormally may lose pending records.
rtcmp comparison files should use the same hit-only capture mode on both sides;
its generation mode also includes misses.  The output can be expanded to also
include primitive segments if the environment variable LIBRT_RTCMP_PRIMITIVES
is set to 1.
