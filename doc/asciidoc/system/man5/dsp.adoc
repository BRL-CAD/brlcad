...
The command `bo -i u nS object_name file.dsp` imports this network-order data
into a uniform binary database object. BINUNIF values are host order while in
memory and network order when serialized in a `.g` database. A DSP using an
object data source consumes the host-order BINUNIF values.
