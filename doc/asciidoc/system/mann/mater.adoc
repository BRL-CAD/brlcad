...
In its default mode, *mater* assigns shader parameters, RGB color, and inheritance to an existing combination. The color is set as the color attribute. If the combination has no color set, the *mater* command will set both the color and rgb attributes. If rgb is present it will not change rgb but will update the color attribute. The color attribute takes precedence over the rgb attribute. The information may be included on the command line; otherwise the user will be prompted for it. Some available shaders are:
