...
In its default mode, *mater* assigns shader parameters, RGB color, and inheritance to an existing combination. The color is set as the color attribute. If the combination has no color set, the *mater* command will set both the color and rgb attributes. If rgb is present it will not change rgb but will update the color attribute. The color attribute takes precedence over the rgb attribute. Likewise, the *mater* command will set both the shader and oshader attributes if neither are present. If the oshader attribute is present *mater* will not change it but will update just the shader attribute. The shader attribute takes precendence over the oshader attribute. The information may be included on the command line; otherwise the user will be prompted for it. Some available shaders are:
...
The *get* filters may be repeated. Numeric *--id* and *--density* filters
accept exact values and the `<`, `<=`, `>`, and `>=` range forms. Repeated
filters within one category are alternatives; active id, density, and name
categories are combined. A positional pattern matches either an id or a
material name.
...
The *map* mode of operation uses density information (either from a file or from the current database) to synchronize the *material_id* and *material_name* properties of objects. The *--ids-from-names* mode will use *material_name* properties, perform a lookup on the density database to find a corresponding id number, and update the *material_id*. If an optional mapping file is supplied, the name lookup will also find id assignments based on the name equivalencies specified by the map.
...
Exactly one of *--ids-from-names* and *--names-from-ids* is required for a
mapping operation.
