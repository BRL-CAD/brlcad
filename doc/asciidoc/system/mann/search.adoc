...
The above operator applies the expression to every object below the current path in the tree hierarchy, and returns TRUE if one or more of them satisfies the expression. An object passes -above when it is above (has in its subtree) at least one object that matches. E.g., searching with -above -name s\* on /component/region.r/start_comb.c/prim1.s would return TRUE for /component and /component/region.r as start_comb.c is below both of them, but would return FALSE for any deeper objects (i.e., prim1.s in this example).
+
Optionally, inequality and equality constraints on depth can be appended to the operator to limit what relative depths are accepted as matches.
+
Note: using _-above_ as the expression within another _-above_ (e.g., **-above -above -name X**) is valid but will be slower than the normal case because the internal optimization for -above cannot be applied to the inner -above expression. Such constructs are rare in practice and will still produce correct results.
