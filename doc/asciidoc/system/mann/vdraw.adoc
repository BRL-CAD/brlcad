...
All textual command words may be abbreviated by their first letter.  Thus
`vdraw d a` is equivalent to `vdraw delete all`, and `vdraw v d name` is
equivalent to `vdraw vlist delete name`.

The command vocabulary, including the nested `vlist` operations, is declared
as a command tree.  Interactive command lines can therefore complete and
validate each level independently: after `vdraw vlist`, Tab offers `list` and
`delete`; after `vdraw vlist delete`, it expects the list name.
