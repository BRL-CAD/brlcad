...
.Add selected primitive parents to a group.
[example]
====
mged> **gr hull_parts.c + 25 -p**

Starts grouper mode, lists up to 25 selected entries, and adds the selected primitives' parent combinations to _hull_parts.c_.

====

.Remove selected primitives from a group.
[example]
====
mged> **gr hull_parts.c - 10**

Starts grouper mode for removing selected primitives from _hull_parts.c_, listing up to 10 selected entries.
