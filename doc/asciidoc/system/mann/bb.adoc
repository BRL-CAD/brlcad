...
*-t*::
Calculate a _tight_ axis-aligned bounding box that accounts for subtracted (negative) geometry. The default AABB is a loose bound derived from the CSG tree: subtracted members are ignored, so the box never shrinks to reflect material that has been carved away by a subtraction. With *-t*, the box is instead derived from the geometry as actually evaluated by the ray tracer (which honors subtractions), so the reported bound contracts to fit the remaining material. This bound is approximate - it is limited to the ray sampling resolution - and is slower to compute than the default. It cannot be combined with *-o*.
