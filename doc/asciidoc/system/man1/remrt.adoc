...
For local worker launches, *remrt* uses *libpkg*(3) inherited endpoint IPC transport instead of TCP. On POSIX this is normally an anonymous pipe or socketpair, and on Windows libpkg uses inherited Win32 pipe handles. In that mode no TCP listener is needed until a remote or passive host must connect back to the dispatcher.
