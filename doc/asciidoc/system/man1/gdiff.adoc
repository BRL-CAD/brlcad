...
The following options are recognized in basic diffing mode (each
short flag also has a long-form equivalent):


*-h*, *--help*, *-?*::
Print a usage message describing all options and exit.
...
*-F* _"filter_string"_, *--filter* _"filter_string"_::
...
*-M* _merged_file.g_, *--merge-file* _merged_file.g_::
Merge the files being diffed into one output file. *gdiff* refuses to
overwrite an existing _merged_file.g_.
...
This software is Copyright (c) 2014-2026 by the United States Government as represented by U.S. Army Research Laboratory.
