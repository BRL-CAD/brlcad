...
.Render a quick 256x256 preview to a file, with a little shading
[example]
====
**rt -A0.9 -c "set ambSamples=4" -s256 -o file.png file.g object

Renders to a PNG image with very minimal ambient occlusion (4 samples per ray).  We increase how bright the image will be with the *-A* ambient light option because of the occlusion option. 
====

.Render a scene but focusing on a specific part
[example]
====
**rt -c "autoview headlight.r" car.g car**

Renders a whole car object to a temporary window with a camera centered on a headlight.r object.
====
