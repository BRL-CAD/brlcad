...
:author_3: Betty J. Schueler
:author_4: Robert G. Parker
:author_5: John R. Anderson
:author_6: Dwayne L. Kregel
:affiliation: Army Research Laboratory
:affiliation_2: SURVICE Engineering Company
:affiliation_3: Quantum Research International, Inc.
[.authors]
--
* {author} — {affiliation}
* {author_2} — {affiliation_2}
* {author_3} — {affiliation_3}
* {author_4} — {affiliation}
* {author_5} — {affiliation}
* {author_6} — {affiliation_2}
--
...
* Use the tops command to identify top-level objects in the database.
...
The *ls* command lists every object, which can be overwhelming in an existing
database. The *tops* command is usually the best starting point when exploring
an unfamiliar database: it lists only objects that are not members of another
combination. These top-level objects are typically the assemblies or models
you will want to draw or inspect first. At the Command Window prompt, type:
*tops[ENTER]*

In the simple _sphere.g_ database, the primitive shapes are still top-level
objects. As you build combinations in later lessons, *tops* will omit their
member shapes and show the higher-level entry points instead.
...
* Used the tops command to identify top-level database objects.
...
[NOTE]
====
Note that we could have achieved the same results on the Command Line by using the rm command to remove rcc1.s from base1.r and then adding rcc1.c: *rm base1.r/rcc1.s[Enter]* *r base1.r u rcc1.c[Enter]*
