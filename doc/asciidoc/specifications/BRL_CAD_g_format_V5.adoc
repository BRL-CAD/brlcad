...
* An Object_Length element that stores the total size of the object, including the magic numbers, as a count of 8-byte chunks.
...
|Object_Name (Name_Length, Name_Data)
|Present when NP = 1

.2+|Object Interior (individually compressible)
|Object_Attributes (Attribute_Length, Attribute_Data)
|Present when AP = 1

|Object_Body (Body_Length, Body_Data)
|Present when BP = 1
...
The Object_Length field stores the number of 8-byte chunks used to store the complete object, including all bytes from Magic1 through Magic2, inclusive. The total object size in bytes is `Object_Length * 8`.
...
ObjLen = 1 (1 byte; one 8-byte chunk)
Magic2 (1 byte)
....
...
Object_Length = 1 (1 byte; one 8-byte chunk)
Magic2 (1 byte)
....
