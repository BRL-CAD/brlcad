...
orbit it and `-p` sets the field of view.

NOTE: Heavily-booleaned models can lose parts that fail on-the-fly
tessellation -- the toy jeep's front wheels and roll bar among them.  See
<<facetize_first>> to render such models faithfully; the jeep panels later in
this article were produced that way.

For scenes that are not simply
...
ADRT tessellates as it loads, but two kinds of geometry do not come through: a
solid with no finite triangulation (an infinite half-space) is skipped, and --
more subtly -- a CSG region whose on-the-fly NMG boolean evaluation *fails* is
dropped silently, contributing no triangles while the rest of the model still
renders.  The load log flags the latter as `FAILED in boolean evaluation:
<path>`.  `rt` traces such models natively, but for ADRT the robust fix is to
convert them to triangles once with the far more capable Manifold facetizer and
render the resulting mesh.

Two of the sample models here need this.  The toy jeep drops its front wheels,
roll bar, and windshield frame on a direct load.  Facetizing with `--regions`
rebuilds each region as a BoT while preserving the region hierarchy -- and thus
the colors inherited from parent combinations -- so the jeep comes back
complete *and* in full color:

----
# in mged, on a working copy of the database
facetize --regions all all_fac
adrt -s 512 -H 256 -a 35 -e 25 -o toyjeep.png toyjeep.g all_fac
----

The castle is a single region unioning dozens of instanced solids; there a
single merged BoT is simplest, carrying one material for the whole model:
...
The `spall` shader is a ballistic-analysis mode: it fires a shot into the model
and draws the resulting fragmentation cone while cutting the geometry away at the
impact plane to reveal the interior.  `adrt` synthesizes a shot across the model
from the current view (tune the cone's full angle with `-A`), or takes an
explicit `"(pos) (dir) angle"` via `-b`:

----
adrt -s 512 -m spall -a 35 -e 25 -A 45 -o spall.png toyjeep.g all
----

.`adrt -m spall`: the fragmentation cone with the vehicle cut away at the impact plane
image::images/adrt_toyjeep_spall.png[spall,384]
...
* More subtly, a CSG region whose on-the-fly NMG boolean evaluation *fails* is
  ...
  Manifold facetizer (see <<facetize_first,Complex CSG: facetize first>>) and render the mesh.
