...
editing keypoint. When __path_rhs__ is also specified, the two paths are
joined conceptually: the matrix between the final component of
__path_lhs__ and the first component of __path_rhs__ is selected, and the
primitive at the end of __path_rhs__ supplies the keypoint.
...
To operate on this combination without choosing a primitive keypoint, use
the complete object path as the single argument:
...
This form uses the center of the bounding box of
__outer-candy-sphere.c__ as the keypoint. To use a primitive's default
keypoint instead, split the path at the matrix to edit and supply a right
hand path ending in that primitive. For example, any of the following
selects the matrix between __outer-candy-shell.r__ and
__outer-candy-sphere.c__ while obtaining the keypoint from a different
primitive:
...
Because they are primitives in the outer-candy-sphere.c combination,
outer-candy-sphere.s, core.s and stick.s are all legal reference shapes -
there will be illustrations later of the consequences of different
primitive choices.
...
Notice in the previous section that sphere.s was explicitly selected as part of the *oed* __path_rhs__. What would the impact have been had cube.s been selected instead?
...
Selection of a particular primitive when using the two-path form of *oed*
will depend on the particular situation. If no primitive has an
appropriate default keypoint, use the one-path form to select the object's
bounding-box center or set the keypoint explicitly after entering object
edit mode.
...
The one-path form of *oed* uses the center of the specified object's
bounding box as the point from which translations, rotations, and scaling
are defined. The two-path form instead derives this point from the
specified primitive. BRL-CAD's term for this point is the _keypoint_. For
cases where neither default is appropriate, BRL-CAD also offers an option
for explicitly setting the keypoint.
...
* If __path_rhs__ is omitted, __path_lhs__ is the complete path to the object being edited and its bounding-box center is the keypoint.

* If __path_rhs__ is supplied, it must end in a primitive shape whose default keypoint will be used for object editing.
...
* _keypoint_ is used in object edit mode as the point moved by *translate* operations and around which rotations occur. It is set from the object's bounding-box center or the primitive supplied to *oed*, and can also be set manually in object edit mode with the *keypoint* command.
