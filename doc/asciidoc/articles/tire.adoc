...
|250-50RD17
|Unsupported separator form and multiple characters before the rim diameter.
|185/65/15
|Second separator is not R.
|===
...
Tread is the most difficult part of a tire to define in terms of the amount of input data required. The tire tool provides several built-in tread presets and can also read a custom JSON tread pattern definition.footnote:[Note that in most cases, BRL-CAD will not have pre-defined knowledge of specific real world tread patterns.]

Tread profile and tread pattern are the two factors to consider when specifying tread. Tread profile is the "shape" of the tread on the edges of the tread pattern. Many truck tires have a "squared off" tread, while automobile tires tend to have more rounded tread. Named tread presets select a default profile and pattern count, and the legacy numeric aliases remain available.

The _t_ flag controls the tread profile, and the _p_ flag controls the tread pattern preset. The following legacy aliases are available:
...
Use _--list-tread-patterns_ to list all built-in presets. Current presets include highway-rib, all-terrain, mud-terrain, winter-siped, commercial-rib, and the legacy aliases. Use _--tread-pattern-file_ to supply a custom JSON definition.


Using either the _t_ or _p_ flags to the tire command will trigger tread generation. If a named preset is supplied, its default profile is used unless a _t_ profile is also supplied. Here is the default output for legacy style one:
...
Using _t_ in place of _p_ with one of the legacy profiles also produces a treaded model. Internally the *tire* command must use some pattern and profile for every tread it creates, and for ease of use it will select a default if a pattern or profile is specified by itself. Using the flags together can produce different results - for example, profile 2 with the legacy-rib preset:
