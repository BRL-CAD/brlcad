...
*bo*_[-o|-i pattern [h|n]type] dest source_

The “bo” command is used to create or retrieve binary opaque objects. One of _-i_ or _-o_ must be specified.

The _-o_ option “outputs” or extracts a binary object from the database object _source_ to a host-order file called _dest_.

The _-i_ option “inputs” or imports a file called _source_ into a binary object called _dest_ in the database. There are two additional arguments that must be specified with the _-i_ option: pattern and type. Currently, only uniform binary objects (arrays of values) are supported. As a result, the _pattern_ is always _u_ for “uniform” pattern. Input defaults to host byte order. An `h` or `n` prefix on _type_ explicitly selects host or network byte order. The base _type_ can be one of the following:
...
`mged>` *bo -i u nS dsp.data terrain.dsp*

– Import a standard network-order DSP data file as an unsigned-short binary object named _dsp.data_.
...
* sxscale – Enter solid scale (x-direction only) mode (must be in primitive edit mode).
* syscale – Enter solid scale (y-direction only) mode (must be in primitive edit mode).
* szscale – Enter solid scale (z-direction only) mode (must be in primitive edit mode).
