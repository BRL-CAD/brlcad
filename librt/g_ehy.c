...
struct rt_i		*rtip;
{
	...
	mat_mul( ehy->ehy_SoR, S, R );
	mat_mul( ehy->ehy_invRoS, Rinv, S );
...
register CONST struct soltab *stp;
{
	...
	fprintf(stderr, "c = %g\n", ehy->ehy_cprime);
	mat_print("S o R", ehy->ehy_SoR );
	mat_print("invR o S", ehy->ehy_invRoS );
}
...
register struct xray	*rp;
{
	...
	case EHY_NORM_TOP:
		...
		break;
	default:
		rt_log("rt_ehy_norm: surfno=%d bad\n", hitp->hit_surfno);
		break;
	}
}
...
struct soltab		*stp;
{
	...
	case EHY_NORM_BODY:
		...
		M1[10] = -1.;
		/* M1 = invR * S * M1 * S * R */
		mat_mul( M2, ehy->ehy_invRoS, M1);
		mat_mul( M1, M2, ehy->ehy_SoR );
...
struct rt_tol		*tol;
{
	...
	VREVERSE( Hunit, xip->ehy_H );
	VUNITIZE( Hunit );
	VMOVE(    Aunit, xip->ehy_Au );		/* make B positive z axis */
	VCROSS(   Bunit, Aunit, Hunit );	/* make R positive y axis */
	r1 = xip->ehy_r1;
	r2 = xip->ehy_r2;
	b = sqrt(MAGSQ(xip->ehy_H));
	c = xip->ehy_c;
	VSCALE(R1, Aunit, r1);
	VSCALE(R2, Bunit, r2);
	rt_ell_16pts( top, xip->ehy_V, R1, R2 );
	...
	for (i = 0; i < 4; i++) {
		...
		VADD2(front+j, front+j, xip->ehy_V);
		/* second point */
		y = -y;
		j = i * 3;
		VBLEND2(front+j, y, Aunit, z, Hunit);
...
struct rt_tol		*tol;
{
	...
	/* number of faces needed for ehy */
	...
	if (!outfaceuses || !ellipses) {
		fprintf(stderr, "rt_ehy_tess: no mem!\n");
		goto fail;
	}
...
register CONST mat_t		mat;
{
	...
	/* Check record type */
	if( rp->u_id != ID_SOLID )  {
		rt_log("rt_ehy_import: defective record\n");
		return(-1);
	}

	RT_INIT_DB_INTERNAL( ip );
	ip->idb_type = ID_EHY;
	ip->idb_ptr = rt_malloc( sizeof(struct rt_ehy_internal), "rt_ehy_internal");
	xip = (struct rt_ehy_internal *)ip->idb_ptr;
	...
	xip->ehy_r2 = rp->s.s_values[4*3];
	xip->ehy_c  = rp->s.s_values[5*3];
...
double				local2mm;
{
	...
	ep->ext_buf = (genptr_t)rt_calloc( 1, ep->ext_nbytes, "ehy external");
	...
	ehy->s.s_values[4*3] = xip->ehy_r2;
	ehy->s.s_values[5*3] = xip->ehy_c;
...
double			mm2local;
{
	...
	RT_EHY_CK_MAGIC(xip);
	rt_vls_strcat( str, "Elliptical Paraboloid (EHY)\n");
	...
	sprintf(buf, "\tAu (%g, %g, %g) mag=%g\n",
		xip->ehy_Au[X] * mm2local,
		xip->ehy_Au[Y] * mm2local,
		xip->ehy_Au[Z] * mm2local,
		MAGNITUDE(xip->ehy_Au) * mm2local);
	rt_vls_strcat( str, buf );
