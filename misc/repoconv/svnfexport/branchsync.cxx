...
void branch_sync(std::ifstream &infile, std::ofstream &outfile, int rev_num, std::string &bsrc)
{
    struct svn_revision &rev = revs[rev_num];
	...
	if (node.branch.length() && !node.branch.compare(bsrc)) {
		...
		if (!gsha1.length()) {
		    std::cout << "Fatal: git blob not found: " << node.local_path << ", content sha1: " << node.text_content_sha1 << " , git sha1: " << svn_sha1_to_git_sha1[node.text_content_sha1] << "\n";
		    exit(1);
		}
		outfile << "M ";
		if (node.exec_path) {
