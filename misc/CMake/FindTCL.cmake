...
MACRO(FIND_LIBRARY_AUTOPATHS FOUND_PATHS targetlib majorlibnum minorlibnum LIBRARY_VARIABLE)
  ...
  FOREACH(foundpath ${${FOUND_PATHS}})
	  ...
	  FIND_LIBRARY(${LIBRARY_VARIABLE} ${targetlib}${MAJORNUM}.${MINORNUM} ${targetlib}${MAJORNUM}${MINORNUM} PATHS ${LIBRARY_SEARCH_PATHS} NO_SYSTEM_PATH)
  ENDFOREACH()
ENDMACRO()
...
FOREACH(tcl_config_file ${TCLCONFIG_LIST}) 
  IF(NOT TCLVALID)
    ...
    IF(TCLVALID)
      ...
      FIND_LIBRARY(TCL_LIBRARY tcl Tcl tcl${TCL_MAJOR_VERSION}.${TCL_MINOR_VERSION} tcl${TCL_MAJOR_VERSION}${TCL_MINOR_VERSION} PATHS ${TCL_LIBRARY_DIR1} ${TCL_LIBRARY_DIR2} ${TCL_PREFIX}/lib NO_SYSTEM_PATH)
      FIND_LIBRARY(TCL_STUB_LIBRARY tclstub${TCL_MAJOR_VERSION}.${TCL_MINOR_VERSION} tclstub${TCL_MAJOR_VERSION}${TCL_MINOR_VERSION} PATHS ${TCL_STUB_LIB_PATH} ${TCL_LIBRARY_DIR1} ${TCL_LIBRARY_DIR2} ${TCL_PREFIX}/lib NO_SYSTEM_PATH)
  ...
  IF(TCLVALID)
	...
	IF(NOT TKVALID)
	  ...
	  IF(TKVALID)
	    ...
	    FIND_LIBRARY(TK_LIBRARY tk Tk tk${TK_MAJOR_VERSION}.${TK_MINOR_VERSION} tk${TK_MAJOR_VERSION}${TK_MINOR_VERSION} PATHS ${TK_LIBRARY_DIR1} ${TK_LIBRARY_DIR2} ${TK_PREFIX}/lib NO_SYSTEM_PATH)
	    FIND_LIBRARY(TK_STUB_LIBRARY tkstub tkstub${TK_MAJOR_VERSION}.${TK_MINOR_VERSION} tkstub${TK_MAJOR_VERSION}${TK_MINOR_VERSION} PATHS ${TK_STUB_LIB_PATH} ${TK_LIBRARY_DIR1} ${TK_LIBRARY_DIR2} ${TK_PREFIX}/lib NO_SYSTEM_PATH)
	  ENDIF(TKVALID)
	ENDIF(NOT TKVALID)
...
IF(NOT TCL_LIBRARY OR NOT TCL_TCLSH)
  ...
  FOREACH(MAJORNUM ${TCL_POSSIBLE_MAJOR_VERSIONS})
    FOREACH(MINORNUM ${TCL_POSSIBLE_MINOR_VERSIONS})
      FOREACH(SPATH ${TCLPATHLIST})
	IF(NOT TCL_FOUND_VERSION)
	  SET(CURRENT_SEARCH_VERSION "${MAJORNUM}.${MINORNUM}")
	  VALIDATE_VERSION(dosearch ${CURRENT_SEARCH_VERSION})
	  IF(NOT TCL_LIBRARY OR NOT TCL_TCLSH)
	    IF(dosearch)
	      FIND_PROGRAM(TCL_TCLSH NAMES tclsh${MAJORNUM}.${MINORNUM} tclsh${MAJORNUM}${MINORNUM} PATHS ${SPATH} ${SPATH}/bin NO_SYSTEM_PATH)
	      FIND_LIBRARY(TCL_LIBRARY NAMES tcl${MAJORNUM}.${MINORNUM} tcl${MAJORNUM}${MINORNUM} PATHS ${SPATH} ${SPATH}/lib ${SPATH}/lib64 ${SPATH}/lib${MAJORNUM}.${MINORNUM} ${SPATH}${MAJORNUM}${MINORNUM} NO_SYSTEM_PATH)
	  ...
	  IF(TCL_CURRENTPATH)
	    ...
	    LIST(REMOVE_ITEM ${pathnames} ${SPATH})
	  ENDIF(TCL_CURRENTPATH)
	  IF(TCL_FOUND_VERSION)
	    IF(TCL_REQUIRE_TK)
	      SET(TK_FOUND_VERSION "NOTFOUND")
	      FOREACH(MAJORNUM ${TCL_POSSIBLE_MAJOR_VERSIONS})
		FOREACH(MINORNUM ${TCL_POSSIBLE_MINOR_VERSIONS})
		  FOREACH(SPATH ${TKPATHLIST})
		    IF(NOT TK_LIBRARY OR NOT TK_WISH)
		      SET(CURRENT_SEARCH_VERSION "${MAJORNUM}.${MINORNUM}")
		      SET(vtcltkcompare 0)
		      COMPARE_VERSIONS(${TCL_FOUND_VERSION} ${CURRENT_SEARCH_VERSION} vtcltkcompare)
		      IF(${vtcltkcompare})
			RESET_TK_VARS()
		      ELSE(${vtcltkcompare})
			FIND_PROGRAM(TK_WISH NAMES wish${MAJORNUM}.${MINORNUM} wish${MAJORNUM}${MINORNUM} PATHS ${SPATH} ${SPATH}/bin NO_SYSTEM_PATH)
			FIND_LIBRARY(TK_LIBRARY NAMES tk${MAJORNUM}.${MINORNUM} tk${MAJORNUM}${MINORNUM} PATHS ${SPATH} ${SPATH}/lib ${SPATH}/lib64 ${SPATH}/lib${MAJORNUM}.${MINORNUM} ${SPATH}${MAJORNUM}${MINORNUM} NO_SYSTEM_PATH)
			FIND_LIBRARY(TK_STUB_LIBRARY tkstub${MAJORNUM}.${MINORNUM} tkstub${MAJORNUM}${MINORNUM} PATHS ${SPATH} ${SPATH}/lib ${SPATH}/lib64 ${SPATH}/lib${MAJORNUM}.${MINORNUM} ${SPATH}${MAJORNUM}${MINORNUM} NO_SYSTEM_PATH)
			FIND_PATH(TK_INCLUDE_PATH NAMES tk.h PATHS ${SPATH}/include ${SPATH}/include/tk${MAJORNUM}.${MINORNUM} ${SPATH}/include/tk${MAJORNUM}${MINORNUM} NO_SYSTEM_PATH)
			IF(TCL_NEED_HEADERS)
			  IF(NOT TK_INCLUDE_PATH)
			    SET(TK_LIBRARY "NOTFOUND")
			  ENDIF(NOT TK_INCLUDE_PATH)
