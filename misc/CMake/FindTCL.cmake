...
MACRO(FIND_CONFIG_FILES FOUND_FILES TARGET_LIST filename)
   SET(LOCAL_FOUND_FILES "${${FOUND_FILES}}")
   STRING(REGEX REPLACE "/s?bin" "/lib" FOUND_FILES32 "${LOCAL_FOUND_FILES}")
   STRING(REGEX REPLACE "/s?bin" "/lib64" FOUND_FILES64 "${LOCAL_FOUND_FILES}")
   SET(LOCAL_FOUND_FILES "${FOUND_FILES32};${FOUND_FILES64}")
   LIST(REMOVE_DUPLICATES FOUND_FILES)
   SET(FOUND_PATHS "")
   FOREACH(foundfile ${LOCAL_FOUND_FILES})
   ...
   FOREACH(foundpath ${FOUND_PATHS})
        ...
        find_file(conffile ${filename} PATHS ${foundpath} NO_SYSTEM_PATH)
...
ELSE(TCL_PREFIX)
   ...
   IF(NOT APPLE OR NOT TCL_USE_FRAMEWORK_ONLY)                                                                      
      ...
      FOREACH(foundpath ${LIBPATHLIST})
           STRING(REGEX REPLACE "/lib" "/lib64" 64BITPATH "${foundpath}")
           SET(LIBPATHLIST "${LIBPATHLIST};${64BITPATH}")
      ENDFOREACH()
      SET(LIBPATHLIST "${TCL_ADDITIONAL_SEARCH_PATHS};${PATHLIST}")
      LIST(REMOVE_DUPLICATES LIBPATHLIST)
      FIND_LIBRARY_VERSIONS(tcl LIBPATHLIST NO_SYSTEM_PATH)
      FIND_LIBRARY_VERSIONS(tk LIBPATHLIST NO_SYSTEM_PATH)
      # Hunt up tclConfig.sh files
      SET(FOUND_FILES "${TCL_tclsh_LIST};${TCL_tcl_LIST}")
      FIND_CONFIG_FILES(FOUND_FILES TCLCONFIG_LIST tclConfig.sh)
      # Hunt up tkConfig.sh files 
      SET(FOUND_FILES "${TCL_wish_LIST};${TCL_tk_LIST}")
   ...
   MESSAGE("Tk config file list: ${TKCONFIG_LIST}")

   SET(TCLVALID 0)
   FOREACH(tcl_config_file ${TCLCONFIG_LIST}) 
      MESSAGE("tcl_config_file: ${tcl_config_file}")
      IF(NOT TCLVALID)
	RESET_TCL_VARS()
	...
	IF(TCLVALID)
	  ...
	  MESSAGE("TCL_LIBRARY_DIR1: ${TCL_LIBRARY_DIR1} TCL_LIBRARY_DIR2: ${TCL_LIBRARY_DIR2}")
