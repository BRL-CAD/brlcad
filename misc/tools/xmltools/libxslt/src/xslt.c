/*
 * xslt.c: Implementation of an XSL Transformation 1.0 engine
 *
 * Reference:
 *   XSLT specification
			     ...
			     int isLRE)
{
	...
	*
	* NOTE: The ns-aliasing mechanism is non-cascading.
...
xsltCompilerNodePush(xsltCompilerCtxtPtr cctxt, xmlNodePtr node)
{
    ...
    * NOTE: Be careful with the @node, since it might be
    *  a doc-node.
    */
    inode->node = node;
...
*         -1 if an internal or API error occurred.
*/
static int
...
for_each:
    ...
    *   The old behaviour did not allow this, but it caught this
...
xsltForwardsCompatUnkownItemCreate(xsltCompilerCtxtPtr cctxt)
{
    ...
    item->type = XSLT_FUNC_UNKNOWN_FORWARDS_COMPAT;
    /*
    * Store it in the stylesheet.
    */
    item->next = cctxt->style->preComps;
...
xsltParseSequenceConstructor(xsltCompilerCtxtPtr cctxt, xmlNodePtr cur)
{
    ...
    */
	...
	return;
    /*
    * This is the content refered to as a "template".
    ...
    */
	...
	if (cur->type == XML_ELEMENT_NODE) {
		...
		* processed by the implementer of the extension function;
 ...
 *  constructor" in XSLT 2.0); it it allows additional xsl:param
		...
		*/
		if (cur->children != NULL) {
		 ...
		 "xsltParseTemplateContent: xslt:text content problem\n");
		    ...
		    /*
		     * replace xsl:text by the list of children
		     */
		    if (text == NULL) {
			text = cur->children;
			while (text != NULL) {
...
xsltParseStylesheetImportedDoc(xmlDocPtr doc,
    ...
    *  the stylesheet to contain distinct namespace prefixes.
    */
