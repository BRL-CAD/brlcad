...
Print error messages generated by Parse::RecDescent to the file
'errfile'

=item B<-f, --functions>

Prints the signatures of functions defined in the source file to
stdout (the function bodys are ommited)
...
Generate precompiled parsers Cgrammar.pm and DecommentGrammar.pm in
the current working directory. Precompiled parsers will speed up
parsing.  If these files are available in the current working
directory they will be used automatically. Every time the --precompile
option is set the precompiled parsers are generateted newly so this
option should only be used only once.  Also, don't forget to recreate
the precompiled parseres if you modify the grammar.

=item B<-s, --skippedlines>

Show which lines had been skipped during parse due to parser errors or
unrecognized tokens in the C source code
...
Print tracecode of parse level 3 (C code without preprocessor
...
If you don't get the output you expect try to set the -sc options to
see what happens.  When the parser can't handle the input it will
silently get skipped (for empty lines this is a normal behaviour).

If the parser doesn't behave as you expect take a look at the tracecode,
e.g., './csourceparser -t file.c 2> trace' for full tracecode or
'./csourceparser -vv file.c 2> trace' to see only parser level 3
(C-parser) trace code. Depending on the size of the input file(s) this
could take some time and may occupy some hd-space.

Please refer to the Parse::RecDescent documentation if you get errors
...
Macros in C-Files could not be parsed at this time. Any declarations
and definitions containing macros my cause errors.  Perhaps this
feature could be implemented in the future by using a "real"
...
This program is Copyright 2005 by Hendrik Sirges.  This program is
